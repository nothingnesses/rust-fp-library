...
- **Commit**: `78986ccf1285c5fcd003c846bcbe89c585e73228`
- Use this commit as the reference point for comparing changes during the release process.

## Execution Steps

### Phase 1: Code Cleanup (Removal of v1)

1.  **Extract Brand Definitions**:
    - Move struct definitions (e.g., `OptionBrand`, `VecBrand`) from `fp-library/src/types/*.rs` to `fp-library/src/brands.rs` (or submodules).
    - *Clarification*: `brands.rs` currently re-exports these from `types`. It must be updated to *define* the structs directly, breaking the dependency on `types`.
    - This decouples the brands from the legacy v1 implementations, ensuring they persist after v1 deletion.
...
2.  **Verify `Kind1L0T` for String**:
    - Verify `fp-library/src/types/string.rs` (which is the moved `v2/types/string.rs`) contains the `Kind1L0T` implementation and no feature flags.
