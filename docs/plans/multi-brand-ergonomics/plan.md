...
4. fp-macros test files (integration.rs, apply_integration.rs,
   document_module_tests.rs) needed a `mod dispatch { pub struct Val;
pub struct Ref; }` shim because the Slot blanket references
   `crate::dispatch::Ref` and impl_kind! Slot impls reference
   `crate::dispatch::Val`, which only exist in fp-library.
5. document_module_tests.rs test_cfg_no_conflict module needed a
   manual `Slot_ad6c20556a82a1f0` trait definition alongside its
   existing manual `Kind_ad6c20556a82a1f0` definition.
6. hkt.md doctest changed to `rust,ignore` because impl_kind!
   expansion references `crate::dispatch::Val` which does not
   resolve in doctest context.
