...
- Steps 2 and 3 were combined. Both concern Slot trait generation
...
- fp-macros test files (integration.rs, apply_integration.rs,
...
- document_module_tests.rs test_cfg_no_conflict module needed a
  manual `Slot_ad6c20556a82a1f0` trait definition alongside its
  existing manual `Kind_ad6c20556a82a1f0` definition.
- hkt.md doctest changed to `rust,ignore` because impl_kind!
  expansion references `crate::dispatch::Val` which does not resolve
  in doctest context. The doctest also removes `#[multi_brand]` since
  it shows a simple single-brand case.
