...
- fp-macros test files (integration.rs, apply_integration.rs,
  document_module_tests.rs) needed a `mod dispatch { pub struct Ref; }`
  shim because the Slot blanket references `crate::dispatch::Ref`,
  which only exists in fp-library.
