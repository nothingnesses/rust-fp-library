...
- **Where:**
  ...
  `arc_run.rs:825-826`,
  `arc_run.rs:1067`,
  `run_explicit.rs:632-633`,
  `run_explicit.rs:852`,
  `rc_run_explicit.rs:885-886`,
  `rc_run_explicit.rs:1118`,
  `arc_run_explicit.rs:975-976`,
  `arc_run_explicit.rs:1262`.
...
- `node.rs:65-74`:
  ...
  `arc_run.rs unwrap_first / lift_node helpers`
  documented in
...
| Plan run_accum docstring at `run.rs:580-600`: "Each handler receives the current state by mutable reference inside the closures..."                                                                                                                      | Implementation drops `init` and delegates to `interpret`; the comment about "mutable reference inside the closures" describes user code, not library code (F1).                   |
...
| Interpreter docstring at `interpreter.rs:14-34` describes mono-in-A as a deliberate downgrade from rank-2 NT. The plan never names "rank-2 NT" as a Success criterion, but the rubric (Section 6.1 #3, #4, #6) treats handlers-as-NT as core capability. | Drift is between the rubric and the implementation, not the plan. The plan's choice is internally consistent; the rubric reads it as a real expressiveness loss (F5).             |
