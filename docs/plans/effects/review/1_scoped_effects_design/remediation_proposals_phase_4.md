...
- _What:_ Define a parallel trait at [`fp-library/src/types/effects/interpreter.rs`](../../../../fp-library/src/types/effects/interpreter.rs) (alongside `DispatchHandlers`):
  ```
...
- _What:_ Add a public method on each Run wrapper (six implementations) of approximate shape
  ...
  ```
  Implementation walks the substrate's Free tree analogously to existing `interpret_with` (at [run.rs:885-900](../../../../fp-library/src/types/effects/run.rs#L885-L900)), but does not narrow the row.
- _Cost:_ Larger than A. Six new public methods, each ~50-100 lines, plus per-wrapper tests. The `interpose` primitive is publicly useful (heftia explicitly ships `interposeBy` and `interposeInWith` as user-facing combinators per [`heftia/src/Control/Monad/Hefty/Interpret.hs`](../../../../../effects/heftia/heftia/src/Control/Monad/Hefty/Interpret.hs)), so it is not throwaway machinery. Estimated 600-900 lines for the primitive plus ~400-600 for the scoped-dispatch glue that uses it. Same six-wrapper surface as Option A. Stack-safety on `interpose` requires the same care as `interpret_with`'s host-stack recursion ([run.rs:947-954](../../../../fp-library/src/types/effects/run.rs#L947-L954)).
...
- _Risks:_ `interpose` traversing the Free tree is more subtle than `interpret_with` because it does not narrow the row; the result type stays the same, which means the substrate must reconstruct the same row's nodes after substitution. On the Erased family this composes via `Coyoneda` re-wrapping; on Explicit, it interacts with the HRTB-poisoning workarounds at [arc_run.rs](../../../../fp-library/src/types/effects/arc_run.rs). One unknown is whether the `dyn Fn` continuation closures inside Coyoneda layers can be re-mapped through a substitution without per-layer cloning of the embedded closure; this needs prototyping. Doubles the API surface compared to A.
...
- _Risks:_ Heftia v0.7 itself **moved away** from HFunctor to a freer-monad encoding (`Eff = D.Eff Freer` per [`heftia/src/Control/Monad/Hefty/Types.hs:32`](../../../../../effects/heftia/heftia/src/Control/Monad/Hefty/Types.hs#L32)); adopting HFunctor would diverge from the library that sources the "heftia-inspired" naming. The paper-form HFunctor encoding has known compile-time and inference costs that heftia's authors found prohibitive in practice. Rust's lack of higher-kinded types makes the Brand-pattern hosting of HFunctor structurally messy (each scoped op's brand would need a per-row-shape parameterisation). Reproducing a design heftia tried and abandoned.

**Recommendation: Option A.** It restores heftia's load-bearing semantics (intercept inner `Throw` before the outer handler discharges it), uses the project's existing handler-list machinery and the frunk row-update primitives already in scope, and closes three other findings (M1, M4, M6) in the same change. Option B is option A plus a publicly-visible `interpose` method; that method is genuinely useful but should land as a follow-up after A is proven, not as the first cut. Option C is a wholesale substrate redesign whose justification (HFunctor cleanly supports multi-shot bodies) is real but whose cost is structurally larger than the v1 set's needs warrant, and adopting the original Hefty Algebras encoding would be **less heftia-faithful** than the current freer-monad-encoded plan since heftia v0.7 itself uses Freer. The recommendation overturns part of [resolutions.md F3A:1232](../../resolutions.md#L1232) (the `S = CNilBrand` tightening on the interpret family); that resolution was correct for Phase 3 closure but assumed Phase 4 would specify a parallel mechanism, which this option now does.
...
**Plan revision required:** Yes (large). [plan.md Phase 4 step 3](../../plan.md#L2015-L2017) must expand from one bullet to a full subsection specifying the `DispatchScopedHandlers` trait shape, its required cons-cell impls, and the handler-list interposition mechanism. [decisions.md section 4.5](../../decisions.md#L436-L573) line 481's "fixed `Run<R, A>`" must become "fixed `Run<R, S, A>` (the scoped row stays in scope so that dispatchers can recurse on actions that themselves contain scoped operations)". [resolutions.md F3A](../../resolutions.md#L1232) needs an addendum noting the `S = CNilBrand` bound on the Phase 3 interpret family is lifted in Phase 4 and the new trait subsumes it.
...
- _Risks:_ Per-pointer-brand parameterisation is one of the Phase 3 patterns most prone to HRTB-over-types friction; the `SendStateBrand` work uncovered an [unimplementable Option (b) per-method bound](../../resolutions.md#L55) and required an [option-(c) parallel `SendFunctor` family](../../resolutions.md#L56). Phase 4 likely surfaces analogous walls. Mitigation is documented in [`limitations-and-workarounds.md` Unexpressible Bounds table](../../../../fp-library/docs/limitations-and-workarounds.md), which Phase 4 will need new rows in.

**Option B: Type-aligned queue continuations for scoped closures, mirroring heftia's FTCQueue.**

- _What:_ Replace the single `Box<dyn FnOnce>` closure cell with a queue of cells. Each scoped operation can store a queue of pending "post-body" continuations, allowing left-bind fusion and natural multi-shot via queue traversal. Heftia's [`Data/FTCQueue.hs:38-40`](../../../../../effects/heftia/heftia/src/Data/FTCQueue.hs#L38-L40) defines the type-aligned queue exactly:
  ```
...
- _What:_ Grow each wrapper's [`interpret`](../../../../fp-library/src/types/effects/run.rs#L552-L568) signature from one handler-list parameter to two. The `Node::First` arm dispatches via the existing `DispatchHandlers`; the `Node::Scoped` arm dispatches via the F1-Option-A `DispatchScopedHandlers` trait. Existing call sites that pass only an FO handler list need a `HandlersNil` placeholder for the scoped slot (or the macro emits one).
...
**Restated:** Phase 3's [`interpret_with::<EBrand>`](../../../../fp-library/src/types/effects/run.rs#L885-L900) narrows one FO effect at a time. Phase 4 needs an equivalent for scoped effects, plus an ordering rule for mixed pipelines.
...
- _Cost:_ Medium. Six new per-wrapper methods plus tests, paralleling [`interpret_with`](../../../../fp-library/src/types/effects/run.rs#L885-L900)'s shape on the scoped row.
...
**Cluster 4 (Plan-text imprecision, several minors).** m1, m3, m4, m6, m8 are plan-text edits without semantic implications. They land as a single docs-only commit per [feedback_doc_only_verify.md](../../../../../../../home/jessea/.claude/projects/-home-jessea-Documents-projects-rust-fp-lib/memory/feedback_doc_only_verify.md) (run `just fmt && just doc`, skip the full test suite).

## Holistic Redesigns

### R1. Specify the scoped-dispatch surface explicitly: parallel `DispatchScopedHandlers` trait, two-list `interpret`, handler-list cell substitution.

**Sketch:** A new trait at [`fp-library/src/types/effects/interpreter.rs`](../../../../fp-library/src/types/effects/interpreter.rs) parallel to `DispatchHandlers`:
...
Each scoped-effect cons-cell impl receives the FO handler list and the scoped layer. For `Catch`, the impl uses frunk's `Plucker` (already in scope per [variant_f.rs](../../../../fp-library/src/types/effects/variant_f.rs)) to locate the `ExceptBrand<E>` cell in `fo_handlers`, constructs a substitute handler that returns to a sentinel value, recursively calls `Run::interpret` with the substituted list on `Catch::action`, observes the sentinel, and routes to `Catch::handler`.

Each Run wrapper's [`interpret`](../../../../fp-library/src/types/effects/run.rs#L552-L568) grows a second handler-list parameter; the loop dispatches `Node::First` to the existing `DispatchHandlers::dispatch` and `Node::Scoped` to the new `DispatchScopedHandlers::dispatch_scoped`, passing the FO list along.

**Phase 1-3 invariants preserved:** `Node` enum shape; `Functor`/`SendFunctor`/`WrapDrop`/`RefFunctor`/`Extract` impls on `NodeBrand<R, S>` ([node.rs:84-449](../../../../fp-library/src/types/effects/node.rs#L84-L449)); the six-variant Free family substrate; per-effect `Coyoneda`-wrapping pattern.
...
1. **Heftia v0.7 itself moved to freer-monad encoding** (`Eff = D.Eff Freer` per [`heftia/src/Control/Monad/Hefty/Types.hs:32`](../../../../../effects/heftia/heftia/src/Control/Monad/Hefty/Types.hs#L32)). Adopting HFunctor would diverge from the library that sources the "heftia-inspired" naming, not converge on it. The plan's freer-monad-encoded scoped row is already structurally aligned with current heftia.
...
1. **Plan-text edits closing minors and clarifying decisions (small, ~half a session).** Lift the heftia-divergence note from [resolutions.md:1893-1906](../../resolutions.md#L1893-L1906) into [decisions.md section 4.5](../../decisions.md#L436) and [plan.md Phase 4 header](../../plan.md#L1970). Fix m1 (Run<R, A> vs Run<R, S, A>). Enumerate Phase 4 step 7 compile_fail tests (m8). Add the `Local`/`Reader::Ask` interaction note (M4-Option-A, M6-Option-A). Documentation only; runs `just fmt && just doc` per [feedback_doc_only_verify.md](../../../../../../../home/jessea/.claude/projects/-home-jessea-Documents-projects-rust-fp-lib/memory/feedback_doc_only_verify.md). Conventional commit prefix: `docs(effects)`.
2. **R1 specification commit (medium, a few days).** Write the `DispatchScopedHandlers` trait spec into [plan.md Phase 4 step 3](../../plan.md#L2015-L2017); document handler-list cell substitution; revise the per-wrapper interpret-method shape; reverse the F3A `S = CNilBrand` tightening note in [resolutions.md:1232](../../resolutions.md#L1232) with an addendum referencing this remediation. Documentation only; no code yet. Conventional commit prefix: `docs(effects)`.
3. **R1 implementation phase (large, one to two weeks).** Phase 4 step 1: `ScopedCoproduct<S>` and `Node`-based dispatch. Phase 4 step 1a (new): `DispatchScopedHandlers` trait and per-wrapper interpret rewrite. Lift `S = CNilBrand` from each of the six wrapper interpret-method signatures. Land cons-cell impls for `Catch` and `Span` (the simplest two) plus integration tests. Conventional commit prefix: `feat(effects)`.
