...
Each closure-bearing constructor is parameterised by a pointer brand `P` selecting the storage shape; `P = BoxBrand` (single-shot `Box<dyn FnOnce>`), `P = RcBrand` (multi-shot `Rc<dyn Fn>`), or `P = ArcBrand` (thread-safe multi-shot `Arc<dyn Fn + Send + Sync>`). The parameterisation mirrors Phase 3's [`StateBrand<P, S>`](../../../fp-library/src/brands/effects.rs) pattern; POC-validated at [`fp-library/tests/poc_send_catch_brand.rs`](../../../fp-library/tests/poc_send_catch_brand.rs). For the Arc family a parallel `Send*Brand` sibling bakes `+ Send + Sync` into the dyn bound at definition time, mirroring the Phase 3 [`SendStateBrand`](../../../fp-library/src/brands/effects.rs) pattern.

| Constructor                             | Payload                                                                                                                                   | Models                                                                                            |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `Catch<'a, P, E, A>`                    | `action: Run<R, S, A>`, `handler: <P>::Of<'a, dyn Fn(E) -> Run<R, S, A>>`                                                                 | `Error.catch`, `try`/`catch`.                                                                     |
| `Local<'a, P, E, A>` (Val flavour)      | `modify: <P>::Of<'a, dyn Fn(E) -> E>`, `action: Run<R, S, A>`                                                                             | `Reader.local` (closure consumes the env).                                                        |
| `RefLocal<'a, P, E, A>` (Ref flavour)   | `modify: <P>::Of<'a, dyn Fn(&E) -> E>`, `action: Run<R, S, A>`                                                                            | `Reader.local` (closure borrows the env).                                                         |
| `Bracket<'a, P, A, B>` (Val flavour)    | `acquire: Run<R, S, A>`, `body: <P>::Of<'a, dyn Fn(A) -> Run<R, S, (A, B)>>`, `release: <P>::Of<'a, dyn Fn(A) -> Run<R, S, ()>>`          | Resource management; `P = BoxBrand` for default substrate, `RcBrand` / `ArcBrand` for refcounted. |
| `RefBracket<'a, P, A, B>` (Ref flavour) | `acquire: Run<R, S, A>`, `body: <P>::Of<'a, dyn Fn(P::Of<A>) -> Run<R, S, B>>`, `release: <P>::Of<'a, dyn Fn(P::Of<A>) -> Run<R, S, ()>>` | Resource management on refcounted substrates with the resource shared via pointer clone.          |
| `Span<'a, Tag>`                         | `tag: Tag`, `action: Run<R, S, A>`                                                                                                        | Instrumentation / tracing (no closure; no `P` parameter needed).                                  |

For `BoxBrand` cells the projection is `Box<dyn FnOnce(...)>` (single-shot) rather than `Box<dyn Fn(...)>`; the `BoxBrand` impl of the storage trait selects `FnOnce` to honour the substrate's owned-program semantics. `RefBracket<'a, P, ...>` requires `P` to be a refcounted brand (`RcBrand` or `ArcBrand`); `BoxBrand` does not satisfy `P::Of<A>: Clone` and is rejected at the type level. Users can define their own higher-order effects by implementing the same interpreter trait.
...
- _Dispatcher trait shape and substrate-level `interpose` primitive._ Decision: scoped dispatch is a parallel `DispatchScopedHandlers` trait at [`fp-library/src/types/effects/interpreter.rs`](../../../fp-library/src/types/effects/interpreter.rs) that takes both the scoped layer and the FO handler list:
  ...
  The trait is supported by a substrate-level `Run::interpose<EBrand, Idx>` primitive on each Run wrapper (the Rust analogue of heftia's [`interposeInWith`](https://github.com/sayo-hs/heftia/blob/master/heftia/src/Control/Monad/Hefty/Interpret.hs)) that walks the program tree and substitutes one effect's dispatch with a replacement, without narrowing the row. POC-validated at [`fp-library/tests/poc_rc_run_interpose.rs`](../../../fp-library/tests/poc_rc_run_interpose.rs) on `RcRun` for a one-effect concrete row; generalising to `<EBrand, Idx, R>` mirrors `interpret_with_shared`'s body line for line, with the rebuilt layer's row staying in `R` instead of narrowing to `RMinusE`. For `Catch` specifically, the cons-cell impl calls `Catch::action.interpose::<ExceptBrand<E>, _>(|throw_op| sentinel)`, observes the sentinel via the dispatcher's outer `interpret` loop, and routes to `Catch::handler`; the `fo_handlers` argument is threaded through so re-interpretation of `Catch::action` and `Catch::handler`'s recovery program use the same FO handler set. Each Run wrapper's `interpret` family grows a second handler-list parameter, and the `S = CNilBrand` tightening from the [2026-05-03 F3A reversal](resolutions.md#resolved-2026-05-03-adversarial-review-reversals-delete-run_accum-ship-interpret_with_rec-parameterise-interpret_with-over-refcountedpointer) is lifted; the new trait subsumes the role.

  The substrate-level primitive (Option B in the Phase 4 design review remediation) was selected over the alternative of handler-list cell substitution via frunk's `Plucker` (Option A) because: (i) POC validation exists for B and not for A; (ii) heftia's `interposeInWith` IS Option B's mechanism (`Eff` is data, walk and rewrite) rather than Option A's (handler-list cell substitution at the type level); and (iii) shipping `Run::interpose` as Phase 4 substrate closes the formerly Phase 6+-deferred `interpose` family entry as a side effect.
