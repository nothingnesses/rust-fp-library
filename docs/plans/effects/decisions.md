...
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `Catch<'a, E>`                          | `action: Run<R, S, A>`, `handler: Box<dyn FnOnce(E) -> Run<R, S, A>>`                                                                                          | `Error.catch`, `try`/`catch`.                                                                              |
| `Local<'a, E>` (Val flavour)            | `modify: Box<dyn FnOnce(E) -> E>`, `action: Run<R, S, A>`                                                                                                      | `Reader.local` (closure consumes the env).                                                                 |
| `RefLocal<'a, E>` (Ref flavour)         | `modify: Box<dyn FnOnce(&E) -> E>`, `action: Run<R, S, A>`                                                                                                     | `Reader.local` (closure borrows the env).                                                                  |
| `Bracket<'a, A, B>` (Val flavour)       | `acquire: Run<R, S, A>`, `body: Box<dyn FnOnce(A) -> Run<R, S, (A, B)>>`, `release: Box<dyn FnOnce(A) -> Run<R, S, ()>>`                                       | Resource management (`Run` / `RunExplicit` users; non-refcounted substrate).                               |
| `RefBracket<'a, P, A, B>` (Ref flavour) | `acquire: Run<R, S, A>`, `body: Box<dyn FnOnce(P::Of<A>) -> Run<R, S, B>>`, `release: Box<dyn FnOnce(P::Of<A>) -> Run<R, S, ()>>` where `P: RefCountedPointer` | Resource management (`RcRun` / `ArcRun` / `RcRunExplicit` / `ArcRunExplicit` users; refcounted substrate). |
| `Span<'a, Tag>`                         | `tag: Tag`, `action: Run<R, S, A>`                                                                                                                             | Instrumentation / tracing.                                                                                 |
