...
| Q3  | Bind overhead at increasing depths?       | Approximately linear in the spine walk. Single bind + evaluate over a pre-built `Wrap` spine: 273 ns / 3.95 us / 28.5 us / 277 us at depths 10 / 100 / 1 000 / 10 000. Per-node cost ~27 ns. Acceptable for realistic effect programs. |
...
**Deferred Coyoneda parallel intersections.** The Coyoneda family today ships four variants ([`Coyoneda`](../../../fp-library/src/types/coyoneda.rs), `RcCoyoneda`, `ArcCoyoneda`, [`CoyonedaExplicit`](../../../fp-library/src/types/coyoneda_explicit.rs)). The two missing intersections, `RcCoyonedaExplicit` and `ArcCoyonedaExplicit`, are deferred follow-ups whose rationale is preserved here so the contrast with the Free decision stays explicit:
...
### 4.5 DECISION: Scoped-effect representation via a heftia-inspired dual row

#### Dual-row scoped-effect integration with the Free family

**Heftia-inspired, not heftia-identical.** The current heftia codebase (v0.7) uses a **single** effect list `es` whose elements carry per-element `KnownOrder` (FO or HO) markers, with a `FOEs es` constraint when all members must be first-order (`Eff = D.Eff Freer` at [`heftia/src/Control/Monad/Hefty/Types.hs:32`](https://github.com/sayo-hs/heftia/blob/master/heftia/src/Control/Monad/Hefty/Types.hs#L32)). fp-library takes the **idea** (separate FO vs HO dispatch) and ships a value-level dual row (`Run<R, S, A>` with `Node<R, S> = First | Scoped`); the row encoding diverges from heftia's. The substrate-level mechanism (action stored as a Free-monad-encoded sub-program, handlers walk it via interpose-style rewrites) IS heftia v0.7's mechanism. "Heftia-inspired" is the accurate framing.
...
Each closure-bearing constructor is parameterised by a pointer brand `P` selecting the storage shape, drawing from the existing pointer-abstraction trait family at [`fp-library/docs/pointer-abstraction.md`](../../../fp-library/docs/pointer-abstraction.md):

- `P = BoxBrand` (default substrate, single-shot): closure cell projects to `Box<dyn FnOnce(...)>` via the new [`ToDynFnOnce`](../../../fp-library/src/classes/) trait introduced in Phase 3.5; user-supplied scoped handlers can be FnOnce-natural.
...
The `Run<R, S, A>` rendering in the constructor signatures below is loose notation: the underlying Rust type is `A` (the next-program type bound by the dispatch context), mirroring how Phase 3 `State<'a, P, S, A>` uses `A` for the next-program type rather than a literal `Run<R, S, ?>`.

The substrate's `NodeBrand<R, S>` impls require `S: Functor + SendFunctor + WrapDrop + RefFunctor + Extract`. Each scoped-effect brand provides these impls explicitly (for the substrate's program-traversal machinery) even though the dispatcher trait itself does not go through Functor; the impls are mechanical and parallel Phase 3's per-effect impls.

| Constructor                             | Payload                                                                                                                                                       | Models                                                                                                                                                                                    |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Catch<'a, P, E, A>`                    | `action: Run<R, S, A>`, `handler: <P>::Of<'a, dyn ClosureTrait(E) -> Run<R, S, A>>`                                                                           | `Error.catch`, `try`/`catch`.                                                                                                                                                             |
| `Local<'a, P, E, A>` (Val flavour)      | `modify: <P>::Of<'a, dyn ClosureTrait(E) -> E>`, `action: Run<R, S, A>`                                                                                       | `Reader.local` (closure consumes the env).                                                                                                                                                |
| `RefLocal<'a, P, E, A>` (Ref flavour)   | `modify: <P>::Of<'a, dyn ClosureTrait(&E) -> E>`, `action: Run<R, S, A>`                                                                                      | `Reader.local` (closure borrows the env).                                                                                                                                                 |
| `Bracket<'a, P, A, B>` (Val flavour)    | `acquire: Run<R, S, A>`, `body: <P>::Of<'a, dyn ClosureTrait(A) -> Run<R, S, (A, B)>>`, `release: <P>::Of<'a, dyn ClosureTrait(A) -> Run<R, S, ()>>`          | Resource management; `P = BoxBrand` for default substrate, `RcBrand` / `ArcBrand` for refcounted.                                                                                         |
| `RefBracket<'a, P, A, B>` (Ref flavour) | `acquire: Run<R, S, A>`, `body: <P>::Of<'a, dyn ClosureTrait(P::Of<A>) -> Run<R, S, B>>`, `release: <P>::Of<'a, dyn ClosureTrait(P::Of<A>) -> Run<R, S, ()>>` | Resource management on refcounted substrates with the resource shared via pointer clone.                                                                                                  |
| `Span<'a, Tag>`                         | `tag: Tag`, `action: Run<R, S, A>`                                                                                                                            | Instrumentation / tracing. User-facing Val-only operation; implementation stores action via per-pointer B-thunks per B21 and keeps by-value tags with bounds only where required per B22. |

`RefBracket<'a, P, ...>` requires `P` to be a refcounted brand (`RcBrand` or `ArcBrand`); `BoxBrand` does not satisfy `P::Of<A>: Clone` and is rejected at the type level. Users can define their own higher-order effects by implementing the same interpreter trait.

**Closure-storage ceiling on default `Run`.** User-defined scoped effects requiring multi-shot bodies (Coroutine, Provider, Retry, scoped NonDet with backtracking) are not expressible on the default `Run` and `RunExplicit` wrappers because `BoxBrand`'s `Box<dyn FnOnce>` storage is single-shot. Users requiring multi-shot scoped bodies must use `RcRun` / `ArcRun` / `RcRunExplicit` / `ArcRunExplicit`, whose `RcBrand` / `ArcBrand` storage projects to multi-shot `Rc<dyn Fn>` / `Arc<dyn Fn + Send + Sync>` cells. The standard scoped set (`Catch`, `Local`, `Bracket`, `Span`) is single-shot on every wrapper and is therefore unaffected. Lifting the multi-shot ceiling on the default `Run` family would require introducing FTCQueue-style scoped continuations as a substrate addition; this is deferred to a v2 surface and is not part of Phase 4.

**Why heftia over the alternatives.**

- _vs polysemy's Tactical_: Tactical threads state through an existential functor `f ()` that must be carried end-to-end; every interpreter has to thread it via `runT`/`bindT`/`pureT`. In Rust, this pattern produces a forest of `Box<dyn Any>` and an existential `F` that `rustc` cannot infer. Heftia's dual row avoids the existential entirely; the action payload is a concrete `Run<R, S, A>`.
...
- _Interpreter continuation type._ Decision: fixed `Run<R, S, A>`. The trait method that interprets a scoped effect returns a `Run<R, S, A>` parameterised generically by `R`, `S`, and `A`; the answer type and effect row can change via the trait method's signature, but the wrapper stays `Run` and the scoped row `S` stays in scope so dispatchers can recurse on actions that themselves contain further scoped operations. Handlers that escape to non-`Run` values (e.g., a `runPure: Run<Void, A> -> A` analogue) live as separate free functions outside the trait, mirroring PureScript Run's split between `interpret` (Run-to-Run) and `run`/`runPure` (Run-to-bare-value). This matches every Haskell library the port can credibly imitate (heftia, in-other-words, PureScript Run): row-narrowing is expressed via the handler's signature, not via a per-impl associated type. An associated continuation type is deferred to a v2 if a real use case surfaces a handler that genuinely needs different per-impl output kinds inside the trait; until then, the simpler shape is preferred.
- _Dispatcher trait shape and substrate-level `interpose` primitive._ Decision: scoped dispatch is a parallel `DispatchScopedHandlers` trait at `fp-library/src/types/effects/interpreter.rs` that takes both the scoped layer and the FO handler list:
  ...
  The trait is supported by two substrate-level primitives on each Run wrapper:
  - `Run::interpose<EBrand, Idx>` (the Rust analogue of heftia's [`interposeInWith`](https://github.com/sayo-hs/heftia/blob/master/heftia/src/Control/Monad/Hefty/Interpret.hs)): walks the program tree and substitutes one effect's dispatch with a replacement, without narrowing the row. POC-validated at `fp-library/tests/poc_rc_run_interpose.rs`.

  - `interpret_with_either<EBrand, Idx>`: specialisation of `interpret_with` returning `Either<A, EBrand::Op>` instead of narrowing the row, short-circuiting at the matched effect. Used by the `Catch` cons-cell impl to structurally distinguish "action completed with `A`" from "action threw `EBrand::Op`":
    ...
    No interior mutability; no placeholder-program sentinel; the type system distinguishes the two outcomes via the `Either` variant. To be POC-validated as POC 3.

  Each Run wrapper's `interpret` family grows a second handler-list parameter, and the `S = CNilBrand` tightening from the [2026-05-03 F3A reversal](resolutions.md#resolved-2026-05-03-adversarial-review-reversals-delete-run_accum-ship-interpret_with_rec-parameterise-interpret_with-over-refcountedpointer) is lifted; the new trait subsumes the role.

  The substrate-level primitive approach was selected over the alternative of handler-list cell substitution via frunk's `Plucker` because: (i) POC validation exists for the substrate path and not for the Plucker path; (ii) heftia's `interposeInWith` IS the substrate-level mechanism (`Eff` is data, walk and rewrite) rather than handler-list cell substitution at the type level; and (iii) shipping `Run::interpose` as Phase 4 substrate closes the formerly Phase 6+-deferred `interpose` family entry as a side effect.
...
- _Concrete payload shapes for the standard scoped constructors._ Decision: `Catch<'a, E>` and `Span<'a, Tag>` carry the day-one `'a` parameter from the previous sub-decision; the table above lists every constructor's full payload. `Bracket` and `Local` ship in two parallel flavours (Val and Ref) that mirror the library's existing Val/Ref dispatch pattern documented at [`fp-library/docs/dispatch.md`](../../../fp-library/docs/dispatch.md); a single user-facing smart constructor (`bracket` / `local`) uses closure-driven dispatch to pick the right flavour at the call site, exactly the way `dispatch::functor::map` already does for `Functor` versus `RefFunctor`.
  ...
  **Bracket: Val and Ref flavours.** The Val flavour `Bracket<'a, A, B>` has `body: Box<dyn FnOnce(A) -> Run<R, S, (A, B)>>` and `release: Box<dyn FnOnce(A) -> Run<R, S, ()>>`. The body consumes `A`, threads it back to the interpreter via `(A, B)`, and the interpreter moves the returned `A` into `release`. The Ref flavour `RefBracket<'a, P, A, B>` has `body: Box<dyn FnOnce(P::Of<A>) -> Run<R, S, B>>` and `release: Box<dyn FnOnce(P::Of<A>) -> Run<R, S, ()>>` where `P: RefCountedPointer` is `RcBrand` for `RcRun`/`RcRunExplicit` users and `ArcBrand` for `ArcRun`/`ArcRunExplicit` users. Body and release both receive a pointer clone, the resource lives until the last clone drops, and PureScript's GC-aliased semantics from [`bracket`](https://github.com/purescript-contrib/purescript-aff/blob/master/src/Effect/Aff.purs#L308) port directly.
  ...
  **Why not the originally drafted `body: Box<dyn FnOnce(&A) -> Run<R, S, B>>` shape.** That shape looked like the Rust-faithful translation of PureScript's `(a -> Aff b)`, but it collapses into a useless signature on closer inspection: the returned `Run<R, S, B>` outlives the `&A` borrow (it is stored in a scoped-effect node and walked by the interpreter long after the body call has returned), so any closure inside the `Run` that captured `&A` would dangle. Effective bodies would be restricted to "look at `&A` synchronously, copy out `Copy` fields, return a `Run` whose closures reference nothing." Almost every real bracketed body wants the resource live throughout the body's effectful tree (e.g., reading from a `BufReader<File>` across nested binds), so this shape is rejected.
  ...
  **Why `Catch` does not get a Ref flavour.** A symmetric `RefCatch<'a, E>` with `handler: Box<dyn FnOnce(&E) -> Run<R, S, A>>` would hit the same lifetime trap as the rejected `Bracket` shape: the returned `Run<R, S, A>` outlives the borrow. Errors in the surveyed languages (Haskell, PureScript) are owned values and rarely the kind of multi-ton handle a bracket guards. The Val-only signature `Catch<'a, E>` with `handler: Box<dyn FnOnce(E) -> Run<R, S, A>>` is correct here; users with non-`Clone` errors that they want to inspect without consuming can wrap them in `Rc` themselves.

  **Why `Span` does not split at the user-facing dispatch level.** No user closure is dispatched over, so there is no Val/Ref flavour split. Implementation clarification from B21: the action program is still stored behind a unit-argument B-thunk to avoid recursive layout cycles, so the substrate uses Box/Rc/Arc sibling cells and brands mirroring Catch and Local. This pointer split is an implementation-level storage detail, not a Ref dispatch flavour. Tag-bound clarification from B22: tags stay stored by value. Default Box-backed cells keep non-`Clone` tags available; Rc paths require `Tag: Clone` where the substrate needs cloneable cells; Arc paths require `Tag: Clone + Send + Sync` where the substrate needs cloneable, thread-safe cells.

  **Bracket panic safety.** The bracket dispatcher constructs a `BracketGuard<A, F>` (or `BracketGuard<P::Of<A>, F>` for the Ref flavour) holding the resource and the `release` closure, with a `Drop` impl that invokes `release` synchronously. `body` is invoked through the guard, so on panic during `body` the unwinding path runs `release` via `Drop` before the unwind continues. `release`'s synchronous-Drop invocation cannot itself perform effects in the row (the interpreter is not running during unwind); effectful release on panic is best-effort, and users who require fully-effectful cleanup on panic should layer their own `Drop`-impl on top of the resource. This is the Rust-conventional answer; alternatives considered (`std::panic::catch_unwind` wrapping, panic-leakiness with documentation) were rejected because Drop-based cleanup is what every `std` resource type does and is what users expect from idiomatic Rust APIs.
...
**Decision: macro DSL primary, builder fallback.** Users assemble natural transformations via a macro (`handlers!{ state: handle_state, reader: handle_reader }`) that expands to the appropriate per-effect tuple-of-closures matching the row's type-level structure. The natural-order builder pattern (`handlers_ordered().on::<State<i32>>(handle_state).on::<Reader<Env>>(handle_reader).finish()`) remains available as the fallback for users who want to bypass the macro or compose handlers programmatically. The lower-level `nt().prepend::<E>(handler)` seed remains available for code that needs to construct the cons-list representation directly. This mirrors section 4.1's workaround 1 + workaround 3 hybrid (macro primary, mechanical fallback for hand-authoring) and section 4.1's row-narrowing handler API note: the consistent design across the plan is "macro for ergonomics, type-level building blocks for escapes".
...
- **Smart constructors.** `ask`, `get`, `put`, `modify`, `tell`, `throw`, `catch`. Each is a thin wrapper over `inj + liftF`/`send`. The concrete signature for the row-aware `lift` combinator (inherent associated function on each Run wrapper, raw-effect input, full Coyoneda-lift / inject / `Node::First` / `send` chain inside the body, named `lift` to match PureScript Run's [`Run.lift`](https://github.com/natefaubion/purescript-run/blob/main/src/Run.purs)) is locked in by [plan.md Phase 2 step 9](plan.md) and the [2026-04-28 resolution](resolutions.md#resolved-2026-04-28-phase-2-step-9-scope-is-under-specified).
...
- **Add `RcFree<F, A>` at `fp-library/src/types/rc_free.rs`.** Follow the `rc_coyoneda.rs` template. Steps:
...
- **Add `ArcFree<F, A>` at `fp-library/src/types/arc_free.rs`.** Follow the `arc_coyoneda.rs` template. Steps:
