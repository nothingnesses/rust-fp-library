...
The `Run<R, S, A>` rendering in the constructor signatures below is loose notation: the underlying Rust type is `A` (the next-program type bound by the dispatch context), mirroring how Phase 3 [`State<'a, P, S, A>`](../../../fp-library/src/types/effects/state.rs) uses `A` for the next-program type rather than a literal `Run<R, S, ?>`.

The substrate's [`NodeBrand<R, S>`](../../../fp-library/src/types/effects/node.rs#L84-L449) impls require `S: Functor + SendFunctor + WrapDrop + RefFunctor + Extract`. Each scoped-effect brand provides these impls explicitly (for the substrate's program-traversal machinery) even though the dispatcher trait itself does not go through Functor; the impls are mechanical and parallel Phase 3's per-effect impls.
...
- _Dispatcher trait shape and substrate-level `interpose` primitive._ Decision: scoped dispatch is a parallel `DispatchScopedHandlers` trait at [`fp-library/src/types/effects/interpreter.rs`](../../../fp-library/src/types/effects/interpreter.rs) that takes both the scoped layer and the FO handler list:
  ...
  - `Run::interpose<EBrand, Idx>` (the Rust analogue of heftia's [`interposeInWith`](https://github.com/sayo-hs/heftia/blob/master/heftia/src/Control/Monad/Hefty/Interpret.hs)): walks the program tree and substitutes one effect's dispatch with a replacement, without narrowing the row. POC-validated at [`fp-library/tests/poc_rc_run_interpose.rs`](../../../fp-library/tests/poc_rc_run_interpose.rs).

  - `interpret_with_either<EBrand, Idx>`: specialisation of [`interpret_with`](../../../fp-library/src/types/effects/run.rs#L885-L900) returning `Either<A, EBrand::Op>` instead of narrowing the row, short-circuiting at the matched effect. Used by the `Catch` cons-cell impl to structurally distinguish "action completed with `A`" from "action threw `EBrand::Op`":
