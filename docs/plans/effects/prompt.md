...
- **Phase 4** (scoped effects via heftia-inspired dual row): steps 0-1 plus step 2 (sub-steps 2.1-2.6) plus step 2a plus step 3.1 (sub-steps 3.1.1-3.1.4) plus the full Local cycle (Val 3.2.1-3.2.4 plus Ref 3.2.5-3.2.8) plus **steps 3.3.1 + 3.3.2 + 3.3.3 + 3.3.4 + 3.3.5 (Bracket Val foundational scaffold + RefFunctor + smart constructors + integration tests, then RefBracket Ref foundational scaffold) plus the B19 closure substrate split** shipped. B14 / B15 / B16 / B17 / B18 / B19 / B20 all closed (resolution entries in [resolutions.md](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/resolutions.md)). Step 3.3.5 (this branch) lands [`fp-library/src/types/effects/ref_bracket.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/ref_bracket.rs); 4 cells per the closed B15 and B19 split (`RefBracket` / `SendRefBracket` over `RcFree` / `ArcFree`, plus `RefBracketExplicit` / `SendRefBracketExplicit` over `RcFreeExplicit` / `ArcFreeExplicit`) and four new brands in [`brands/effects.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/brands/effects.rs). Acquire remains a B-thunk per B14; body and release receive `Rc<A>` / `Arc<A>` resource pointer clones, matching decisions.md's rejected-`&A` rationale and correcting the previous short-form wording. Foundational trait impls only: Functor identity on Rc brands, SendFunctor identity on Arc brands plus Rc stubs, WrapDrop None, Extract panic-stub; `RefFunctor` deferred to step 3.3.6. Step 2a closure delivered the substrate-level `interpret_with_either<EBrand, Idx, RMinusE>` primitive across all six Run wrappers; 24 integration tests plus 6 doctests. The Phase 4 design review ([`review/1_scoped_effects_design/review_phase_4_design.md`](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/review/1_scoped_effects_design/review_phase_4_design.md)) and its remediation report ([`review/1_scoped_effects_design/remediation_proposals_phase_4.md`](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/review/1_scoped_effects_design/remediation_proposals_phase_4.md)) shipped earlier, with three POC validations now complete ([`poc_send_catch_brand.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_send_catch_brand.rs) for the F2 parallel-Send-brand pattern; [`poc_rc_run_interpose.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpose.rs) for the F1 substrate-level `Run::interpose` primitive; [`poc_rc_run_interpret_with_either.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs) for the B4 substrate-level `interpret_with_either` primitive). The 2026-05-06 K1 / K2 [implementation-kickoff sequencing resolution](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries) set POC 3's standalone-commit-first ordering and plan.md step numbering as the authoritative commit boundary. Phase 4 ships `Catch<'a, P, E, A>`, `Local<'a, P, E, A>` / `RefLocal`, `Bracket<'a, P, A, B>` / `RefBracket`, and `Span<'a, Tag>` scoped-effect constructors; a parallel `DispatchScopedHandlers` trait; substrate-level `Run::interpose` and `interpret_with_either` primitives on each Run wrapper. **Next greenfield step**: step 3.3.6 (RefBracket Ref `RefFunctor` impls + brand-projection helpers if needed).
...
`1be2af3e` under Option A fallback; Bracket Val `RefFunctor`
(3.3.2) shipped at `46754fc0`; Phase 4 step 3.3.1 B19 closure
(foundational-scaffold substrate split per Free family) shipped
at `2e7a9048`; step 3.3.3 (per-wrapper smart constructors) shipped
at `b13fce28`; step 3.3.4 (Bracket Val integration tests) shipped
at `28046f25`. **Phase 4 step 3.3.5 (RefBracket Ref foundational
scaffold) shipped on this branch**: new
[`fp-library/src/types/effects/ref_bracket.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/ref_bracket.rs)
plus four brands at
[`fp-library/src/brands/effects.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/brands/effects.rs).
The scaffold has 4 cells per B15 and B19: `RefBracket` /
`SendRefBracket` over `RcFree` / `ArcFree`, plus
`RefBracketExplicit` / `SendRefBracketExplicit` over
`RcFreeExplicit` / `ArcFreeExplicit`. Acquire remains a B-thunk
per B14. Body and release receive `Rc<A>` / `Arc<A>` pointer
clones, matching decisions.md's rejected-`&A` rationale and
correcting the prior short-form wording. Foundational trait impls
only: Functor identity on Rc brands, SendFunctor identity on Arc
brands plus Rc stubs, WrapDrop None, Extract panic-stub.
`RefFunctor` is deferred to 3.3.6. **Next greenfield step**:

**Step 3.3.6** (RefBracket Ref `RefFunctor` impls + brand-projection
helpers if needed): clone-based `RefFunctor` for
`RefBracketBrand<RcBrand, Sub, A, B>` and
`RefBracketExplicitBrand<RcBrand, Sub, A, B>` if the Option A GAT
projection keeps normalising cleanly; expected deliberate omission
for `SendRefBracketBrand` and `SendRefBracketExplicitBrand`.
Subsequent sub-steps mirror the Bracket Val cycle: 3.3.7
per-wrapper `ref_bracket` smart constructors (4 wrappers per B15),
3.3.8 integration tests at `tests/run_ref_bracket.rs`.
...
B14 / B15 / B16, of which 3.3.1-3.3.5 have now shipped under
Option A plus the B19 per-Free-family split), 3.4 Span. R2
...
- **Phase 4 step 3.3.5** (this commit): RefBracket Ref foundational scaffold at [`fp-library/src/types/effects/ref_bracket.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/ref_bracket.rs); 4 cells per B15 and B19 (`RefBracket` / `SendRefBracket` over `RcFree` / `ArcFree`, plus `RefBracketExplicit` / `SendRefBracketExplicit` over `RcFreeExplicit` / `ArcFreeExplicit`); four new brands at [`fp-library/src/brands/effects.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/brands/effects.rs). Acquire is a B-thunk per B14; body and release receive `Rc<A>` / `Arc<A>` resource pointer clones per decisions.md's rejected-`&A` rationale. Foundational trait set only: Functor identity on Rc brands, SendFunctor identity on Arc brands plus Rc stubs, WrapDrop None, Extract panic-stub. `RefFunctor` deferred to step 3.3.6. Deviation logged at [deviations.md Phase 4 step 3.3.5](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/deviations.md).
- **Phase 4 step 3.3.4** (`28046f25`): Bracket Val integration tests at [`fp-library/tests/run_bracket.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_bracket.rs); 23 shape-only tests across 5 Run wrappers (Run, RcRun, RunExplicit, RcRunExplicit, ArcRunExplicit). `ArcRun::bracket` skipped per the closed B20 closure entry; end-to-end exercise deferred to step 8's bracket-dispatcher tests with escalation to step 8a `SendBracketBrand` redesign if still blocked. Each section uses the marker-struct workaround validated by [poc_bracket_marker_row.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_bracket_marker_row.rs); module-level `#![recursion_limit = "512"]` for ArcRunExplicit's Send + Sync trait-resolution depth. T1 verifies suspended-shape; T2 / T3 / T4 destructure the cell and exercise acquire / body / release at the cell level (manual wrapping via each wrapper's `from_*` constructor is required because Option A's identity `Functor::map` on `*BracketBrand` does not rewrite cell continuations during `peel`); T5 multi-shot clone covers RcRun / RcRunExplicit / ArcRunExplicit. ArcRunExplicit section delegates `SendFunctor` instead of `Functor`; BoxBracketExplicit's substrate uses `*acquire(())` deref because the cell stores `Box<FreeExplicit<...>>`.
- **Phase 4 step 3.3.3** (`b13fce28`): six per-wrapper Bracket Val smart constructors at run.rs / rc_run.rs / arc_run.rs / run_explicit.rs / rc_run_explicit.rs / arc_run_explicit.rs; each pairs the wrapper with its substrate-correct cell from B19 closure (`Run` -> `BoxBracket`, `RcRun` -> `Bracket`, `ArcRun` -> `SendBracket`, `RunExplicit` -> `BoxBracketExplicit`, `RcRunExplicit` -> `BracketExplicit`, `ArcRunExplicit` -> `SendBracketExplicit`). Marker-struct doctests for 4 of 6 wrappers; ArcRun::bracket's doctest uses `ignore` block + simpler runnable assertion (rustc overflow on Send check via marker-struct + SendBracketBrand's GAT-Send-Sync bound). `BoxBracketExplicit` substrate refined to `Box<FreeExplicit<...>>`. 5 new smart-constructor doctests (1 ignored). Deviation logged at [deviations.md Phase 4 step 3.3.3](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/deviations.md).
- **Phase 4 step 3.3.1 B19 closure** (`2e7a9048`): foundational-scaffold substrate split per Free family. Existing 3 cells fixed for substrate-correctness (`Bracket` (RcBrand) -> `RcFree<Sub, _>`; `SendBracket` (ArcBrand) -> `ArcFree<Sub, _>` with GAT-projection-Send-Sync bound on `Sub`; `BoxBracket` keeps `Free<Sub, _>`). Three new Explicit-family cells `BoxBracketExplicit` / `BracketExplicit` / `SendBracketExplicit` (storing `FreeExplicit<'a, Sub, _>` / `RcFreeExplicit<'a, Sub, _>` / `ArcFreeExplicit<'a, Sub, _>`); three new brand declarations; per-brand trait impls (Functor identity, SendFunctor identity-or-stub, WrapDrop None, Extract panic-stub, RefFunctor stub-or-clone). 30 bracket.rs doctests pass. Send-family doctests use `IdentityBrand` substrate (not `ThunkBrand`) to satisfy `ArcFree`'s GAT-projection-Send-Sync bound. Closes B19. Deviation logged at [deviations.md Phase 4 step 3.3.1 B19 closure](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/deviations.md).
- **Phase 4 step 3.3.2** (`46754fc0`): Bracket Val `RefFunctor` impls on `BoxBracketBrand<BoxBrand, Sub, A, B>` (panicking stub) and `BracketBrand<RcBrand, Sub, A, B>` (one-line `fa.clone()`); `SendBracketBrand` deliberately omits `RefFunctor`. No brand-projection helpers needed (Option A's GAT projection is independent of the trait's universal X, so the WF/normalization issue Catch/Local/RefLocal hit doesn't apply). 2 new doctests. Deviation logged at [deviations.md Phase 4 step 3.3.2](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/deviations.md).
