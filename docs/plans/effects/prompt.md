...
- **Phase 4** (scoped effects via heftia-inspired dual row): steps 0-1 plus all of step 2 (sub-steps 2.1-2.6) shipped; step 2a in-flight. Step 2 closure delivers the substrate-level `interpose<EBrand, Idx, RMinusE, EmbedIndices>` primitive across all six Run wrappers ([rc_run.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/rc_run.rs), [run.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/run.rs), [arc_run.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/arc_run.rs), [run_explicit.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/run_explicit.rs), [rc_run_explicit.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/rc_run_explicit.rs), [arc_run_explicit.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/arc_run_explicit.rs)) plus 24 integration tests at [run_interpose.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_interpose.rs) (4 per wrapper) and 12 doctests. Steps 2a-8 follow. The Phase 4 design review ([`review/1_scoped_effects_design/review_phase_4_design.md`](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/review/1_scoped_effects_design/review_phase_4_design.md)) and its remediation report ([`review/1_scoped_effects_design/remediation_proposals_phase_4.md`](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/review/1_scoped_effects_design/remediation_proposals_phase_4.md)) shipped earlier, with three POC validations now complete ([`poc_send_catch_brand.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_send_catch_brand.rs) for the F2 parallel-Send-brand pattern; [`poc_rc_run_interpose.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpose.rs) for the F1 substrate-level `Run::interpose` primitive; [`poc_rc_run_interpret_with_either.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs) for the B4 substrate-level `interpret_with_either` primitive). The 2026-05-06 K1 / K2 [implementation-kickoff sequencing resolution](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries) set POC 3's standalone-commit-first ordering and plan.md step numbering as the authoritative commit boundary. Phase 4 ships `Catch<'a, P, E, A>`, `Local<'a, P, E, A>` / `RefLocal`, `Bracket<'a, P, A, B>` / `RefBracket`, and `Span<'a, Tag>` scoped-effect constructors; a parallel `DispatchScopedHandlers` trait; substrate-level `Run::interpose` and `interpret_with_either` primitives on each Run wrapper.

### Next greenfield work

Phase 4 step 2 closed (sub-steps 2.1-2.6 shipped). The next
concrete commit is **Phase 4 step 2a**: generalising the
[POC 3-validated `interpret_with_either<EBrand, Idx>(self, fo_handlers) -> Result<A, EBrand::Op>`
substrate primitive](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs)
across all six Run wrappers, mechanically following the same
per-wrapper bound shapes that step 2 navigated for `interpose`.
Each wrapper's `interpret_with_either` body short-circuits on
the matched effect (returns `Err(op)`) and dispatches the
unmatched arm through the user-supplied first-order handler
list (returns `Ok(a)` once pure). The `Catch` cons-cell in
step 4 consumes this primitive: a `Catch` handler installs
`interpret_with_either::<ExceptBrand<E>, _>` to test the body,
and on `Err(throw)` invokes the recovery program. Default to
one bundled commit covering all six wrappers (low per-wrapper
risk; bound shapes well-understood from step 2's empirical
validation); fall back to six sub-step commits if per-wrapper
specifics surface. Steps 3-8 follow. Phase 4 then proceeds through plan.md
...
- **Phase 4 step 2.6**: `ArcRunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the thread-safe explicit-lifetime Run wrapper at [`fp-library/src/types/effects/arc_run_explicit.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/arc_run_explicit.rs). Union of 2.3's Send+Sync bound surface (closure `+ Send + Sync + 'a`, `<ArcBrand as RefCountedPointer>::new`, `SendFunctor::send_map` traversal, `A: Clone + Send + Sync`, multiple substrate-clone projection bounds) and 2.4 / 2.5's `'a`-lifetime threading (`ArcFreeExplicit<'a, ...>` substrate, closure `+ 'a` instead of `+ 'static`). Closes Phase 4 step 2 (all six wrappers ship `interpose`). R1 again did not surface, mirroring `ArcRunExplicit::interpret_with`'s direct-literal style: no `unwrap_first` / `make_node_first` / `wrap_first_arc` workaround helpers required, but a much longer explicit `Send + Sync` projection bound list per method (~6 extra projection bounds) declared inline. Four parallel integration tests added at [`run_interpose.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_interpose.rs) under a `// -- ArcRunExplicit --` divider, prefixed `arc_run_explicit_t1`-`arc_run_explicit_t4`; total tests in the file now 24 across 6 wrappers, plus 12 doctests. `just verify` clean.
