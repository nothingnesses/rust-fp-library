...
- **Phase 4** (scoped effects via heftia-inspired dual row): steps 0-1 plus steps 2.1-2.4 shipped; step 2.5 in-flight (POC 3 validation at [`poc_rc_run_interpret_with_either.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs); `ScopedCoproduct<H, T>` and `ScopedNil` row-encoding aliases at [`scoped.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/scoped.rs); `RcRun::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`rc_run.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/rc_run.rs); `Run::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`run.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/run.rs); `ArcRun::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`arc_run.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/arc_run.rs); `RunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`run_explicit.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/run_explicit.rs); `RcRunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`rc_run_explicit.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/rc_run_explicit.rs)). Step 2 splits into one sub-step per wrapper (2.1 RcRun, 2.2 Run, 2.3 ArcRun, 2.4 RunExplicit, 2.5 RcRunExplicit, 2.6 ArcRunExplicit) per the per-step protocol's splitting clause. Steps 2a-8 follow. The Phase 4 design review ([`review/1_scoped_effects_design/review_phase_4_design.md`](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/review/1_scoped_effects_design/review_phase_4_design.md)) and its remediation report ([`review/1_scoped_effects_design/remediation_proposals_phase_4.md`](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/review/1_scoped_effects_design/remediation_proposals_phase_4.md)) shipped earlier, with three POC validations now complete ([`poc_send_catch_brand.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_send_catch_brand.rs) for the F2 parallel-Send-brand pattern; [`poc_rc_run_interpose.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpose.rs) for the F1 substrate-level `Run::interpose` primitive; [`poc_rc_run_interpret_with_either.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs) for the B4 substrate-level `interpret_with_either` primitive). The 2026-05-06 K1 / K2 [implementation-kickoff sequencing resolution](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries) set POC 3's standalone-commit-first ordering and plan.md step numbering as the authoritative commit boundary. Phase 4 ships `Catch<'a, P, E, A>`, `Local<'a, P, E, A>` / `RefLocal`, `Bracket<'a, P, A, B>` / `RefBracket`, and `Span<'a, Tag>` scoped-effect constructors; a parallel `DispatchScopedHandlers` trait; substrate-level `Run::interpose` and `interpret_with_either` primitives on each Run wrapper.

### Next greenfield work

Phase 4 steps 0-1 plus 2.1-2.4 shipped; step 2.5
(`RcRunExplicit::interpose`) in-flight. The next concrete commit
is **Phase 4 step 2.6**: `ArcRunExplicit::interpose` on the
thread-safe explicit-lifetime Run wrapper, mechanically following
the union of 2.3's `ArcRun::interpose` Send+Sync bound surface
and HRTB-poisoning workaround helpers with 2.4's `RunExplicit`
`'a`-lifetime threading. R1 + R2 risks surface together;
expected to require ArcRunExplicit-specific HRTB workaround
helpers analogous to ArcRun's (the Send+Sync GAT projection
bound on `ArcRunExplicit`'s `NodeBrand` is what triggers the
poisoning, not the `'a` lifetime). After 2.6 closes step 2,
step 2a generalises the [`interpret_with_either`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs)
substrate primitive across all six wrappers in parallel. Phase 4 then proceeds through plan.md
