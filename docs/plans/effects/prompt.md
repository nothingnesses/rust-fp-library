...
- **Phase 4** (scoped effects via heftia-inspired dual row): steps 0-1 plus step 2 (sub-steps 2.1-2.6) plus step 2a plus step 3.1.1 plus step 3.1.2 shipped; **step 3.1.3 (Catch smart constructors per wrapper, plus B-thunk action-representation refactor in catch.rs) in the working tree, ready to commit**. Step 3.1.2 shipped at `5bb2d1ae`. B7 (Catch action-field layout cycle when embedded in substrate) closed via B-thunk with unit-arg `Fn(()) -> A` form per the [resolved B7 entry](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/resolutions.md#resolved-2026-05-07-phase-4-step-3.1.3-catch-action-field-layout-cycle-b7-closed): action and handler fields are both per-pointer-brand pointer-of-thunk projections leveraging the existing pointer-abstraction `ToDyn*Fn::new` family per [`pointer-abstraction.md`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/docs/pointer-abstraction.md)'s `(pointer-capability, closure-semantic)` matrix. Step 3.1.1 closure landed the foundational scaffold (3 sibling types, 3 brands, 4 of 5 substrate-required trait impls per brand) at `abd3d1a3`. Step 3.1.2 adds `RefFunctor` to `BoxCatchBrand<BoxBrand, E>` and `CatchBrand<RcBrand, E>` via three `#[doc(hidden)]` brand-projection helpers (`box_catch_action_ref` / `catch_action_ref` / `catch_handler_ref`) escaping the trait impl's HRTB scope so the GAT projection normalizes (B5 Option A); `SendCatchBrand` omits `RefFunctor` because the cascade through `ArcRunExplicitBrand` does not require it (`ArcFreeExplicitBrand: !RefFunctor` per the brand's docs) and a hypothetical impl would face the same `Send + Sync` bound mismatch on `func` that prevents `Functor`. Step 2a closure delivered the substrate-level `interpret_with_either<EBrand, Idx, RMinusE>(self, fo_handlers) -> Result<A, EBrand::Of<'a, Self>>` primitive across all six Run wrappers, generalising [POC 3's](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs) concrete two-effect-row template. 24 integration tests at [run_interpret_with_either.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_interpret_with_either.rs) (4 per wrapper) plus 6 doctests. Steps 3.1.3 onwards follow. The Phase 4 design review ([`review/1_scoped_effects_design/review_phase_4_design.md`](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/review/1_scoped_effects_design/review_phase_4_design.md)) and its remediation report ([`review/1_scoped_effects_design/remediation_proposals_phase_4.md`](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/review/1_scoped_effects_design/remediation_proposals_phase_4.md)) shipped earlier, with three POC validations now complete ([`poc_send_catch_brand.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_send_catch_brand.rs) for the F2 parallel-Send-brand pattern; [`poc_rc_run_interpose.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpose.rs) for the F1 substrate-level `Run::interpose` primitive; [`poc_rc_run_interpret_with_either.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs) for the B4 substrate-level `interpret_with_either` primitive). The 2026-05-06 K1 / K2 [implementation-kickoff sequencing resolution](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries) set POC 3's standalone-commit-first ordering and plan.md step numbering as the authoritative commit boundary. Phase 4 ships `Catch<'a, P, E, A>`, `Local<'a, P, E, A>` / `RefLocal`, `Bracket<'a, P, A, B>` / `RefBracket`, and `Span<'a, Tag>` scoped-effect constructors; a parallel `DispatchScopedHandlers` trait; substrate-level `Run::interpose` and `interpret_with_either` primitives on each Run wrapper.

### Next greenfield work

Phase 4 steps 2, 2a, 3.1.1, and 3.1.2 closed. **Phase 4 step 3.1.3
(Catch smart constructors per wrapper, plus B-thunk action-representation
refactor in catch.rs) is in the working tree, ready to commit**.
Six smart constructors land at the six Run wrappers (`Run::catch`,
`RcRun::catch`, `ArcRun::catch`, `RunExplicit::catch`,
`RcRunExplicit::catch`, `ArcRunExplicit::catch`); each takes
`(action, handler)` and returns a `Run`-flavoured program suspended
at the scoped Catch layer (verified via `peel().is_err()`
doctests). All three Catch sibling enums updated for the B-thunk
action representation: `BoxCatch::action: Box<dyn 'a + FnOnce(()) -> A>`,
`Catch::action: Rc<dyn 'a + Fn(()) -> A>`,
`SendCatch::action: Arc<dyn 'a + Fn(()) -> A + Send + Sync>`. Manual
`Clone` impls added for `Catch` and `SendCatch` (Rc/Arc-bump on
both action and handler pointers); `BoxCatch` does not impl `Clone`.
ArcRun gains a `make_node_scoped` HRTB-poisoning workaround helper.
Step 3 splits per the per-step protocol: 3.1 Catch (sub-split per B6),
3.2 Local / RefLocal, 3.3 Bracket / RefBracket, 3.4 Span. R2 risk (Send+Sync threading through
scoped-effect closure cells) surfaces here. Step 4 introduces
the `DispatchScopedHandlers` trait + per-wrapper interpret
rewrite (R1 implementation kickoff). Step 5 lands the
`scoped_effects!` and `scoped_handlers!` macros. Step 6
implements the bracket dispatcher with Drop-guard for panic
safety (M3). Step 7 lands standard scoped-handler implementations
consuming `interpret_with_either` (Catch's recovery path) and
`interpose` (Local's environment-replacement path). Step 8
closes Phase 4 with the review-remediation documentation pass. Phase 4 then proceeds through plan.md
steps 2a, 3, 4, 5, 6, 7, 8 in order with plan.md numbering as
the authoritative commit boundary (per K2): substrate-level
`Run::interpose` (POC 2 validated) and `interpret_with_either`
(POC 3 in step 0) primitives, `DispatchScopedHandlers` trait

- per-wrapper interpret rewrite (R1 implementation); standard
  scoped-effect constructors using the Phase 3.5 retrofit's
  pointer-brand pattern (R2 implementation); bracket dispatcher
  with Drop-guard for panic safety (M3); standard scoped-effect
  rollout (`Catch`, `Local` / `RefLocal`, `Bracket` / `RefBracket`,
  `Span` plus `scoped_effects!` and `scoped_handlers!` macros via
  the new `handler_list_emitter` helper module shared with
  `handlers!`); standard scoped handlers; review-remediation
  documentation pass closing Phase 4. The Q4 / R1 / R2 half-day
  prototypes land during R1 implementation kickoff (alongside
  steps 1-4 substrate work); the R3 benchmark commit lands
  alongside the standard scoped-effect rollout (step 6 onward).
  Sub-step planning lives in plan.md's
...
- **Phase 4 step 3.1.1** (`abd3d1a3`): `Catch` foundational scaffold. Three sibling effect types ([`BoxCatch<'a, P, E, A>`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/catch.rs) where `P: ToDynFnOnce`, [`Catch<'a, P, E, A>`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/catch.rs) where `P: ToDynCloneFn`, [`SendCatch<'a, P, E, A>`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/catch.rs) where `P: ToDynSendFn`); three brands at [`brands/effects.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/brands/effects.rs); four of five substrate-required trait impls per brand (`Functor` on Box+Rc, `SendFunctor` across all three with `SendCatch` load-bearing, `WrapDrop` and `Extract` across all three). 11 doctests passing. `SendCatchBrand` does not impl `Functor` (logged as deviation; mirrors `SendStateBrand` precedent). Module wired through [`fp-library/src/types/effects.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects.rs).
- **Phase 4 step 2a**: substrate-level `interpret_with_either<EBrand, Idx, RMinusE>` primitive across all six Run wrappers. Lands the matched-effect short-circuit primitive at all six wrapper sites; generalises [POC 3's concrete two-effect-row template](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs) to walk arbitrary first-order rows with arbitrary `EBrand` matched-effect position. Returns `Result<A, EBrand::Of<'a, Self>>`: matched arm returns `Err(coyo.lower[_ref]())`; unmatched arm dispatches through `fo_handlers.dispatch(rest)`. Body shape simpler than `interpret_with` (flat `peel` loop, no `Functor::map` recursion). Per-wrapper bound shapes mirror their respective `interpret_with` bodies. Bundled commit covering all six wrappers (no per-wrapper specifics surfaced). 24 integration tests at [`run_interpret_with_either.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_interpret_with_either.rs) plus 6 doctests. The `Catch` cons-cell in step 7 consumes this primitive. `just verify` clean.
- **Phase 4 step 2.6**: `ArcRunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the thread-safe explicit-lifetime Run wrapper at [`fp-library/src/types/effects/arc_run_explicit.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/arc_run_explicit.rs). Union of 2.3's Send+Sync bound surface and 2.4 / 2.5's `'a`-lifetime threading. Closes Phase 4 step 2 (all six wrappers ship `interpose`). R1 again did not surface. Four parallel integration tests at [`run_interpose.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_interpose.rs) under a `// -- ArcRunExplicit --` divider; total now 24 across 6 wrappers.
- **Phase 4 step 2.5**: `RcRunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the explicit-lifetime multi-shot Run wrapper at [`fp-library/src/types/effects/rc_run_explicit.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/rc_run_explicit.rs). Union of step 2.1's RcRun template and [`RcRunExplicit::interpret_with`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/rc_run_explicit.rs#L999)'s `'a`-lifetime bound surface. Notably simpler than 2.4: `RcFreeExplicit::wrap` accepts raw projections directly without `Box::new(...)` wrapping. R1 again did not surface. Four parallel integration tests under a `// -- RcRunExplicit --` divider.
- **Phase 4 step 2.4**: `RunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the explicit-lifetime Erased Run wrapper at [`fp-library/src/types/effects/run_explicit.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/run_explicit.rs). Union of step 2.2's Run template (matched-arm shape, `Coyoneda::lower` consuming variant, embed-back via `CoproductEmbedder`) and [`RunExplicit::interpret_with`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/run_explicit.rs#L746)'s `'a`-lifetime bound surface plus the `Box::new(...into_free_explicit())` boxing pattern in the unmatched arm (`FreeExplicit::wrap` requires `Box<dyn ...>`-shaped continuations). The `EmbedIndices` `CoproductEmbedder` bound's inner type is therefore `Box<FreeExplicit<'a, NodeBrand<R, CNilBrand>, A>>`. Closure-refcount uses `<RcBrand as RefCountedPointer>::new` with `+ 'a` (not `+ 'static`). R1 (Explicit-family HRTB-poisoning) anticipated by plan.md did NOT surface: compiled cleanly without ArcRun-style workaround helpers.
- **Phase 4 step 2.3**: `ArcRun::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the thread-safe Erased Run wrapper at [`fp-library/src/types/effects/arc_run.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/arc_run.rs). Union of step 2.1's RcRun template (matched-arm shape, `EmbedIndices` extra type parameter, embed-back path) and [`ArcRun::interpret_with`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/arc_run.rs#L942)'s Send+Sync bound surface. R2 (Send+Sync propagation) cleared structurally with no new substrate machinery: closure-refcount uses `<ArcBrand as RefCountedPointer>::new` with `Send + Sync + 'static` on the user closure; traversal uses `<EBrand as SendFunctor>::send_map` and `<RMinusE as SendFunctor>::send_map`; recursion routes through the existing `unwrap_first` / `make_node_first` / `wrap_first_arc` HRTB-poisoning workaround helpers. Method-level `R: WrapDrop + SendFunctor + 'static` is required because the embed-back path rewraps in `R` (not `RMinusE`); mechanically expected when staying in the original row, not a new structural deviation.
- **Phase 4 step 2.2**: `Run::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the default Erased Run wrapper at [`fp-library/src/types/effects/run.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/run.rs). Mechanical translation of [step 2.1's RcRun template](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/rc_run.rs) with substrate paths swapping `RcFree` -> `Free` (via [`Free::wrap`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/free.rs)), `RcCoyoneda` -> `Coyoneda` (via [`Coyoneda::lower`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/coyoneda.rs), the consuming variant for single-shot programs). Both wrappers reuse `RcBrand` for closure-refcount (`BoxBrand` cannot refcount), so the public/inner split is identical between wrappers. Same `EmbedIndices` deviation as 2.1. Four parallel integration tests added at [`run_interpose.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_interpose.rs) under a `// -- Run --` divider; existing 2.1 RcRun tests renamed `rc_run_t1`-`rc_run_t4` for wrapper-prefixed parity (matching the [`run_interpret.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_interpret.rs) convention).
- **Phase 4 step 2.1**: `RcRun::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive at [`fp-library/src/types/effects/rc_run.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/rc_run.rs). Generalises [POC 2's template](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpose.rs) (one-effect concrete row) to a production shape that walks an arbitrary first-order row with arbitrary `EBrand` position. Public `interpose` wraps the user closure once via [`RefCountedPointer::new`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/classes/ref_counted_pointer.rs); inner `interpose_shared` recurses with `Rc<F>` clones. Implementation: peel + `Member::project<RcCoyoneda<EBrand, Self>, Idx>`; matched arm lowers, recurses interpose on inner sub-programs via `<EBrand as Functor>::map`, applies replacement; unmatched arm walks `Self::Remainder` via `<RMinusE as Functor>::map`, embeds back to R via [`CoproductEmbedder<R::Of<...>, EmbedIndices>`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/coproduct.rs), rewraps. The `EmbedIndices` extra type parameter beyond plan.md's sketched `<EBrand, Idx>` signature is a structural necessity; deviation logged at [deviations.md Phase 4 step 2.1](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/deviations.md). Four integration tests cover both the matched arm and the runtime embed-back path.
- **Phase 4 step 1**: `ScopedCoproduct<H, T>` and `ScopedNil` row-encoding aliases at [`fp-library/src/types/effects/scoped.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/scoped.rs). Transparent type aliases over [`CoproductBrand`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/brands/effects.rs) / [`CNilBrand`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/brands/effects.rs) (no new substrate machinery; dual-row integration into `Run<R, S, A>` was already in place via `NodeBrand<R, S>` since Phase 2). Pure naming layer giving Phase 4 program declarations a self-documenting cue at the scoped row position. Module-level docs explain the no-Coyoneda-on-scoped rationale (case-analysis dispatch in step 4 rather than `Functor::map`) and enumerate the substrate-required traits each scoped-effect brand must implement (`Functor` / `SendFunctor` / `WrapDrop` / `RefFunctor` / `Extract`). Two regression tests confirm the alias transparency. `just verify` clean.
- **Phase 4 step 0**: standalone POC 3 validation at [`fp-library/tests/poc_rc_run_interpret_with_either.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_rc_run_interpret_with_either.rs). Proves the `interpret_with_either<EBrand, Idx>(self, fo_handlers) -> Result<A, EBrand::Op>` substrate primitive shape compiles and short-circuits correctly on a concrete two-effect row (`Identity` FO-dispatched, `ExceptBrand<String>` matched-and-short-circuited). Five tests covering Pure (`Ok(value)`), single Throw (`Err(error)`), Identity-then-Throw (FO arm dispatches before short-circuit), Identity-only (runs to completion), and Catch-dispatcher-shape compatibility. Generic rollout across all six Run wrappers ships in step 2a; the `Catch` cons-cell in step 4 consumes the validated primitive. Adopted per the [K1 resolution](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries). `just verify` clean.
- **Phase 3.5 sub-step 5**: F4 closure [resolutions.md entry](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/resolutions.md#resolved-2026-05-06-phase-3-prior-review-f4-closed-structurally-via-phase-35-retrofit-sibling-boxbrand-family-on-default-run-substrates); Phase 3.5 closes. Documents that the Phase 3 prior-review F4 finding (per-effect closure cells multi-shot-callable on single-shot wrappers despite the wrapper-level guarantee) is closed structurally rather than as accepted-tradeoff per the 2026-05-05 Phase 3 step 8 disposition (Option A doc-only). The 2026-05-03 (3.a-1) "one effect type per operation" sub-decision is re-opened with a four-argument justification (cost amortised across Phase 3 + Phase 4; brand surface stays alphabetised; doc surface stays single-source ~30 lines; macro complexity unaffected); the user-API-level "one type per operation" promise still holds since under-the-hood splits are invisible to user code (one `Run::get`, one `RcRun::get`, one `ArcRun::get`).
- **Phase 3.5 sub-step 3**: docs-only update to [`fp-library/docs/pointer-abstraction.md`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/docs/pointer-abstraction.md). Adds `ToDynFnOnce` to the mermaid trait diagram (parallel to `ToDynFn` under `Pointer`), the trait table, and the `BoxBrand` row in the brand-implementations table. New `ToDynFnOnce is BoxBrand-only by structural necessity` subsection adds a (closure-semantic, pointer-capability) matrix and explains the structural reason `Rc<dyn FnOnce>` / `Arc<dyn FnOnce>` cannot be implemented; cross-references `BoxState` / `BoxReader` / `BoxChoose` from sub-step 2 as the production use-case. `just fmt && just doc` clean.
- **Phase 3.5 sub-step 2**: three sibling effect types and brands ([`BoxState`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/state.rs) / [`BoxReader`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/reader.rs) / [`BoxChoose`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/choose.rs); `BoxStateBrand<P, S>` / `BoxReaderBrand<P, E>` / `BoxChooseBrand<P>` each `where P: ToDynFnOnce`) plus default Run / RunExplicit smart constructors switched to thread `Box<dyn FnOnce>` continuations. Functor impls specialised to `BoxBrand` (Box's blanket FnOnce impl). RcRun / ArcRun smart constructors unchanged. UI test [`run_smart_constructor_type_mismatch.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/ui/run_smart_constructor_type_mismatch.rs) updated to use `BoxReaderBrand<BoxBrand, String>`; sibling `.stderr` regenerated. The three-sibling interpretation diverges from plan.md's literal "single brand parametrised over `P`" reading; full rationale in [deviations.md Phase 3.5 sub-step 2](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/deviations.md).
- **Phase 3.5 sub-step 1**: `ToDynFnOnce` trait at [`fp-library/src/classes/to_dyn_fn_once.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/classes/to_dyn_fn_once.rs) (extends `Pointer + 'static`; one method `new<'a, A: 'a, B: 'a>(impl 'a + FnOnce(A) -> B) -> <Self as Pointer>::Of<'a, dyn 'a + FnOnce(A) -> B>`; free function `to_dyn_fn_once` paralleling `to_dyn_fn`). `BoxBrand`-only impl at [`box_ptr.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/box_ptr.rs); `RcBrand` / `ArcBrand` excluded structurally (FnOnce::call_once consumes the trait object). Three regression tests, including a moved-non-Clone-capture test exercising the FnOnce-only feature. No pointer-abstraction.md update yet (sub-step 3).
