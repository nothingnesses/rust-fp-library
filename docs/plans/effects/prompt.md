...
at `2e7a9048`. **Phase 4 step 3.3.3 (Bracket Val smart
constructors per wrapper) shipped on this branch**: six per-wrapper
`bracket` smart constructors at run.rs / rc_run.rs / arc_run.rs /
run_explicit.rs / rc_run_explicit.rs / arc_run_explicit.rs; each
pairs the wrapper with its substrate-correct cell from B19 closure
(`Run` -> `BoxBracket`, `RcRun` -> `Bracket`, `ArcRun` ->
`SendBracket`, `RunExplicit` -> `BoxBracketExplicit`,
`RcRunExplicit` -> `BracketExplicit`, `ArcRunExplicit` ->
`SendBracketExplicit`). Box-family takes `FnOnce` (single-shot);
Rc/Arc-family takes `Fn` (multi-shot, clone-able); Arc-family
additionally requires `Send + Sync` on body and release.
ArcRun::bracket reuses `make_node_scoped` / `wrap_first_arc` HRTB
workaround helpers. Three structural deviations: marker-struct
doctests for 4 of 6 wrappers (Run / RcRun / RunExplicit /
RcRunExplicit) following the B18 POC pattern; ArcRunExplicit
additionally needs `#![recursion_limit = "512"]` and `SendFunctor`
delegation; ArcRun::bracket's doctest uses `ignore` block + simpler
runnable assertion (rustc overflow on Send check via marker-struct

- SendBracketBrand's GAT-Send-Sync bound, smart constructor
  itself compiles cleanly, step 3.3.4 integration tests cover
  end-to-end). `BoxBracketExplicit` substrate refined from
  `FreeExplicit<'a, Sub, _>` to `Box<FreeExplicit<'a, Sub, _>>`
  (matches `FreeExplicit::wrap`'s expected layer-program type,
  applied retroactively at the B19 closure code in bracket.rs).
  5 new smart-constructor doctests (1 ignored). Deviation logged at
  [deviations.md Phase 4 step 3.3.3](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/deviations.md).
  **Next greenfield step**:

**Step 3.3.4** (Bracket Val integration tests): ~18 shape-only
tests at `fp-library/tests/run_bracket.rs` (file lands in this
step) across 5 Run wrappers (Run, RcRun, RunExplicit, RcRunExplicit,
ArcRunExplicit), mirroring [run_local.rs](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_local.rs)'s
template. Per wrapper: T1 verifies the smart constructor peels to
`Err(Node::Scoped(Coproduct::Inl(*::Bracket { .. })))`; T2/T3/T4
exercise acquire / body / release at the cell level; T5 (Clone-able
wrappers only) clones the suspended program and confirms both peelable
handles materialise the original three steps independently.
**`ArcRun::bracket` is skipped per active blocker B20** (rustc
Send+Sync overflow on `SendBracketBrand`'s GAT-Send-Sync bound when
the marker-struct workaround references the cell brand recursively;
see plan.md's Active blockers section). End-to-end exercise of
`ArcRun::bracket` lives in step 8 (which tests the step 7 bracket dispatcher; escalation to step 8a if still blocked)
where the dispatch API changes the test surface.
...
- **Phase 4 step 3.3.3** (this commit): six per-wrapper Bracket Val smart constructors at run.rs / rc_run.rs / arc_run.rs / run_explicit.rs / rc_run_explicit.rs / arc_run_explicit.rs; each pairs the wrapper with its substrate-correct cell from B19 closure (`Run` -> `BoxBracket`, `RcRun` -> `Bracket`, `ArcRun` -> `SendBracket`, `RunExplicit` -> `BoxBracketExplicit`, `RcRunExplicit` -> `BracketExplicit`, `ArcRunExplicit` -> `SendBracketExplicit`). Marker-struct doctests for 4 of 6 wrappers (Run / RcRun / RunExplicit / RcRunExplicit) following the B18 POC pattern; ArcRunExplicit additionally needs `#![recursion_limit = "512"]` and `SendFunctor` delegation. ArcRun::bracket's doctest uses `ignore` block + simpler runnable assertion (rustc overflow on Send check via marker-struct + SendBracketBrand's GAT-Send-Sync bound; smart constructor itself compiles cleanly; step 3.3.4 integration tests cover end-to-end). `BoxBracketExplicit` substrate refined to `Box<FreeExplicit<...>>` (matches `FreeExplicit::wrap`'s expected layer-program type, applied retroactively at B19 closure code in bracket.rs). 5 new smart-constructor doctests (1 ignored). Deviation logged at [deviations.md Phase 4 step 3.3.3](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/deviations.md). `just verify` clean.
