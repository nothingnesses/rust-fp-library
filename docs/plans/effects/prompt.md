...
/ `970ad399` / `ba080e44`. **Phase 4 step 3.2.5 (RefLocal Ref
foundational scaffold + `ToDynFnOnce::ref_new` matrix
completion) is in the working tree, ready to commit**. Step
3.2.5 closes B11 (closure-trait matrix gap on `ToDynFnOnce`) and
B12 (variant naming uniformly `Local`). Four pieces in one
focused unit: (1)
[`ToDynFnOnce::ref_new`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/classes/to_dyn_fn_once.rs)
trait method plus `to_ref_dyn_fn_once` free-function shim,
mirroring the existing `ref_new` on `ToDynFn` / `ToDynCloneFn` /
`ToDynSendFn`; (2) `BoxBrand`'s `ToDynFnOnce::ref_new` impl
returning `Box::new(f)` at
[`box_ptr.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/box_ptr.rs);
(3) three sibling effect types at
[`ref_local.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/ref_local.rs)
(`BoxRefLocal<'a, P, E, A>` where `P: ToDynFnOnce`,
`RefLocal<'a, P, E, A>` where `P: ToDynCloneFn`,
`SendRefLocal<'a, P, E, A>` where `P: ToDynSendFn`) each holding
a `Local { modify, action }` variant with
`modify: <P>::Of<'a, dyn ClosureTrait(&E) -> E>` (Ref flavour,
borrows env) and `action: <P>::Of<'a, dyn ClosureTrait(()) -> A>`
(B-thunk per the closed B9 decision); (4) three brand
declarations (`BoxRefLocalBrand` / `RefLocalBrand` /
`SendRefLocalBrand`) at
[`brands/effects.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/brands/effects.rs).
Four of five substrate-required trait impls per brand: `Functor`
on Box+Rc; `SendFunctor` across all three with `SendRefLocal`
load-bearing; `WrapDrop` and `Extract` across all three. Manual
`Clone` for `RefLocal` (Rc-bump) and `SendRefLocal` (Arc-bump);
`BoxRefLocal` is non-`Clone`. `RefFunctor` lands in step 3.2.6.
13 doctests passing. `SendRefLocalBrand` does not impl `Functor`
(logged as deviation; mirrors `SendLocalBrand` / `SendCatchBrand`
/ `SendStateBrand` precedents). Steps 3.2.6-3.2.8 ship the rest
of the RefLocal Ref cycle (RefFunctor + helpers / smart
constructors / integration tests). Step 3
