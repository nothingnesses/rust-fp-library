...
on this branch**: existing 3 cells fixed for substrate-correctness
([`Bracket`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/bracket.rs)
(RcBrand) -> `RcFree<Sub, _>`,
[`SendBracket`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/src/types/effects/bracket.rs)
(ArcBrand) -> `ArcFree<Sub, _>` with GAT-projection-Send-Sync
bound on `Sub`, `BoxBracket` keeps `Free<Sub, _>`). Three new
Explicit-family cells `BoxBracketExplicit` / `BracketExplicit` /
`SendBracketExplicit` (storing `FreeExplicit<'a, Sub, _>` /
`RcFreeExplicit<'a, Sub, _>` / `ArcFreeExplicit<'a, Sub, _>`);
three new brand declarations; per-brand trait impls (Functor
identity, SendFunctor identity-or-stub, WrapDrop None, Extract
panic-stub, RefFunctor stub-or-clone). 30 bracket.rs doctests
pass (15 existing + 15 new). The 4 SendBracket and 4
SendBracketExplicit doctests use `IdentityBrand` substrate (not
`ThunkBrand`) because `Thunk<'a, A>` contains `Box<dyn FnOnce>`
which is not `Sync`, so `ThunkBrand` cannot satisfy `ArcFree`'s
GAT-projection-Send-Sync bound. Deviation logged at
[deviations.md Phase 4 step 3.3.1 B19 closure](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/deviations.md).
**Next greenfield step**:

**Step 3.3.3** (Bracket Val smart constructors per wrapper): six
per-wrapper smart constructors, each pairing the wrapper with its
appropriate Erased or Explicit cell (`Run::bracket` -> `BoxBracket`,
`RcRun::bracket` -> `Bracket`, `ArcRun::bracket` -> `SendBracket`,
`RunExplicit::bracket` -> `BoxBracketExplicit`,
`RcRunExplicit::bracket` -> `BracketExplicit`,
`ArcRunExplicit::bracket` -> `SendBracketExplicit`). Restores stashed
`Run::bracket` probe (`git stash@{0}`) and adds 5 more wrapper
constructors. Each smart-constructor doctest uses the marker-struct
row pattern validated by the
