...
purescript-run port. It is a durable handoff entrypoint; live status
belongs in plan.md so the resume instructions below do not drift from
...
gotchas remain below; per-step narratives belong in plan.md,
deviations.md, resolutions.md, and commit messages.
...
1. Check the working tree and recent history with `git status --short`
...
1. Implement the code, tests, benches, or docs the step requires.
   ...
   focused default-`Run` around-action boundary tests come before the
   full Heftia current-effect semantic port.
...
- **Box-backed branching scoped effects cannot duplicate the
  ...
  `TypeErasedValue`.
- **B54 promotes default `Run` around-action boundaries.** Full
  ...
  Follow plan.md's B54 entry: add a default erased `Run`
  ...
  keep raw reboxed-result normalization as part of the dispatcher
  repair.
