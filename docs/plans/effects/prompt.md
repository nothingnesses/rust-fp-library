...
`purescript-run` into `fp-library`. The design is fixed; your job is
to land code, tests, and benches against the phased steps in
[plan.md](plan.md), one step per commit, until the phase is complete
or you hit a blocker.

## Where to start

1. Read [plan.md](plan.md). The `Current progress` section names the
   active phase and what was finished last. The implementation phasing
   sections (Phase 1 through Phase 5, plus Phase 6+ deferred) list
...
3. Read [decisions.md](decisions.md) for any sections referenced by
   that step. The plan cross-references decisions whenever the
   implementation choice is non-obvious.
4. Skim relevant entries under [research/](research/) only if a step
   names them. Do not re-read the full corpus.
...
1. Implement the code, tests, benches, or docs the step requires.
   ...
   project's [CLAUDE.md](../../../CLAUDE.md) for usage) for type
   info, go-to-definition, and find-references. The Brand-and-Kind
   machinery and the existing four-variant `Coyoneda` family are the
   templates the new code follows.
...
4. Update the top of [plan.md](plan.md): `Current progress` to
   reflect what now exists, `Open questions, issues and blockers` if
   you found any (see "When you hit something unexpected" below),
   and `Deviations` if your implementation diverges from what the
   step text said.
...
- **A step that doesn't make sense given the current code state.**
  ...
  [plan.md](plan.md) describing what's unclear, commit that single
  edit, and report back to the user. Do not invent an interpretation.
- **A genuine design conflict** (a decision in [decisions.md](decisions.md)
  ...
  Do not edit [decisions.md](decisions.md) yourself.
...
- **`fp-library/` is the production crate.** Code, tests, and
  benches go here.
- **`fp-macros/` holds proc-macros.** The `effects!`,
  `effects_coyo!`, `handlers!`, `define_effect!`, and
  `define_scoped_effect!` macros land in `fp-macros/src/effects/`.
- **`poc-effect-row/` is a separate Cargo workspace and a reference
  implementation.** Do not modify it during the port; migrate code
  out of it into `fp-library` and `fp-macros` per the phase
  instructions, and delete it only when its tests have a production
  equivalent (Phase 2 step 8).
- **Documentation lives in `docs/`.** Do not invent new top-level
  docs without an explicit step asking for them. Phase 5 step 4
  schedules `fp-library/docs/run.md`.
- **Out-of-scope items in [plan.md](plan.md)'s `Out of scope`
  section** are off-limits. Surveying alternatives, prototyping
  evidence-passing, exploring tag-based type-level sorting, etc. are
  not part of this implementation effort.

## Project conventions

- **Hard tabs for Rust indentation.** The project's `rustfmt.toml`
  uses hard tabs. When using the Edit tool, the `old_string` must
  match the file's tab characters exactly. Do not fall back to
  `sed`, `awk`, or `python` to edit whitespace.
...
- All build / test / lint commands go through `just` (the project
  has a [justfile](../../../justfile) that handles the Nix
  environment). Examples: `just verify`, `just test`, `just clippy`,
  `just doc`.
...
- The LSP tool (`rust-analyzer` via MCP) is the right tool for type
  ...
  etc. See the project's [CLAUDE.md](../../../CLAUDE.md) for
  examples. Reach for it whenever you would otherwise be tracing
...
- [plan.md](plan.md): the phased steps and success criteria. The
  authoritative answer to "what do I do next."
- [decisions.md](decisions.md): the design rationale. The
  authoritative answer to "why this way."
- [research/](research/): per-codebase classifications, three Stage
  2 deep dives, and a synthesis. Source material for the decisions.
...
- [poc-effect-row/](../../../poc-effect-row/): standalone Cargo
  workspace with the row-encoding hybrid POC. Reference
  implementation only; migrates into production during Phase 2.
- [fp-library/tests/free_explicit_poc.rs](../../../fp-library/tests/free_explicit_poc.rs):
  `FreeExplicit` POC; promotes into `fp-library/src/types/` during
  Phase 1.
- [../../../CLAUDE.md](../../../CLAUDE.md): project-wide agent
  instructions including LSP usage.
- [../../../AGENTS.md](../../../AGENTS.md): broader agent contract
  for this repo.
