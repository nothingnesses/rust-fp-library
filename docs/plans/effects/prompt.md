...
body return is `Free<Sub, (A_brand, B_brand)>` where A_brand and
...
blocked by B18** (user-facing recursive type alias rejection
on Bracket-containing scoped rows). Probe of `Run::bracket`
stashed at `git stash@{0}` validates that the smart-constructor
signature itself compiles; user-facing rows trip Rust's
type-alias cyclicity rule. Three options surfaced in
[plan.md Active blockers](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/plan.md#active-blockers);
recommendation **Option A**: marker-struct workaround
documented per-call-site, with macro-generation in step 5
(`scoped_effects!`). Awaiting user input on B18 resolution
before step 3.3.3 implementation. Step 3
