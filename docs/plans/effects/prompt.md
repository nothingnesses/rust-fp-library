...
at `b13fce28`. **Phase 4 step 3.3.4 (Bracket Val integration tests)
shipped on this branch**: 23 shape-only tests at
[`fp-library/tests/run_bracket.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/run_bracket.rs)
across 5 Run wrappers (Run, RcRun, RunExplicit, RcRunExplicit,
ArcRunExplicit). `ArcRun::bracket` is skipped per the
[closed B20 closure entry](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/resolutions.md#resolved-2026-05-08-phase-4-step-3.3.4-arcrunbracket-integration-tests-blocked-by-rustc-sendsync-overflow-b20-closed-via-option-a-skip-arcrunbracket-integration-tests-defer-to-step-8-bracket-dispatcher-tests-escalate-to-option-d-sendbracketbrand-redesign-if-step-8-still-cannot-exercise-it):
end-to-end exercise of `ArcRun::bracket` is deferred to step 8's
bracket-dispatcher tests; escalation to step 8a `SendBracketBrand`
redesign if still blocked there. Per included wrapper: T1 verifies
the smart constructor peels to
`Err(Node::Scoped(Coproduct::Inl(*::Bracket { .. })))`; T2 / T3 /
T4 destructure the cell, invoke acquire / body / release, and
manually wrap each substrate-typed result via the wrapper's
`from_*` constructor (`Run::from_free` / `RcRun::from_rc_free` /
`RunExplicit::from_free_explicit` / `RcRunExplicit::from_rc_free_explicit` /
`ArcRunExplicit::from_arc_free_explicit`, plus `*acquire(())` deref
for `BoxBracketExplicit`'s boxed `FreeExplicit` substrate) before
peeling, since Option A's identity `Functor::map` on `*BracketBrand`
does not rewrite cell continuations during `peel`. T5 (Clone-able
wrappers only: RcRun, RcRunExplicit, ArcRunExplicit) clones the
suspended program and confirms both peelable handles independently
materialise the original acquire thunk. Each section uses the
marker-struct workaround validated by
[`poc_bracket_marker_row.rs`](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_bracket_marker_row.rs);
the file uses module-level `#![recursion_limit = "512"]` for
ArcRunExplicit's Send + Sync trait-resolution depth (same raise
its doctest needs); the ArcRunExplicit section delegates `SendFunctor`
instead of `Functor` per the smart constructor's substrate bound.
**Next greenfield step**:

**Step 3.3.5** (RefBracket Ref foundational scaffold): new
`fp-library/src/types/effects/ref_bracket.rs` introducing 4 cells
per the closed B15 (2 pointer brands x 2 Free families); inherits
Option A from step 3.3.1 (5-param struct + 4-param brand) and the
per-Free-family substrate split from B19 closure. The Ref flavour
stores `release` (and possibly `body`) as `&A`-borrowing closures
using `<P>::Of<'a, dyn 'a + Fn(&A) -> ...>`; acquire remains a
B-thunk per B14. Per-cell trait impls mirror the Bracket Val
cycle: Functor identity, SendFunctor identity-or-stub, WrapDrop
None, Extract panicking-stub. Subsequent sub-steps mirror the
Bracket Val cycle: 3.3.6 RefFunctor impls, 3.3.7 per-wrapper
`ref_bracket` smart constructors, 3.3.8 integration tests at
`tests/run_ref_bracket.rs`.
