...
**Step 3.3.3 (Bracket Val smart constructors per wrapper) is
blocked by B19** (foundational-scaffold cells hardcode `Free<Sub, *>`which is correct only for`Run::bracket`; 5 of 6 wrappers need
their wrapper-specific substrate). Resolution: **Option C
(split into 6 cells per Free family)** per the
[B19 entry in plan.md Active blockers](file:///home/jessea/Documents/projects/rust-fp-lib/docs/plans/effects/plan.md#active-blockers).
The fix-forward commit reworks the foundational scaffold (fix
existing 3 cells + add 3 Explicit-family cell siblings + 3 new
brand declarations + parallel trait impls) before step 3.3.3
can resume. The stashed Run::bracket probe (`git stash@{0}`)
remains valid for `Run::bracket`(uses`BoxBracket`+`Free`).
Step 3
