...
the next greenfield step**, restoring the stashed Run::bracket
probe (`git stash@{0}`) and adapting all six wrappers to use
the marker-struct doctest pattern (validated by the
[POC](file:///home/jessea/Documents/projects/rust-fp-lib/fp-library/tests/poc_bracket_marker_row.rs)
that closes B18). Step 3
