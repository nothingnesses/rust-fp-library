...
Status: in progress. Step 2's catalog audit (the erasure axis) is done: non-`'static` payload support is genuinely needed, so approach A (drop the Explicit capability) is eliminated and the erasure axis takes the unify-internally / two-forms path, with C-versus-B left to POC-11. Remaining gate before the destructive steps: the POC-11 integration spike, which settles the pointer axis and the C-versus-B erasure choice. The rest of FS-1 (adopt the unified row, hybrid method, single-spine invariant, steps 1, 2, and 6) is decided; implementation of the destructive steps is not started.
