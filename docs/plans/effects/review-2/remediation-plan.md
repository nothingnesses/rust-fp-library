...
**OQ-4I: the built-in effects' documentation status in the effects story.** Item 4's effects-story step must present the built-in effect catalog, but every built-in lives in the `pub(crate)` `fs1` slice: the interpreter (`run`/`run_except`/`Handlers`) is crate-internal and effect-set-specific, and the slice pins effect payloads to concrete conformance types (`Reader`'s environment and `Local`'s pins to `i32`, `Writer`'s log to `String`, `Except`'s error to `&'static str`, the one sixteen-effect `Row`). The guide cannot describe the catalog without deciding what status those effects have for a reader.

Viable approaches:

- Approach A (recommended): document the built-ins as the crate-internal reference catalog. The guide presents each built-in through its `define_effect!` definition (which, since item 11's ports, is its actual source) and its pinned reference semantics (the OQ-21 decisions: `Catch` recovers `Throw` only; `Bracket` releases on body abort with body-abort priority; `Censor` is transactional on abort; `Writer`'s log is append-only), stating explicitly that the built-ins are crate-internal conformance fixtures today and that their public, payload-generalised form ships with the generic runner surface (item 8's handler surface and item 14's threaded runners). A reader who wants a built-in's behaviour now copies its definition shape into their own crate and interprets it per the custom-effects guide, which the catalog links.
- Approach B: promote the built-ins to public first, then write the story against the public catalog.
- Approach C: defer the built-in catalog until items 8 and 14 land, shipping the story without it.

Trade-offs and reasoning against the Project Principles: approach B is rejected for the reason OQ-11E rejected building a generic `handle` first and OQ-4H rejected writing the effects guide against the earlier `pub(crate)` slice: the built-ins' payloads are pinned to conformance types and there is no public interpreter to run them, so promoting them as-is publishes a known-throwaway shape, while generalising the payloads now front-runs the handler-attachment design round items 8 and 14 own. This is not churn-avoidance; the clean end state (principle 1) is the payload-generalised public catalog over a generic runner, which B does not produce, so B is a premature commitment the end state must break. Approach C withholds documentation for semantics that are settled, tested, and stable, and which are exactly the contract items 8 and 14 must preserve; rejected as withholding docs for a settled surface (the OQ-11E reasoning against deferral). Approach A is truthful to the tree (the docs-never-overpromise rule item 14's decision states), documents the limitation explicitly (principle 3), and is evidence-grounded (principle 7): the semantics claims are pinned by the slice's bucket-A suite, and the definition shapes shown are the macro invocations themselves, so the catalog cannot drift from the source while the built-ins remain the macro's conformance suite.
...
**Effects story.** Rewrite the effects documentation against the public FS-1 surface (which item 11 has shipped): a fresh effects guide in place of the deleted `run.md`, carrying the design rationale, the effect catalog, handler ordering, and the dual-row-API edits item 1 deferred here, plus architecture.md's full effects-design section. It builds on the custom-effects guide already written under item 11 (`fp-library/docs/custom-effects.md`, commit `d142102f`), which covers defining effects and interpreting them by hand; this step adds the story around the built-in catalog and its handlers, and its built-in catalog must reckon with the built-ins being `pub(crate)`. This is the effects half of the OQ-4H doc split (the substrate-truth half is done); writing it against the `pub(crate)` slice earlier was rejected as certain throwaway that would embed slice internals item 11's codegen replaces, and deferring the substrate fixes was rejected because they describe types item 11 never touches.
