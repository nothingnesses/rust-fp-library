...
The first round of pre-implementation readiness questions surfaced by reading the as-built code (substrate identity, the confirming integration spike, migration strategy, async during the transition, the behaviour-parity oracle, and Phase A sequencing) have been resolved along the principle-aligned approaches and folded into the implementation steps: item 4's Grounding, adopted Method, and steps 1, 2, and 6 carry the substrate, spike, migration, and async decisions; the Phase A sequencing decision is recorded in the Sequencing overview and item 1. One further readiness question surfaced on a later review pass and is recorded below; it is a hard prerequisite for item 4 alongside the substrate-identity decision.

### Open question: the erased-versus-Explicit substrate axis

A later alignment pass found that item 5's claim, that the single `Store`-parameterised substrate "answers the erased-versus-explicit and per-pointer matrix questions," overstates the evidence. POC-8/POC-8b collapsed the pointer axis (Box/Rc/Arc) via the `ClosureStorage` `Store` parameter, on the erased path only. The erased-versus-Explicit axis is a second, orthogonal axis the `Store` parameter does not touch, and it is unproven under FS-1.

Grounding: the effects subsystem ships two substrate forms per pointer, the six wrappers being {erased, Explicit} x {Box, Rc, Arc}. The erased form (`Free`/`RcFree`/`ArcFree`) erases values to `Box<dyn Any>`, which forces `A: 'static` and enables the heterogeneous-`CatList` O(1) stack-safe `bind`. The Explicit form (`FreeExplicit`/`RcFreeExplicit`/`ArcFreeExplicit`) keeps concrete payloads, so it supports non-`'static`/borrowed data, at the cost of a naive recursive `bind` that is not O(1) stack-safe. The Explicit wrappers are exercised by many effect tests (`run_coroutine_helpers`, `run_lift`, `run_choose`, `run_local`, `run_state_helpers`, `im_do`, ...), so non-`'static` payload support is a real, used capability, not speculative. The two properties appear to be in fundamental tension: O(1) stack-safe `bind` relies on a heterogeneous continuation queue, which relies on type erasure, which forces `'static`; non-`'static` support forbids that erasure. (Brand-keyed dispatch is not part of this tension: it operates on the concrete row `F`, so it works regardless of value erasure, POC-2.)

The question: does FS-1 keep the non-`'static` (Explicit) capability, and if so, can one `Store`-parameterised substrate provide both O(1) stack-safe `bind` and non-`'static` payloads, or must FS-1 retain two `Store`-parameterised forms (collapsing the six wrappers to two, not one)?

Approaches:

- A. Drop the non-`'static` capability. If the effect catalog can be expressed entirely over `'static` payloads, keep only the erased form: one `Store`-parameterised substrate (six wrappers to one), and delete the Explicit family. Trade-offs: the cleanest end state; but it removes a currently-used capability (borrowed payloads in effects) and so is an API-breaking capability removal that must be confirmed safe first.
- B. Keep two `Store`-parameterised forms. Retain an erased substrate (`'static`, O(1) stack-safe `bind`) and an Explicit substrate (non-`'static`, recursive `bind`), each `Store`-parameterised, collapsing the six wrappers to two. Trade-offs: preserves both capabilities and is honest about the tension; but it is not the "one substrate" item 5 implies, and the two forms share a spine shape (some residual duplication, far less than six).
- C. Attempt true unification. Find one representation that delivers O(1) stack-safe `bind` and non-`'static` payloads together (six to one, fully). Trade-offs: the ideal end state; but it must overcome the erasure-versus-`'static` tension, which looks like a concrete type-system limitation (a heterogeneous queue needs erasure needs `'static`); if no such representation exists, this reduces to B.

Recommendation: resolve by evidence, in this order. First audit the effect catalog and its tests for genuinely borrowed (non-`'static`) payloads; if none truly need it, adopt A (one substrate, the capability dropped with the break documented). If non-`'static` is needed, attempt C in the POC-11 spike (item 4 step 2), and fall back to B only if the erasure-versus-`'static` tension is confirmed as a concrete limitation, documenting it per principle 4. Extend POC-11's scope to carry a non-`'static` payload through a deep `bind` chain so the axis is decided on evidence, not assumed. Reasoning: the principles favour the fewest substrate forms (A over B over the status-quo six), but not by dropping a genuinely-used capability (correctness) nor by assuming an unproven unification; the requirement audit plus the POC decide, and the two-form fallback is principled only once the limitation is shown and documented. Until this resolves, item 5's "one parameterised substrate" holds for the pointer axis but is open for the erased/Explicit axis.
