...
The first round of pre-implementation readiness questions surfaced by reading the as-built code (substrate identity, the confirming integration spike, migration strategy, async during the transition, the behaviour-parity oracle, and Phase A sequencing) have been resolved along the principle-aligned approaches and folded into the implementation steps: item 4's Grounding, adopted Method, and steps 1, 2, and 6 carry the substrate, spike, migration, and async decisions; the Phase A sequencing decision is recorded in the Sequencing overview and item 1. A further readiness question, the erased-versus-Explicit substrate axis (a second axis orthogonal to the pointer axis: `'static` + `dyn Any` + O(1) `CatList` bind versus non-`'static` + concrete payloads + recursive bind), surfaced on a later review pass; it has been adopted along the principle-aligned approach and folded into item 4 step 2, which now settles both the pointer and erasure axes of the substrate-identity decision. No open strategic questions remain. A further round of build-readiness questions has since surfaced from reading the substrate code as item 4 reached step 5, and is open below: the erased-substrate type architecture and the queue/drop/sequencing choices, plus one genuine gap the prior POCs did not cover, a unified `Store`-parameterised Coyoneda. These are recorded with approaches, trade-offs, recommendations, and reasoning, to be adopted and folded into item 4 step 5 as concrete sub-steps (the same convention the earlier rounds followed).

**Step-5 substrate build-readiness questions (open).** The substrate-identity decision (POC-11) and the closure-storage reconciliation (POC-8b) settled feasibility, but scoped their proofs to the continuation spine and the continuation-closure storage. The questions below are the architecture decisions that step 5's "mechanical production work" actually rests on; at least one (OQ-5A) is a genuine gap the POCs did not cover. Adopting them now and converting them into step-5 sub-steps avoids the back-tracking earlier rounds incurred.

OQ-5A. Coyoneda unification under `Store` (the gap the POCs did not cover). The unified row wraps every effect in a `Coyoneda` cell. Today there are three Coyoneda types for the same per-`Store` reason the spine has three: the public `Coyoneda` holds `Box<dyn CoyonedaInner>` and is explicitly not `Clone`/`Send`/`Sync`, while `RcCoyoneda` and `ArcCoyoneda` (the `Send + Sync` variant) exist so Rc/Arc rows can be shared and, for Arc, cross threads. POC-11 parameterised the continuation queue/spine and POC-8b the continuation closure, but neither touched the Coyoneda wrapper; POC-8b explicitly excluded Coyoneda from its slice, and the `fs1` slice has only ever used the Box `Coyoneda`. The plan's step-2 decision to retire the Rc/Arc Coyoneda therefore implies a unified `Store`-parameterised Coyoneda that no POC has built, and it is a hard prerequisite for the Rc and Arc erased substrates to carry Coyoneda-wrapped rows at all.

- Approaches: (i) `Store`-parameterise `Coyoneda` via the same `ClosureStorage` mechanism used for the continuation queue, so its inner map-closure is `Store::Stored` (one `Coyoneda` type, Box keeps `FnOnce`, Rc/Arc keep `Fn (+ Send + Sync)`), retiring the three variants into one. (ii) Keep the three Coyoneda variants (reverse the retire decision), pairing each with its `Store`. (iii) Drop the Coyoneda wrapping for the Rc/Arc substrates (carry bare effect values), narrowing the unified row to Box-only Coyoneda.
- Trade-offs: (i) collapses three types to one and matches the spine's own `Store` axis, but it is unproven and the Coyoneda `FnOnce`/`Fn` split must be shown to compose with `lower` and with the brand-keyed dispatch already built; (ii) is the least new code but contradicts the six-to-two collapse the whole substrate decision rests on, leaving the Coyoneda axis un-unified; (iii) loses the free-functor convenience (smart constructors would owe an explicit `Functor` per effect) and fragments the row representation by `Store`, which fights brand-keyed dispatch.
- Recommendation: (i), and prove it with a small spike first (an Arc-store, Coyoneda-wrapped, multi-arm row that statically asserts `Send + Sync` and runs `lower` through dispatch) before the full port, since this is the one combination no POC has exercised.
- Reasoning: the per-`Store` `FnOnce`/`Fn` split inside Coyoneda is identical to the one `ClosureStorage` already reconciles for continuations (POC-8b), so reusing that mechanism keeps one idiom and honours the single-representation goal; spiking it first is consistent with the foundation sweep's gate-before-build discipline and is cheap relative to a mid-port discovery that the Arc row cannot be expressed.

OQ-5B. Erased-substrate type architecture and the shared `Free`. POC-11 fixed that the Box case reuses the existing `Free`, but not how: `Free` is shared infrastructure consumed outside effects by `Trampoline`, `TryTrampoline`, `Thunk`, and `Identity`.

- Approaches: (i) add a new effects-only `Store`-parameterised erased substrate whose `Store = Box` representation is structurally `Free`'s queue (the continuation `CatList` element is `Box<dyn FnOnce>`), leaving `Free` and its non-effects consumers untouched and retiring `RcFree`/`ArcFree`. (ii) Parameterise `Free` itself into `Free<Store, F, A>` and keep the bare name via `type Free<F, A> = Free<Box, F, A>` so consumers compile unchanged. (iii) Parameterise `Free` and migrate every consumer's signatures to the new shape.
- Trade-offs: (i) zero churn on shared infra and a clean effects boundary, at the cost of a thin Box-case type that must be shown to be the same spine (not a parallel Box enum); (ii) one literal type for all stores with minimal consumer churn, but the `Store` parameter can leak into non-effects inference and the alias must preserve inherent-method resolution; (iii) maximally unified but inflicts API-breaking churn on `Trampoline`/`Thunk`/`Identity` for no effects-side gain.
- Recommendation: (ii) by default, parameterise `Free` into `Free<Store, F, A>` with `type Free<F, A> = Free<Box, F, A>`, so there is one literal free-monad type with Box as an instantiation; fall back to (i) only if a concrete limitation is found (for example the `Store` parameter leaking into non-effects inference in a way the alias cannot hide), and document that limitation per the project principles before adopting the fallback. (iii) is rejected because it pays the same churn as (ii) while also rewriting consumer signatures the alias would otherwise absorb, for no additional gain.
- Reasoning: the Project Principles accept API-breaking changes when they lead to a better end state and direct choosing the cleaner architecture unless a concrete limitation prevents it, with churn-avoidance ranked below clean architecture, not above it. (ii) is the cleaner end state: one free-monad type where Box is an instantiation, versus (i)'s separate substrate that merely mirrors `Free`'s queue and so risks a parallel Box spine (the coherence debt the principles disfavour). The Trampoline/Thunk/Identity churn is exactly the acceptable API break, and the alias absorbs the common case, so it is not a concrete limitation on its own; only a demonstrated inference or resolution failure would justify falling back to (i).

OQ-5C. Depth of the continuation-queue (`CatList`) parameterisation. `CatList<A>` is element-generic, so it can hold `Store::Stored` continuations directly; but the shipped `RcCatList`/`ArcCatList` also differ in their internal node pointers (for example `Rc<VecDeque<RcCatList<A>>>`), and `CatList` is itself general infrastructure used outside effects.

- Approaches: (i) parameterise only the continuation element (`CatList<Store::Stored<...>>`) and keep the existing owned-node `CatList` internals for all stores. (ii) `Store`-parameterise the queue's internal node pointers too, unifying `RcCatList`/`ArcCatList` into one parameterised queue. (iii) keep `CatList` as is for non-effects and add a separate effects-only `Store`-parameterised queue.
- Trade-offs: (i) is the smallest change and reuses the shared `CatList`, but only works if the internal Rc-sharing in `RcCatList`/`ArcCatList` is a sharing optimisation, not a correctness requirement for O(1) bind; (ii) fully collapses the queue family but parameterises shared infrastructure; (iii) isolates effects but risks a second queue spine.
- Recommendation: start from (i); before relying on it, confirm whether the Rc/Arc CatLists' internal pointer sharing is load-bearing for the O(1) stack-safe bind or merely avoids clones. If it is load-bearing, escalate to (ii) scoped to the effects queue.
- Reasoning: the erased form's value proposition is the O(1) `CatList` bind, so the queue's asymptotics must be preserved exactly; (i) is preferred for minimal surface but is explicitly gated on verifying the internal-sharing question rather than assumed, since guessing wrong here is a spine-level rewrite.

OQ-5D. Drop-safety of the parameterised spines. The shipped erased and concrete forms each have hand-written iterative `Drop` (`WrapDrop`) for deep chains; POC-11's concrete mirror and POC-8b's slice did not exercise iterative `Drop`.

- Approaches: (i) preserve each spine family's existing iterative `Drop` as the `Store`-parameterisation is applied, adding a per-`Store` deep-chain drop test. (ii) unify the drop logic across stores into one parameterised `Drop`.
- Trade-offs: (i) keeps a proven property and isolates risk per family, at the cost of near-duplicated drop code; (ii) is less code but moves a safety-critical, currently-proven path onto an unproven generic at the same time as everything else.
- Recommendation: (i): keep the existing iterative `Drop` per family and add deep-chain drop tests for each `Store` on the slice before the dual-row deletion.
- Reasoning: drop-safety is an acceptance-oracle guarantee (parity-oracle bucket A includes drop safety); changing the substrate and its drop strategy in one move would conflate two risks, so the conservative path tests the property explicitly rather than assuming the generic preserves it.

OQ-5E. Sequencing, and landing `ClosureStorage` in `src`. `ClosureStorage` exists only in the spike test files, not in `src`. The Box erased case is already what the `fs1` slice runs (reused public `Free` + Box `Coyoneda`), so the stores can be staged.

- Approaches: (i) port `ClosureStorage` into `src`, build the `Store`-parameterised substrate as its own module with per-`Store` unit tests (including the OQ-5A Arc+Coyoneda spike and OQ-5D drop tests), then point `fs1`'s row/interpreter at the Box instantiation (a near-noop, since Box is `Free`), then add an Rc and an Arc parity test on the slice. (ii) build all three stores into the slice at once.
- Trade-offs: (i) keeps the slice green throughout and stages risk Box -> Rc -> Arc (the Arc/Coyoneda unknown last), at the cost of an intermediate standalone module; (ii) reaches end-to-end coverage sooner but couples the substrate build, the Coyoneda unification, and the slice migration into one large unreviewable change.
- Recommendation: (i), staged Box -> Rc -> Arc, with the OQ-5A spike gating the Rc/Arc stores.
- Reasoning: it matches the adopted hybrid method (keep the crate compiling and the change reviewable) and front-loads the cheap, already-proven Box case while quarantining the one genuine unknown (Arc Coyoneda) behind a spike and the last stage.
