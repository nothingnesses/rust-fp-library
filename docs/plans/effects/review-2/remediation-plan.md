...
- Recommendation: (i), with the Box case implemented so its continuation queue is exactly `Free`'s (reuse, not re-encode); fall back to (ii) only if the Box case cannot be made structurally identical without an extra indirection.
- Reasoning: principle 1 (single free-monad spine) is satisfied by either (i) or (ii); the deciding factor is not destabilising shared infrastructure that has no effects stake, which favours (i). (iii) is rejected because it pays churn on non-effects code that the rebuild does not otherwise touch.
