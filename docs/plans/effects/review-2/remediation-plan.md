...
The first round of pre-implementation readiness questions surfaced by reading the as-built code (substrate identity, the confirming integration spike, migration strategy, async during the transition, the behaviour-parity oracle, and Phase A sequencing) have been resolved along the principle-aligned approaches and folded into the implementation steps: item 4's Grounding, adopted Method, and steps 1, 2, and 6 carry the substrate, spike, migration, and async decisions; the Phase A sequencing decision is recorded in the Sequencing overview and item 1. A further readiness question, the erased-versus-Explicit substrate axis (a second axis orthogonal to the pointer axis: `'static` + `dyn Any` + O(1) `CatList` bind versus non-`'static` + concrete payloads + recursive bind), surfaced on a later review pass; it has been adopted along the principle-aligned approach and folded into item 4 step 2, which now settles both the pointer and erasure axes of the substrate-identity decision. No open strategic questions remain. The substrate build-readiness questions for item 4 step 5 that surfaced from reading the substrate code, the erased-substrate type architecture, the continuation-queue, Coyoneda, and drop-safety parameterisations, and the build sequencing, have been adopted along the principle-aligned approaches and folded into item 4 step 5 as concrete sub-steps, each with proof-of-concept validation and a documented fallback to the next-best approach.

One build-readiness decision for step 5.2 surfaced on a closer reading of `free.rs` against the evidence-grounding and W8-trigger principles, and is recorded here pending adoption.

OQ-6B: how is the erased spine's per-`Store` continuation construction produced, and when is item 7's construction generator built? The construction-bearing spine methods (`bind`, `map`, `erase_type`, `lift_f`, all currently `pub fn`, plus the internal downcast/unbox/rebox helpers, about eight `Box::new` sites) must split per-`Store`, because the `FnOnce` (Box) versus `Fn` (Rc/Arc) boundary forbids one generic constructor (POC-8b); the `Store`-generic interpreter/invocation core (`to_view`, `evaluate`, `resume`, `Drop`) does not split. The public Box arm is the existing shipped `Free` with meaningful, must-preserve doctests; the Rc/Arc arms are new and effects-internal (`ClosureStorage` is `pub(crate)`). Two grounding facts remove the apparent feasibility unknowns: `document_module` accepts a defaulted, bounded type parameter (precedent `Lazy<'a, A, Config: LazyConfig = RcLazyConfig>` and `TryLazy`, which document the defaulted parameter in `#[document_type_parameters(...)]` and write it without the default on impls), and it documents non-public methods via `#[document_examples(skip_call_check, reason = ...)]` whose doctest exercises the public path (precedent `Free::from_raw_parts`, which is `pub(crate)`). So the question is not "can it be done" but "hand-write the bounded per-`Store` spine construction now, or stand up item 7's macro generator now", which the plan currently leaves contradictory: item 7's Decision text builds the generator at the step-7 catalog port or first Phase D port, while item 7 step 1 and step 5.2 say the generator "lands together" with 5.2.

Approaches:

- A (hand-write the spine arms now; defer the generator to its trigger). Keep the `Store`-generic core in one `impl<F, A, Store: ClosureStorage> Free<F, A, Store>`; keep the public Box construction arm hand-written and byte-identical (`impl<F, A> Free<F, A, BoxBrand>`, existing doctests preserved); hand-write the new `pub(crate)` Rc and Arc construction arms (`impl<F, A> Free<F, A, RcBrand>` and `...ArcBrand>`) using `#[document_examples(skip_call_check, ...)]`. Build item 7's construction generator later, at the step-7 catalog port (its genuine multi-consumer W8 trigger), using these hand-written spine arms as the `cargo expand` equivalence oracle. Trade-off: the Rc/Arc spine arms are written once by hand (bounded: three stores, a fixed small method set) and may later be re-expressed as generator output, but no macro infrastructure is built before a multi-consumer surface exists.
- B (build the generator now, against the spine as its first consumer; generate all three arms). Trade-off: a generator that abstracts a single bounded consumer before the per-effect catalog exists, risking a shape that must flex when the genuinely multi-consumer per-effect surface (smart constructors plus injection, a different shape than the spine's `bind`/`map`) arrives; and the generator must reproduce the existing rich Box doctests or they are lost.
- C (hybrid: hand-keep the Box arm, generate only Rc/Arc now). Trade-off: still builds macro infrastructure before the catalog, for only two generated arms, intermediate between A and B without A's deferral benefit.

Recommendation: A, judged against the Project Principles. Evidence-grounding (principle 5) favours the smallest validatable step: hand-writing the bounded per-`Store` spine arms proves the `FnOnce`/`Fn` seam on the real `Free` before any generator infrastructure, with the existing `Free`/`Trampoline`/`Thunk`/`Identity` suite as the Box byte-identical oracle and per-arm Rc/Arc `pure`/`bind`/`map` tests. The W8 trigger logic (item 7) explicitly builds the generator against a concrete multi-consumer port (catalog or Phase D), which the spine is not. Clean architecture (principle 3) does not favour a macro that abstracts a bounded three-store surface over three explicit arms until the N-effect catalog multiplies it. A also resolves the plan's internal contradiction in favour of item 7's own Decision text. The `skip_call_check` precedent removes the doctest cost that would otherwise argue for generation, and A loses nothing: the hand-written arms become the generator's equivalence oracle at step 7 (the intended W8 methodology), not throwaway work.
