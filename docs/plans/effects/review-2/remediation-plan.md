...
The first round of pre-implementation readiness questions surfaced by reading the as-built code (substrate identity, the confirming integration spike, migration strategy, async during the transition, the behaviour-parity oracle, and Phase A sequencing) have been resolved along the principle-aligned approaches and folded into the implementation steps: item 4's Grounding, adopted Method, and steps 1, 2, and 6 carry the substrate, spike, migration, and async decisions; the Phase A sequencing decision is recorded in the Sequencing overview and item 1. A further readiness question, the erased-versus-Explicit substrate axis (a second axis orthogonal to the pointer axis: `'static` + `dyn Any` + O(1) `CatList` bind versus non-`'static` + concrete payloads + recursive bind), surfaced on a later review pass; it has been adopted along the principle-aligned approach and folded into item 4 step 2, which now settles both the pointer and erasure axes of the substrate-identity decision. No open strategic questions remain. The substrate build-readiness questions for item 4 step 5 that surfaced from reading the substrate code, the erased-substrate type architecture, the continuation-queue, Coyoneda, and drop-safety parameterisations, and the build sequencing, have been adopted along the principle-aligned approaches and folded into item 4 step 5 as concrete sub-steps, each with proof-of-concept validation and a documented fallback to the next-best approach. The one build-readiness decision that surfaced on a closer reading of `free.rs` (OQ-6B: hand-write the bounded per-`Store` spine construction now versus stand up item 7's generator now) has been adopted along the principle-aligned approach (hand-write now, defer the generator to its W8 trigger) and folded into item 4 step 5.2 and item 7 step 1, also resolving the plan's internal contradiction over when item 7's generator is built.

A further build-readiness decision for step 5.2 surfaced on reading the stepping core itself, and is recorded here pending adoption.

OQ-6C: can the erased stepping core be a single `Store`-generic implementation, or must the stepping interpreter be written per-`Store`? The "interpreter/invocation core" is not purely invocation: `to_view`, `continue_from_erased`, and `continue_from_reboxed_erased` each construct a downcast/unbox continuation inline (a `Continuation<F>` built with `Box::new`), and `to_view` additionally uses a `std::cell::Cell<Option<...>>` to move the pending continuation queue into the `F::map` closure (the functor calls it exactly once). Two store-sensitivities follow. (a) The inline downcast/unbox continuations capture nothing (only the zero-size type parameter `A`), so they are `Fn + Send + Sync + 'static`-compatible and can be built for every store by one bridge, a `ClosureStorage::from_fn(f: impl Fn(I) -> O + Send + Sync + 'a) -> Self::Stored<'a, I, O>` (Box stores the `Fn` as `Box<dyn FnOnce>`, Rc as `Rc<dyn Fn>`, Arc as `Arc<dyn Fn + Send + Sync>`; the strict bound is harmless for Box and Rc). The `bind`/`map` continuations, which move the user's `FnOnce`, stay per-`Store` as already decided (OQ-6B). (b) The `Cell` one-shot is `Send` but not `Sync`, so the Arc (`Send + Sync`) store cannot reuse `to_view` as written: its `F::map` closure would not be `Sync`, and the resulting `Free` would not be `Send + Sync`. Note the underlying distinction: a Box continuation (`FnOnce`) is not `Clone`, forcing the move-once threading, whereas Rc/Arc continuations (`Fn`) are `Clone`; the move-once is correct for the single-element functor `to_view` assumes, so what (b) needs is a `Sync` one-shot for the Arc store, not a different algorithm. So (a) is resolvable and keeps construction-free invocation generic; (b) is the genuine unknown for keeping `to_view` itself generic across all three stores.

Approaches:

- A (keep the stepping core generic; resolve both sensitivities). Add `ClosureStorage::from_fn` (the `Fn + Send + Sync` bridge) for the capture-free internal continuations, and replace the `Cell` one-shot with an always-`Sync` one-shot move (for example a `Store`-provided one-shot cell that is `Cell` for the non-`Sync` stores and a `Sync` equivalent such as `Mutex<Option<...>>` for Arc, or restructure the queue threading so the `F::map` closure captures nothing consumable). Trade-off: a small addition to `ClosureStorage` plus a rewrite of `to_view`'s queue threading, but the stepping interpreter stays one definition.
- B (make the stepping interpreter per-`Store`). Keep only the construction-free accessors (`take_parts`, `from_raw_parts`, `cast_phantom`, the iterative `Drop`) generic; write `to_view`/`into_raw_step`/`evaluate` per-`Store` (Box hand-written byte-identical, Rc/Arc hand-written, differing only in continuation storage and the one-shot move). Trade-off: triples the stepping surface and its doctests (via `skip_call_check`), but each arm is mechanical and certain, with no `Cell`/`Sync` puzzle.
- C (split by threadedness). Share one generic stepping core for Box and Rc (both non-`Sync`, `Cell` is fine) and write only the Arc stepping arm separately. Trade-off: ad hoc two-of-three sharing, a maintenance seam that does not follow the `Store` abstraction cleanly.

Recommendation: A, with B as the documented fallback, judged against the Project Principles. Clean architecture (principle 3) prefers the single generic stepping core unless a concrete limitation prevents it; (a) is cleanly resolvable, and (b) reduces to providing a `Sync` one-shot cell for the Arc store, not a new algorithm. Evidence-grounding (principle 5) requires this be a proof-of-concept before the buildout: a generic `to_view` instantiated at Box, Rc, and Arc, run over a deep type-erased chain, with the Arc instantiation statically asserted `Send + Sync`. If the `Sync` one-shot cannot be made to work for the `F::map` closure (a concrete limitation), fall back to B (per-`Store` stepping), re-validated by the existing suite at Box plus Rc/Arc deep-chain tests; if B also fails, surface. C is rejected: it abandons the `Store` abstraction for an ad hoc Box-plus-Rc-versus-Arc seam, contradicting the coherence principle. What is left, once OQ-6C is adopted and folded, is execution.
