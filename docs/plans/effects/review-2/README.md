...
Foundation sweep outcome. After this review, the [foundation-sweep/](foundation-sweep/) investigation prototyped the most foundational decision (remediation item 4, dual rows versus a unified row) and concluded, across ten POCs and four decision gates, to adopt FS-1: a unified effect row with per-brand order markers, elaboration of higher-order effects, brand-keyed dispatch, and a single closure-storage-parameterised substrate, replacing the dual rows, boundary frames, positional dispatch, and six-wrapper duplication this review describes. The descriptive documents here (architecture.md, refactoring-opportunities.md) remain accurate as the as-built `2e0a7417` snapshot; the adopted direction and its consequences are folded into [remediation-plan.md](remediation-plan.md) (the FS-1 spine is item 4) and recorded in [foundation-sweep/charter.md](foundation-sweep/charter.md).
