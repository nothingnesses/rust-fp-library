...
Step 3 completed the convergence (commit `ed25ba3b` on `feat/effects-fs1`): the thirteen remaining built-ins ported onto `define_effect!`, so every fs1 slice effect is now a macro invocation and the built-ins are the macro's permanent conformance suite, and the proof-of-concept module was retired because the slice now covers every path it validated (the two-operation first-order effect via `State`, the `-> !` path via `Throw`, and the higher-order nominal-row knot via `Catch`). Two decisions were settled and folded into the port. `Identity` became a value-echoing operation `fn identity_op(value: i32) -> i32;` rather than the trivial functor `IdentityF<A>(A)`, because the emission model has no bare-hole cell shape (every variant ends in a continuation or `PhantomData`); the interpose walker and its oracle tests destructure the emitted enum, the faithful no-op being `|op| match op { IdentityF::IdentityOp(v, k) => k(v) }`. And the higher-order `Functor` arm was fixed to rebind its payload fields to positional names (`f: payload_0`, matching the first-order arm), because a payload named `f`, as the built-in `Censor`'s log transform is, otherwise shadowed the emission's map-function parameter and miscompiled; `Censor` ported with its `f` field intact as the conformance proof, and its `cargo expand` shows the rebinding directly. One ergonomic consequence of the row-generic constructors surfaced in the slice's own tests: a partial turbofish such as `throw::<()>()` no longer compiles once a constructor carries inferred `R`/`I` parameters, since Rust's turbofish is all-or-`_`, so the tests use `throw::<(), _, _>()` or let the result type infer from context; this is the idiom the step 4 custom-effects guide teaches. `just verify` and the effects-off build are green over the fully ported slice.
