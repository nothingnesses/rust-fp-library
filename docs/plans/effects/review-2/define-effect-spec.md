...
Status: draft under validation (item 11 step 1). The specification is validated in two halves per the adopted OQ-11A decision: the expressibility half (written spec instances for the Phase D shapes, no emission) is recorded in this document and has passed; the emission half (a proof-of-concept macro generating real code under test for `State`, `Throw`, and `Catch`) is recorded at the end and gates the step.
...
11. Stable kind-bound naming: emitted code never writes a hash-named generated kind trait (`Kind_cdc7cd43dac7585f`); `fp-library`'s `kinds` module gains a stable re-export alias for the `type Of<'a, T: 'a>: 'a` kind trait, `pub use ... as LifetimeUnaryKind`, and emitted bounds spell that alias. The alias lands with the emission proof of concept.
