...
**Next greenfield step: Phase 4 step 7.4.4a, add private carrier
raw-step extraction for default and erased shared wrappers.** B38 is
resolved via Option A: wrapper interpreters first get private
carrier-extraction boundaries, then the carrier-aware scoped-handler
list is wired through those boundaries. The constructor-stored
runner/carrier shape from B38 Option C stays on file as a fallback if
private raw-step extraction proves too invasive.
