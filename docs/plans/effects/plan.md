...
**Phase 4 step 3.1.2: `RefFunctor` impls for `BoxCatchBrand` and `CatchBrand` plus three brand-projection helpers.** One `feat(effects)` commit. Closes [B5](resolutions.md#resolved-2026-05-07-phase-4-step-3.1-sub-step-splitting-b5--b6-closed) (Option A: brand-projection helpers escape the trait impl's HRTB-bearing scope so the GAT projection normalizes against the concrete `BoxCatch` / `Catch` enum). Three `#[doc(hidden)]` helpers ship at [`fp-library/src/types/effects/catch.rs`](../../../fp-library/src/types/effects/catch.rs): `box_catch_action_ref` and `catch_action_ref` extract `&action` references; `catch_handler_ref` extracts the `&Rc<dyn Fn>` handler reference for the multi-shot path. Each helper takes a separate `'b` borrow lifetime (distinct from the catch effect's content lifetime `'a`) so the input reference's anonymous lifetime threads cleanly through the impl call site. `BoxCatchBrand<BoxBrand, E>::ref_map` builds the new catch with `func(action_ref)` and a `unreachable!`-stub handler (suppressed via `#[expect(clippy::unreachable, reason = "...")]` mirroring the [`unwrap_first` precedent](../../../fp-library/src/types/effects/arc_run.rs)) because `Box<dyn FnOnce>` cannot be cloned or invoked through a reference; the path is structurally unreachable in real programs since [`RunExplicitBrand: RefFunctor`](../../../fp-library/src/types/effects/run_explicit.rs#L1742) is reachable only through synthetic non-Coyoneda rows. `CatchBrand<RcBrand, E>::ref_map` is faithful: clones the `Rc<dyn Fn(E) -> A>` handler, shares `func` via `<RcBrand as ToDynCloneFn>::ref_new`, then post-composes for both the action call and the new handler closure. `SendCatchBrand<ArcBrand, E>` deliberately omits `RefFunctor`: the cascade through [`ArcRunExplicitBrand`](../../../fp-library/src/brands/effects.rs) does not require it (`ArcFreeExplicitBrand: !RefFunctor` per the brand's docs) and the `func: Fn(&A) -> B + 'a` bound lacks `Send + Sync` for closure storage, mirroring the [`SendCatchBrand`-no-`Functor` precedent](deviations.md). 5 new doctests (3 for the helpers, 2 for the `RefFunctor` impls); total catch.rs doctests now 16. `just verify` clean.

**Phase 4 step 3.1.1 (commit `abd3d1a3`): `Catch` foundational scaffold.** One `feat(effects)` commit. Three sibling effect types at [`fp-library/src/types/effects/catch.rs`](../../../fp-library/src/types/effects/catch.rs) (`BoxCatch<'a, P, E, A>` where `P: ToDynFnOnce`, `Catch<'a, P, E, A>` where `P: ToDynCloneFn`, `SendCatch<'a, P, E, A>` where `P: ToDynSendFn`) each holding a `Catch { action, handler }` variant whose handler is `<P as Pointer / RefCountedPointer / SendRefCountedPointer>::Of<'a, dyn 'a + FnOnce / Fn / Fn + Send + Sync>(E) -> A`. Three brand declarations at [`fp-library/src/brands/effects.rs`](../../../fp-library/src/brands/effects.rs) (`BoxCatchBrand<P, E>`, `CatchBrand<P, E>`, `SendCatchBrand<P, E>`) plus the `impl_kind!` registrations. Four of five substrate-required trait impls per brand: `Functor` on Box+Rc flavours (specialised via `ToDynFnOnce::new` / `ToDynCloneFn::new`); `SendFunctor` across all three (load-bearing on `SendCatchBrand<ArcBrand, E>`, stub-delegating-to-`Functor::map` on the Box and Rc flavours since they don't appear in Arc-substrate scoped rows at runtime); `WrapDrop` and `Extract` across all three (returning `Some(action)` / `action`, dropping the handler). The fifth trait `RefFunctor` lands in step 3.1.2. Module wired through [`fp-library/src/types/effects.rs`](../../../fp-library/src/types/effects.rs)'s `pub mod catch;`. 11 doctests passing. `SendCatchBrand` does NOT implement `Functor`: the trait's `f: impl Fn(A) -> B + 'a` lacks the `Send + Sync` bounds needed for `<ArcBrand as ToDynSendFn>::new` closure storage; deviation logged at [deviations.md Phase 4 step 3.1.1](deviations.md), mirroring Phase 3's `SendStateBrand` precedent.
...
No active blockers. The B5 (`RefFunctor` GAT-normalization on scoped-effect closure-cell brands) and B6 (step 3.1 sub-step splitting) items closed on 2026-05-07 per the [resolved B5 / B6 entry](resolutions.md#resolved-2026-05-07-phase-4-step-3.1-sub-step-splitting-b5--b6-closed): Option A adopted for B5 (brand-projection helper analogous to [`arc_run::unwrap_first`](../../../fp-library/src/types/effects/arc_run.rs)); Option B adopted for B6 (4-commit split into 3.1.1 foundational scaffold / 3.1.2 `RefFunctor` + helpers / 3.1.3 smart constructors / 3.1.4 integration tests). 3.1.1 shipped at `abd3d1a3`; 3.1.2 in working tree.
