...
**Phase 4 step 1: `ScopedCoproduct<H, T>` and `ScopedNil` row-encoding aliases at [`fp-library/src/types/effects/scoped.rs`](../../../fp-library/src/types/effects/scoped.rs).** One `feat(effects)` commit. Adds a new `scoped` submodule under [`fp-library::types::effects`](../../../fp-library/src/types/effects.rs) exporting two transparent type aliases: `ScopedCoproduct<H, T> = CoproductBrand<H, T>` and `ScopedNil = CNilBrand`. The aliases inherit all trait impls verbatim (transparent at the Rust type level) and introduce no new substrate machinery; the dual-row integration into `Run<R, S, A>` (and the five sibling Run wrappers) was already in place via [`NodeBrand<R, S>`](../../../fp-library/src/brands/effects.rs) since Phase 2. The naming layer's purpose is self-documenting cue at user-facing type signatures so Phase 4 program declarations like `Run<FirstRow, ScopedCoproduct<CatchBrand<P, E>, ScopedNil>, A>` read directly as "first-order row, scoped row containing Catch" without reader disambiguation between the two `CoproductBrand` instances. Module-level docs explain the no-Coyoneda-on-scoped rationale (scoped effects dispatched via case analysis rather than `Functor::map`, deferred to step 4's `DispatchScopedHandlers` trait) and enumerate the substrate-required traits each scoped-effect brand must implement (`Functor` / `SendFunctor` / `WrapDrop` / `RefFunctor` / `Extract`). Two regression tests confirm the alias transparency: `scoped_nil_is_alias_for_cnil_brand` constructs identical `RcRun` programs through both spellings and interprets both to the same value; `scoped_coproduct_is_alias_for_coproduct_brand` shows a `CoproductBrand`-typed value passes unchanged through a `ScopedCoproduct`-typed parameter. `just verify` clean.
