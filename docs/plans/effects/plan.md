...
**Phase 4 step 0: POC 3 substrate-primitive validation (`interpret_with_either` on `RcRun`).** One `test(effects)` commit. Lands a standalone validation POC at [`fp-library/tests/poc_rc_run_interpret_with_either.rs`](../../../fp-library/tests/poc_rc_run_interpret_with_either.rs) that proves the `interpret_with_either<EBrand, Idx>(self, fo_handlers) -> Result<A, EBrand::Op>` substrate primitive shape compiles and short-circuits correctly on a concrete two-effect row (`Identity` FO-dispatched, `ExceptBrand<String>` matched-and-short-circuited). The POC's `interpret_with_either_except` function uses literal `Coproduct` pattern-matching for the concrete row; the generic shape across all six Run wrappers is mechanical from this template (replace concrete coproduct match with `Member::project::<EBrand, Idx>` and add the standard `interpret_with_shared` bounds at [rc_run.rs:992-1023](../../../fp-library/src/types/effects/rc_run.rs#L992-L1023)). Five tests cover Pure (returns `Ok(value)`), single Throw (short-circuits to `Err(error)`), Identity-then-Throw (FO arm dispatches before short-circuit fires), Identity-only (runs to completion via FO handler, returns `Ok`), and a Catch-dispatcher-shape simulation (matched payload is shape-compatible with downstream handler invocation). Adopted per the [K1 resolution](resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries); generic rollout across all six Run wrappers ships in step 2a; the `Catch` cons-cell in step 4 consumes the validated primitive. `just verify` clean.
