...
**Phase 4 step 2.2: `Run::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the default Erased Run wrapper.** One `feat(effects)` commit. Lands the `interpose` primitive at [`fp-library/src/types/effects/run.rs`](../../../fp-library/src/types/effects/run.rs); mechanical translation of [step 2.1's RcRun template](../../../fp-library/src/types/effects/rc_run.rs) with substrate paths swapping `RcFree` -> `Free` (via [`Free::wrap`](../../../fp-library/src/types/free.rs)), `RcCoyoneda` -> `Coyoneda` (via [`Coyoneda::lower`](../../../fp-library/src/types/coyoneda.rs), the consuming variant for single-shot programs), `RcRun::from_rc_free` / `into_rc_free` -> `Run::from_free` / `into_free`. Both wrappers reuse `RcBrand` for the closure-refcount mechanism (`BoxBrand` cannot refcount; the substrate-pointer difference is in the program tree shape, not in the user-closure refcount), so the public/inner split (`interpose` wraps in `Rc<F>` once, `interpose_shared` recurses with `Rc<F>` clones) is identical between wrappers. Same `EmbedIndices` extra type parameter inherits 2.1's [deviation entry](deviations.md). Four parallel integration tests added at [`fp-library/tests/run_interpose.rs`](../../../fp-library/tests/run_interpose.rs) under a `// -- Run --` divider, with the existing 2.1 RcRun tests renamed `rc_run_t1`-`rc_run_t4` for wrapper-prefixed parity (matching the [`run_interpret.rs`](../../../fp-library/tests/run_interpret.rs) convention); covers T1 single-effect no-op, T2 single-effect constant replacement, T3 / T4 dual-row embed-back path runtime-fires at both head and tail positions. The 4 remaining wrapper interpose primitives (ArcRun, RunExplicit, RcRunExplicit, ArcRunExplicit) follow as sub-steps 2.3-2.6; R1 / R2 risks surface in their respective wrapper commits. `just verify` clean.
