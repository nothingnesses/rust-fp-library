...
**Next greenfield step: Phase 4 step 7.4.4c.1d, prove the other
around-action effects still fit.** Steps
...
an indexed boundary. The next step is to move Local / RefLocal, Catch,
Bracket, and RefBracket from carrier-cell proofs toward the
action-owned scoped-row projection path while preserving their existing
semantics. Prior carrier-cell proofs to reuse as regression coverage:
