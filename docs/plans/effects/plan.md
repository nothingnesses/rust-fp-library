...
**Phase 4 step 3.2.2 (commit `cbe401a4`): Local (Val flavour) `RefFunctor` impls + brand-projection helpers.** One `feat(effects)` commit. Two `#[doc(hidden)]` helpers ship at [`fp-library/src/types/effects/local.rs`](../../../fp-library/src/types/effects/local.rs): `local_modify_ref` extracts the `&Rc<dyn Fn(E) -> E>` modify reference; `local_action_thunk_ref` extracts the `&Rc<dyn Fn(()) -> A>` action thunk reference. Each helper takes a separate `'b` borrow lifetime (distinct from the local effect's content lifetime `'a`) so the input reference's anonymous lifetime threads cleanly through the impl call site. `BoxLocalBrand<BoxBrand, E>::ref_map` builds the new local with `unreachable!`-stub modify and action closures (suppressed via `#[expect(clippy::unreachable, reason = "...")]` mirroring the `BoxCatchBrand` precedent) because both `Box<dyn FnOnce(E) -> E>` and `Box<dyn FnOnce(()) -> A>` cannot be replicated through a reference; the path is structurally unreachable in real programs since `RunExplicitBrand: RefFunctor` is reachable only through synthetic non-Coyoneda rows. `LocalBrand<RcBrand, E>::ref_map` is faithful: clones the modify Rc (preserved unchanged because the modify signature `E -> E` does not depend on the result type), clones the action thunk's Rc, and post-composes `func` over the action's output. `SendLocalBrand<ArcBrand, E>` deliberately omits `RefFunctor`: cascade through `ArcRunExplicitBrand` does not require it and the `func: Fn(&A) -> B + 'a` bound lacks `Send + Sync` for closure storage, mirroring the `SendCatchBrand`-no-`RefFunctor` precedent. Compared to Catch's `RefFunctor` impl, Local's pattern is simpler because `func` is only used by the action (modify is `E -> E`, untouched by the result-type map); no Rc-sharing of `func` between two closures is needed. 4 new doctests (2 for the helpers, 2 for the `RefFunctor` impls); total local.rs doctests now 17. `just verify` clean.
