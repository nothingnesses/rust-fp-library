...
**Continue Phase 4 step 4: wrapper interpreter plumbing.** Wire the
six Run wrappers' interpreter loops to accept a scoped-handler list,
dispatch `Node::First` through `DispatchHandlers`, and dispatch
`Node::Scoped` through `DispatchScopedHandlers` while threading the
first-order handler list into scoped handlers. Keep the Q4-proven
static-dispatch shape unless the full implementation surfaces a new,
concrete compiler wall; if it does, record a new active blocker before
switching to the trait-object fallback.
