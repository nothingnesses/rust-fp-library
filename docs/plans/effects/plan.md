...
**Next greenfield step: finish Phase 4 step 6a fan-out.** The `Run` /
`RcRun` proof slice is verified: both wrappers now expose
`interpret_scoped_with`, and their `interpret_with` / `interpose`
primitives preserve nested scoped operations. Integration coverage lives
in [`fp-library/tests/run_scoped_row_primitives.rs`](../../../fp-library/tests/run_scoped_row_primitives.rs)
for nested `Span` preservation across first-order row narrowing and
interpose-style replacement. Next, fan the same retrofit out to
`ArcRun`, `RunExplicit`, `RcRunExplicit`, and `ArcRunExplicit`, reusing
the established Arc-family HRTB helper pattern where needed. B27 closed
via Option A: keep `interpret_with_either` as the first-order-only
primitive shipped for `S = CNilBrand`, and build `CatchDispatcher` on
scoped-row-preserving `interpose::<ExceptBrand<_>, _, _, _>` in step
7.1.

After step 6a covers all six wrappers, implement the standard
scoped-handler dispatcher set (`LocalDispatcher`, `RefLocalDispatcher`,
