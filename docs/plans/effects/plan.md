...
**Phase 4 step 3.2.7: RefLocal (Ref flavour) smart constructors per wrapper.** One `feat(effects)` commit. Six per-wrapper `ref_local` smart constructors land at [`run.rs`](../../../fp-library/src/types/effects/run.rs) (`Run::ref_local`), [`rc_run.rs`](../../../fp-library/src/types/effects/rc_run.rs) (`RcRun::ref_local`), [`arc_run.rs`](../../../fp-library/src/types/effects/arc_run.rs) (`ArcRun::ref_local`), [`run_explicit.rs`](../../../fp-library/src/types/effects/run_explicit.rs) (`RunExplicit::ref_local`), [`rc_run_explicit.rs`](../../../fp-library/src/types/effects/rc_run_explicit.rs) (`RcRunExplicit::ref_local`), and [`arc_run_explicit.rs`](../../../fp-library/src/types/effects/arc_run_explicit.rs) (`ArcRunExplicit::ref_local`); each takes `(modify, action)` (PureScript Run's `local` argument convention) and returns a wrapper-typed program suspended at the scoped Local layer (verified via `peel().is_err()` doctests). Per-wrapper bound shapes mirror `local`'s: BoxRefLocalBrand (`'static` lifetime, `FnOnce(&E) -> E` single-shot) for default Run/RunExplicit; RefLocalBrand (`Rc + Fn(&E) -> E`, `A: Clone` for Erased / Explicit) for Rc family; SendRefLocalBrand (`Arc + Fn(&E) -> E + Send + Sync`, `A: Clone + Send + Sync`) for Arc family. ArcRun::ref*local reuses the existing `make_node_scoped` / `wrap_first_arc` HRTB-poisoning workaround helpers landed in step 3.1.3; ArcRunExplicit::ref_local needs no such helpers. RunExplicit::ref_local carries `#[expect(clippy::type_complexity)]` for the same reason `RunExplicit::local` does (`Box<FreeExplicit<...>>` deep-chain). The implementation is mechanically derived from the corresponding `local` smart constructors with three surface-level swaps: `modify` parameter signature changes from `Fn(E) -> E` to `Fn(&E) -> E` (or `FnOnce(&E) -> E` for Box); the modify field constructed via `<P>::ref_new` (closing the matrix gap landed in step 3.2.5) instead of `<P>::new`; and the cell type swapped from `BoxLocal` / `Local` / `SendLocal` to `BoxRefLocal` / `RefLocal` / `SendRefLocal`. The action B-thunk wrapping is identical to the Val cycle. The smart-constructor name `ref_local` follows the `ref*_`prefix convention used uniformly across the codebase (mirrors`Functor::map`/`RefFunctor::ref*map`, `<P>::new`/`<P>::ref_new`, `local*_`helpers /`ref*local*\*`helpers). 6 smart-constructor doctests (one per wrapper).`just verify` clean.
