...
**Phase 4 step 2.1: `RcRun::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive.** One `feat(effects)` commit. Lands the first per-wrapper `interpose` primitive at [`fp-library/src/types/effects/rc_run.rs`](../../../fp-library/src/types/effects/rc_run.rs); generalises [POC 2's template](../../../fp-library/tests/poc_rc_run_interpose.rs) (one-effect concrete row) to a production shape that walks an arbitrary first-order row with arbitrary `EBrand` position. Public `interpose` wraps the user closure once via [`<RcBrand as RefCountedPointer>::new`](../../../fp-library/src/classes/ref_counted_pointer.rs); inner `interpose_shared` recurses with `Rc<F>` clones (refcount bumps) so the user-facing API drops the `Clone` bound on the closure, mirroring the [`interpret_with` / `interpret_with_shared`](../../../fp-library/src/types/effects/rc_run.rs#L913-L1063) pattern. Implementation: peel + `Member::project<RcCoyoneda<EBrand, Self>, Idx>`; matched arm lowers the coyoneda, recurses interpose on inner sub-programs via `<EBrand as Functor>::map`, then applies the replacement; unmatched arm walks inner programs of `Self::Remainder` (RMinusE-typed) via `<RMinusE as Functor>::map`, embeds the rebuilt layer back to R via [`CoproductEmbedder<R::Of<...>, EmbedIndices>`](../../../fp-library/src/types/effects/coproduct.rs), and rewraps as `RcFree::wrap(Node::First(...))`. The `EmbedIndices` extra type parameter beyond plan.md's sketched `<EBrand, Idx>` signature is a structural necessity (frunk's `CoproductEmbedder` indices cannot be inferred from `Idx` alone); deviation logged. Four integration tests at [`fp-library/tests/run_interpose.rs`](../../../fp-library/tests/run_interpose.rs) cover: T1 single-effect no-op interpose; T2 single-effect constant replacement; T3 two-effect row, target = head, program emits tail-only -> embed-back path runtime-fires; T4 two-effect row, target = tail (`There<Here>`), program emits head-only -> embed-back path runtime-fires at the head position. The 5 remaining wrapper interpose primitives (Run, ArcRun, RunExplicit, RcRunExplicit, ArcRunExplicit) follow as sub-steps 2.2-2.6; R1 / R2 risks surface in their respective wrapper commits. `just verify` clean.
