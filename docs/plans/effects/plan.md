...
- **Phase 4** (scoped effects via heftia-inspired dual row): design adopted, implementation-kickoff sequencing adopted, ready to start. The Phase 4 design review ([`review/1_scoped_effects_design/review_phase_4_design.md`](review/1_scoped_effects_design/review_phase_4_design.md)) and its remediation report ([`review/1_scoped_effects_design/remediation_proposals_phase_4.md`](review/1_scoped_effects_design/remediation_proposals_phase_4.md)) shipped, with two POC validations ([`fp-library/tests/poc_send_catch_brand.rs`](../../../fp-library/tests/poc_send_catch_brand.rs) for the F2 parallel-Send-brand pattern, [`fp-library/tests/poc_rc_run_interpose.rs`](../../../fp-library/tests/poc_rc_run_interpose.rs) for the F1 substrate-level `Run::interpose` primitive). The 2026-05-06 K1 / K2 [implementation-kickoff sequencing resolution](resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries) sets POC 3 (`interpret_with_either` substrate primitive on `RcRun`) as the next commit (Phase 4 step 0, standalone validation), with plan.md step numbering authoritative for commit boundaries. Phase 4 ships `Catch<'a, P, E, A>`, `Local<'a, P, E, A>` / `RefLocal`, `Bracket<'a, P, A, B>` / `RefBracket`, and `Span<'a, Tag>` scoped-effect constructors; a parallel `DispatchScopedHandlers` trait; substrate-level `Run::interpose` and `interpret_with_either` primitives on each Run wrapper.

### Next greenfield work

Phase 3 closes with step 8; Phase 3.5 closes with sub-step 5. The next concrete work is **Phase 4 implementation**, starting with **step 0**: standalone POC 3 validation commit at [`fp-library/tests/poc_rc_run_interpret_with_either.rs`](../../../fp-library/tests/) (per the [K1 resolution](resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries)). Phase 4 then proceeds through plan.md steps 1, 2, 2a, 3, 4, 5, 6, 7, 8 in order with plan.md numbering as the authoritative commit boundary (per K2): substrate-level `Run::interpose` (POC 2 validated) and `interpret_with_either` (POC 3 in step 0) primitives, `DispatchScopedHandlers` trait + per-wrapper interpret rewrite (R1 implementation); standard scoped-effect constructors using the Phase 3.5 retrofit's pointer-brand pattern (R2 implementation); bracket dispatcher with Drop-guard for panic safety (M3); standard scoped-effect rollout (`Catch`, `Local` / `RefLocal`, `Bracket` / `RefBracket`, `Span` plus `scoped_effects!` and `scoped_handlers!` macros via the new `handler_list_emitter` helper module shared with `handlers!`); standard scoped handlers; review-remediation documentation pass closing Phase 4. The Q4 / R1 / R2 half-day prototypes land during R1 implementation kickoff (alongside steps 1-4 substrate work); the R3 benchmark commit lands alongside the standard scoped-effect rollout (step 6 onward). Sub-step planning lives in plan.md's [Phase 4 section](#phase-4-scoped-effects-heftia-inspired-dual-row) below.
...
No active blockers. The eight Phase 4 pre-implementation design questions B1-B4 + Q1-Q3 + Q5 closed via the 2026-05-05 design-adoption commit `6e960701` (and Phase 3.5 retrofit landings `b067f912` / `a762fa27` / `89546709` / `4471629d` for B3 implementation); the two implementation-kickoff sequencing decisions K1 + K2 closed 2026-05-06 (Option A adopted for both: POC 3 standalone commit first; plan.md numbering authoritative). Compact summaries with plan-text-amendment pointers live in [resolutions.md](resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries) (K1, K2) and [resolutions.md](resolutions.md#resolved-2026-05-05-phase-4-pre-implementation-design-questions-b1-b4-q1-q3-q5-closed-by-design-adoption-commit-6e960701) (B1-B4 / Q1-Q3 / Q5).
