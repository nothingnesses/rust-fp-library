...
`interpose::<ExceptBrand<_>, _, _, _>` path and implement
`SpanDispatcher` as an around-action dispatcher that observes the
by-value tag and returns the action result unchanged. B25 and B26 are
