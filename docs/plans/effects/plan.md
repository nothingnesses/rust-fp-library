...
**Phase 4 step 3.2.6: RefLocal (Ref flavour) `RefFunctor` impls + brand-projection helpers.** One `feat(effects)` commit. Mirrors step 3.2.2's pattern for the Ref flavour. Two `#[doc(hidden)]` helpers ship at [`fp-library/src/types/effects/ref_local.rs`](../../../fp-library/src/types/effects/ref_local.rs): `ref_local_modify_ref` extracts the `&Rc<dyn Fn(&E) -> E>` modify reference; `ref_local_action_thunk_ref` extracts the `&Rc<dyn Fn(()) -> A>` action thunk reference. Each helper takes a separate `'b` borrow lifetime distinct from the ref-local effect's content lifetime `'a` so the input reference's anonymous lifetime threads cleanly through the impl call site. `BoxRefLocalBrand<BoxBrand, E>::ref_map` builds the new ref-local with `unreachable!`-stub modify and action closures (suppressed via `#[expect(clippy::unreachable, reason = "...")]` mirroring the `BoxLocalBrand` precedent) because both `Box<dyn FnOnce(&E) -> E>` and `Box<dyn FnOnce(()) -> A>` cannot be replicated through a reference; the path is structurally unreachable in real programs since `RunExplicitBrand: RefFunctor` is reachable only through synthetic non-Coyoneda rows. `RefLocalBrand<RcBrand, E>::ref_map` is faithful: clones the modify Rc (preserved unchanged because the modify signature `&E -> E` does not depend on the result type), clones the action thunk's Rc, and post-composes `func` over the action's output via `<RcBrand as ToDynCloneFn>::new`. `SendRefLocalBrand<ArcBrand, E>` deliberately omits `RefFunctor`: cascade through `ArcRunExplicitBrand` does not require it and the `func: Fn(&A) -> B + 'a` bound lacks `Send + Sync` for closure storage, mirroring the `SendLocalBrand`-no-`RefFunctor` and `SendCatchBrand`-no-`RefFunctor` precedents. Compared to step 3.2.2's Val-cycle impl, the only shape differences are: the helper return type `&Rc<dyn Fn(&E) -> E>` (Ref) vs `&Rc<dyn Fn(E) -> E>` (Val) and the doctest closure shape `|e: &i32| *e + 1` (Ref) vs `|e: i32| e + 1` (Val); the RefFunctor::ref_map composition pattern itself is identical because `func` only goes through the action thunk (modify is preserved unchanged in both flavours). 4 new doctests (2 for the helpers, 2 for the `RefFunctor` impls); total ref_local.rs doctests now 17. `just verify` clean.

**Phase 4 step 3.2.5 (commit `1668b2e5`): RefLocal (Ref flavour) foundational scaffold + `ToDynFnOnce::ref_new` matrix completion.** One `feat(effects)` commit. Closed B11 + B12 (closure-trait matrix gap on `ToDynFnOnce`; variant naming uniformly `Local`). Four pieces in one focused unit. **Closure-trait matrix completion (B11):** [`ToDynFnOnce`](../../../fp-library/src/classes/to_dyn_fn_once.rs) gained `ref_new<'a, A: 'a, B: 'a>(f: impl 'a + FnOnce(&A) -> B) -> Self::Of<'a, dyn 'a + FnOnce(&A) -> B>` plus a free-function shim `to_ref_dyn_fn_once`; `BoxBrand`'s impl gained the parallel `ref_new` returning `Box::new(f)` at [`box_ptr.rs`](../../../fp-library/src/types/box_ptr.rs). The matrix in [`pointer-abstraction.md`](../../../fp-library/docs/pointer-abstraction.md) is now structurally complete: every closure trait ships both `new` and `ref_new`. **RefLocal foundational scaffold:** three sibling effect types at [`ref_local.rs`](../../../fp-library/src/types/effects/ref_local.rs) (`BoxRefLocal<'a, P, E, A>` where `P: ToDynFnOnce`, `RefLocal<'a, P, E, A>` where `P: ToDynCloneFn`, `SendRefLocal<'a, P, E, A>` where `P: ToDynSendFn`) each holding a `Local { modify, action }` variant (B12: variant name uniformly `Local`) with per-pointer-brand pointer storage: `modify: <P>::Of<'a, dyn ClosureTrait(&E) -> E>` (1-arg, borrows env per Ref flavour) and `action: <P>::Of<'a, dyn ClosureTrait(()) -> A>` (B-thunk for layout-cycle break per the closed B9 decision). Three brand declarations at [`brands/effects.rs`](../../../fp-library/src/brands/effects.rs) plus `impl_kind!` registrations. Four of five substrate-required trait impls per brand: `Functor` on `BoxRefLocalBrand<BoxBrand, E>` and `RefLocalBrand<RcBrand, E>`; `SendFunctor` across all three (load-bearing on `SendRefLocalBrand<ArcBrand, E>`, stub-delegating-to-`Functor::map` on Box and Rc flavours); `WrapDrop` and `Extract` across all three. Manual `Clone` impls for `RefLocal` (Rc-bump on both pointers) and `SendRefLocal` (Arc-bump); `BoxRefLocal` is unconditionally non-`Clone`. `SendRefLocalBrand` does NOT implement `Functor` for the same `Send + Sync` bound mismatch as `SendLocalBrand` and `SendCatchBrand`. Module wired through [`fp-library/src/types/effects.rs`](../../../fp-library/src/types/effects.rs)'s `pub mod ref_local;`. 13 doctests passing.
