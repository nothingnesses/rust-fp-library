...
- **Phase 4** (scoped effects via heftia-inspired dual row): steps 0-1 plus step 2 (sub-steps 2.1-2.6) plus step 2a shipped; **step 3.1.1 (Catch foundational scaffold) implementation-ready but blocked on active blockers [B5](#b5-reffunctor-gat-normalization-for-scoped-effect-closure-cell-brands) (RefFunctor GAT-normalization) and [B6](#b6-step-31-sub-step-splitting-given-the-b5-deferral) (sub-step splitting decision) before commit**. Step 2a closure delivered the substrate-level `interpret_with_either<EBrand, Idx, RMinusE>(self, fo_handlers) -> Result<A, EBrand::Of<'a, Self>>` primitive across all six Run wrappers, generalising [POC 3's](../../../fp-library/tests/poc_rc_run_interpret_with_either.rs) concrete two-effect-row template. 24 integration tests at [run_interpret_with_either.rs](../../../fp-library/tests/run_interpret_with_either.rs) (4 per wrapper, mirroring POC 3's coverage: T1 Pure -> Ok, T2 single matched -> Err, T3 FO-then-matched -> Err after FO dispatch, T4 FO-only -> Ok) plus 6 doctests. Steps 3-8 follow. The Phase 4 design review ([`review/1_scoped_effects_design/review_phase_4_design.md`](review/1_scoped_effects_design/review_phase_4_design.md)) and its remediation report ([`review/1_scoped_effects_design/remediation_proposals_phase_4.md`](review/1_scoped_effects_design/remediation_proposals_phase_4.md)) shipped earlier, with three POC validations now complete ([`poc_send_catch_brand.rs`](../../../fp-library/tests/poc_send_catch_brand.rs) for the F2 parallel-Send-brand pattern; [`poc_rc_run_interpose.rs`](../../../fp-library/tests/poc_rc_run_interpose.rs) for the F1 substrate-level `Run::interpose` primitive; [`poc_rc_run_interpret_with_either.rs`](../../../fp-library/tests/poc_rc_run_interpret_with_either.rs) for the B4 substrate-level `interpret_with_either` primitive). The 2026-05-06 K1 / K2 [implementation-kickoff sequencing resolution](resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries) set POC 3's standalone-commit-first ordering and plan.md step numbering as the authoritative commit boundary. Phase 4 ships `Catch<'a, P, E, A>`, `Local<'a, P, E, A>` / `RefLocal`, `Bracket<'a, P, A, B>` / `RefBracket`, and `Span<'a, Tag>` scoped-effect constructors; a parallel `DispatchScopedHandlers` trait; substrate-level `Run::interpose` and `interpret_with_either` primitives on each Run wrapper.

### Next greenfield work

Phase 4 steps 0-1 plus 2.1-2.6 plus 2a shipped (steps 2 and 2a closed). **Phase 4 step 3.1.1** (`Catch` foundational scaffold) is implementation-ready in the working tree but blocked on the [B5 / B6 active-blocker resolutions](#active-blockers) before it can commit. The foundational scaffold ships three sibling types (`BoxCatch<'a, P, E, A>` where `P: ToDynFnOnce`, `Catch<'a, P, E, A>` where `P: ToDynCloneFn`, `SendCatch<'a, P, E, A>` where `P: ToDynSendFn`) at [`fp-library/src/types/effects/catch.rs`](../../../fp-library/src/types/effects/catch.rs) with three brands (`BoxCatchBrand<P, E>`, `CatchBrand<P, E>`, `SendCatchBrand<P, E>`) at [`brands/effects.rs`](../../../fp-library/src/brands/effects.rs) plus four of the five substrate-required trait impls per brand (`Functor` on Box+Rc flavours, `SendFunctor` across all three with the SendCatch impl load-bearing, `WrapDrop` across all three, `Extract` across all three); the fifth trait `RefFunctor` is deferred per [B5](#b5-reffunctor-gat-normalization-for-scoped-effect-closure-cell-brands). The 3-sibling pattern matches [decisions.md section 4.5](decisions.md) and [plan.md's step 3 detail](#phase-4-scoped-effects-heftia-inspired-dual-row); the POC at [`poc_send_catch_brand.rs`](../../../fp-library/tests/poc_send_catch_brand.rs) validates the parallel-Send-brand portion (Box+FnOnce path); the Rc-Fn and Arc-Fn paths inherit Phase 3.5's State pattern's empirical validation. Step 3 splits per the per-step protocol: 3.1 Catch (sub-split per [B6](#b6-step-31-sub-step-splitting-given-the-b5-deferral) into 3.1.1-3.1.4), 3.2 Local / RefLocal, 3.3 Bracket / RefBracket, 3.4 Span. R2 risk (Send+Sync threading through scoped-effect closure cells) surfaced cleanly via the parallel-Send-brand pattern; one structural deviation from scoped.rs's "all five required" claim is logged at [deviations.md Phase 4 step 3.1.1](deviations.md): `SendCatchBrand` ships only with `SendFunctor` (not `Functor`), mirroring the Phase 3 [`SendStateBrand` precedent](../../../fp-library/src/brands/effects.rs). Step 4 introduces the `DispatchScopedHandlers` trait + per-wrapper interpret rewrite (R1 implementation kickoff). Step 5 lands the `scoped_effects!` and `scoped_handlers!` macros via the new `handler_list_emitter` helper module shared with `handlers!`. Step 6 implements the bracket dispatcher with Drop-guard for panic safety (M3). Step 7 lands standard scoped-handler implementations consuming `interpret_with_either` (Catch's recovery path) and `interpose` (Local's environment-replacement path). Step 8 closes Phase 4 with the review-remediation documentation pass. Phase 4 then proceeds through plan.md steps 2a, 3, 4, 5, 6, 7, 8 in order with plan.md numbering as the authoritative commit boundary (per K2): substrate-level `Run::interpose` (POC 2 validated) and `interpret_with_either` (POC 3 validated, generic rollout in step 2a) primitives; `DispatchScopedHandlers` trait + per-wrapper interpret rewrite (R1 implementation); standard scoped-effect constructors using the Phase 3.5 retrofit's pointer-brand pattern (R2 implementation); bracket dispatcher with Drop-guard for panic safety (M3); standard scoped-effect rollout (`Catch`, `Local` / `RefLocal`, `Bracket` / `RefBracket`, `Span` plus `scoped_effects!` and `scoped_handlers!` macros via the new `handler_list_emitter` helper module shared with `handlers!`); standard scoped handlers; review-remediation documentation pass closing Phase 4. The Q4 / R1 / R2 half-day prototypes land during R1 implementation kickoff (alongside steps 1-4 substrate work); the R3 benchmark commit lands alongside the standard scoped-effect rollout (step 6 onward). Sub-step planning lives in plan.md's [Phase 4 section](#phase-4-scoped-effects-heftia-inspired-dual-row) below.
...
**Phase 4 step 2a: substrate-level `interpret_with_either<EBrand, Idx, RMinusE>` primitive across all six Run wrappers.** One `feat(effects)` commit. Lands the matched-effect short-circuit primitive at all six Run-wrapper sites (rc*run.rs, run.rs, arc_run.rs, run_explicit.rs, rc_run_explicit.rs, arc_run_explicit.rs); generalises [POC 3's concrete two-effect-row template](../../../fp-library/tests/poc_rc_run_interpret_with_either.rs) to a production shape walking arbitrary first-order rows with arbitrary `EBrand` matched-effect position. Returns `Result<A, <EBrand as Kind>::Of<'a, Self>>`: `Ok(a)` when the program reduces to a pure value without firing the matched effect; `Err(op)` carrying the matched effect's lowered payload (post-`coyo.lower[_ref]()`) the moment the matched effect is dispatched. Body shape is simpler than `interpret_with` (no `Functor::map` recursion in the unmatched arm; just a flat `peel` loop with `Member::project<EBrand-Coyoneda, Idx>` per layer): matched arm returns `Err(matched_coyo.lower[_ref]())`; unmatched arm dispatches through `fo_handlers.dispatch(rest)` and rebinds the next program. Per-wrapper bound shapes mirror their respective `interpret_with` bodies (closure-refcount via `RcBrand` / `ArcBrand`, `'static` vs `'a` lifetimes, `Send + Sync` threading on the Arc family, ArcRun's `unwrap_first` HRTB-poisoning workaround helper for the peel-match). 24 integration tests at [`fp-library/tests/run_interpret_with_either.rs`](../../../fp-library/tests/run_interpret_with_either.rs) (4 per wrapper) cover T1 Pure -> Ok / T2 single matched -> Err / T3 FO-then-matched short-circuits after FO dispatch / T4 FO-only runs to completion; plus 6 doctests. Bundled commit covering all six wrappers (no per-wrapper specifics surfaced; bound shapes from step 2 carried over directly). The `Catch` cons-cell in step 7 consumes this primitive: a `Catch` handler installs `interpret_with_either::<ExceptBrand<E>, *, \_>`to test the body, and on`Err(throw)`invokes the recovery program.`just verify` clean.

**Phase 4 step 2.6: `ArcRunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the thread-safe explicit-lifetime Run wrapper.** One `feat(effects)` commit. Lands the `interpose` primitive at [`fp-library/src/types/effects/arc_run_explicit.rs`](../../../fp-library/src/types/effects/arc_run_explicit.rs); union of 2.3's `ArcRun::interpose` Send+Sync bound surface (closure `+ Send + Sync + 'a`, `<ArcBrand as RefCountedPointer>::new`, `SendFunctor::send_map` traversal on both `EBrand` and `RMinusE`, `A: Clone + Send + Sync`, multiple substrate-clone projection bounds) and 2.4 / 2.5's `'a`-lifetime threading (`ArcFreeExplicit<'a, ...>` substrate, closure `+ 'a` instead of `+ 'static`). Closes Phase 4 step 2 (all six Run wrappers now ship `interpose`). R1 (Explicit-family HRTB-poisoning) again did NOT surface, mirroring `ArcRunExplicit::interpret_with`'s direct-literal style: `ArcFreeExplicit::wrap(Node::First(...))` normalizes cleanly inside method scope without the `unwrap_first` / `make_node_first` / `wrap_first_arc` workaround helpers that `ArcRun` requires; the price `ArcRunExplicit` pays is a much longer explicit `Send + Sync` projection bound list per method (~6 extra projection bounds) declared inline. Same `EmbedIndices` deviation as 2.1 (no new deviation logged). Four parallel integration tests added at [`fp-library/tests/run_interpose.rs`](../../../fp-library/tests/run_interpose.rs) under a `// -- ArcRunExplicit --` divider, prefixed `arc_run_explicit_t1`-`arc_run_explicit_t4`; total tests in the file now 24 across 6 wrappers, plus 12 doctests across the wrappers' `interpose` / `interpose_shared` methods. Step 2 closes with this commit. `just verify` clean.

**Phase 4 step 2.5: `RcRunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the explicit-lifetime multi-shot Run wrapper.** One `feat(effects)` commit. Lands the `interpose` primitive at [`fp-library/src/types/effects/rc_run_explicit.rs`](../../../fp-library/src/types/effects/rc_run_explicit.rs); union of [step 2.1's RcRun template](../../../fp-library/src/types/effects/rc_run.rs) (matched-arm shape, `RcCoyoneda::lower_ref` for multi-shot Rc-shared programs, `EmbedIndices` extra type parameter, embed-back via `CoproductEmbedder`) and [`RcRunExplicit::interpret_with`](../../../fp-library/src/types/effects/rc_run_explicit.rs#L999)'s `'a`-lifetime bound surface (closure `+ 'a`, `RcBrand`-refcount with `'a`, `RcFreeExplicit<'a, ...>` substrate, `A: Clone`, substrate-clone bound on `NodeBrand<R, CNilBrand>::Of<'a, RcFreeExplicit<'a, NodeBrand<R, CNilBrand>, A>>`). Notably simpler than 2.4: `RcFreeExplicit::wrap` accepts raw projections directly without the `Box::new(...)` wrapping that `FreeExplicit::wrap` requires, so the unmatched arm is a direct port of 2.1's `Functor::map`+`embed`+`wrap` shape with `into_rc_free_explicit()` swapped for `into_rc_free()`. R1 (Explicit-family HRTB-poisoning) again did not surface; consistent with 2.4's empirical finding that the `'a`-parameterised GAT projection alone does not poison normalization. Same `EmbedIndices` deviation as 2.1. Four parallel integration tests added at [`fp-library/tests/run_interpose.rs`](../../../fp-library/tests/run_interpose.rs) under a `// -- RcRunExplicit --` divider, prefixed `rc_run_explicit_t1`-`rc_run_explicit_t4`; total tests in the file now 20 across 5 wrappers. The 1 remaining wrapper interpose primitive (`ArcRunExplicit`) follows as sub-step 2.6; R1 + R2 risks surface together there. `just verify` clean.
...
Two active items surfaced during Phase 4 step 3.1 foundational implementation: B5 (`RefFunctor` GAT-normalization on scoped-effect closure-cell brands) and B6 (step 3.1 sub-step splitting given the B5 deferral). Both must be resolved before step 3.1's commit can land. An informational deviation (`SendCatchBrand` has no `Functor` impl, mirroring Phase 3's `SendStateBrand` precedent) is logged at [deviations.md Phase 4 step 3.1](deviations.md) and is not a blocker.
...
**Issue.** Scoped-effect brands placed in a `ScopedCoproduct` row are documented (per [scoped.rs's "Per-scoped-effect-brand substrate-required traits" section](../../../fp-library/src/types/effects/scoped.rs)) as needing five trait impls: [`Functor`](../../../fp-library/src/classes/functor.rs), [`SendFunctor`](../../../fp-library/src/classes/send_functor.rs), [`WrapDrop`](../../../fp-library/src/classes/wrap_drop.rs), [`RefFunctor`](../../../fp-library/src/classes/ref_functor.rs), and [`Extract`](../../../fp-library/src/classes/extract.rs). Step 3.1's foundational implementation lands the first four cleanly across all three [`Catch`](../../../fp-library/src/types/effects/catch.rs) sibling brands but blocks on `RefFunctor` because of a GAT-normalization issue: the trait method takes `fa: &<Self as Kind>::Of<'a, A>`, and the compiler will not unify `&<Self as Kind>::Of<'a, A>` with the concrete enum type (`&BoxCatch<'a, BoxBrand, E, A>` etc.) inside the impl scope, even with explicit type annotation `let fa: &BoxCatch<'a, BoxBrand, E, A> = fa;`. Pattern matching on the variants therefore fails to type-check. [`Identity`](../../../fp-library/src/types/identity.rs) sidesteps this via tuple-struct field access (`fa.0`), which doesn't require a match; for sum types like `Catch` no such workaround exists in stable Rust.

**Options:**

- **A. Brand-projection helper analogous to `unwrap_first`.** Land a free function (e.g., `pub fn project_box_catch_ref<'a, P, E, A>(fa: &<BoxCatchBrand<P, E> as Kind>::Of<'a, A>) -> &BoxCatch<'a, P, E, A>`) outside the HRTB-bearing impl scope. The function's where-clause carries only `Kind` bounds; the return type forces normalization. Mirrors the [`arc_run.rs`'s `unwrap_first` workaround](../../../fp-library/src/types/effects/arc_run.rs#L1248-L1443) pattern. Three helpers needed (one per `Catch` sibling) plus one each for `Local` / `RefLocal` / `Bracket` / `RefBracket` / `Span` in steps 3.2-3.4. _Pros:_ Mechanical, well-precedented (same shape as the `unwrap_first` family), keeps the impl bodies clean. _Cons:_ Adds N free functions per scoped-effect family; pollutes the module's public surface (helpers are `#[doc(hidden)]` per `unwrap_first` precedent so user-API impact is minimal). _Risk:_ Low; the pattern compiles for `unwrap_first` already.

- **B. `unsafe { core::mem::transmute(fa) }` cast.** Use unsafe transmute to coerce `&<Self as Kind>::Of<'a, A>` to `&BoxCatch<'a, P, E, A>`. The two types are guaranteed structurally identical by the `impl_kind!` macro's expansion; the transmute is sound. _Pros:_ Zero new code surface; one-line per impl. _Cons:_ Introduces `unsafe` blocks for what is a safe operation in principle; future maintainers may not see the structural equivalence at a glance. _Risk:_ Medium; `unsafe` audit burden grows as more scoped effects ship with the same pattern.

- **C. Substrate redesign: drop `RefFunctor` from the scoped-row trait surface.** Refine [scoped.rs](../../../fp-library/src/types/effects/scoped.rs) to require only four traits (omit `RefFunctor`) and update [`NodeBrand<R, S>: RefFunctor` impl](../../../fp-library/src/types/effects/node.rs) to require `S: RefFunctor` only when an alternative substrate path is taken (e.g., conditional impl gated on a sub-trait). Programs using by-reference traversal on the scoped row would lose that path on the Catch family. _Pros:_ No new code; defers the RefFunctor problem entirely. _Cons:_ Breaks scoped.rs's "all five required" claim more substantially than B6's deviation does; reduces the substrate's by-reference traversal coverage. _Risk:_ High; users who rely on `NodeBrand<R, S>: RefFunctor` for their programs would see compilation failures with no clear migration.

- **D. Skip `RefFunctor` impls on Catch and accept the bound failure.** Don't implement `RefFunctor` on the three Catch brands; if a downstream program tries to use `NodeBrand<R, S>: RefFunctor` with `S` containing a Catch brand, compilation fails. _Pros:_ Zero new code; matches what was done for `SendCatchBrand`'s missing `Functor` impl (B6). _Cons:_ Worse than `SendCatchBrand`-no-Functor case because by-reference traversal is more pervasive (the Phase 3 effect suite uses it extensively); user programs may compile-fail unexpectedly. _Risk:_ Medium; user-visible compile errors with cryptic trait-bound messages.

**Recommendation: Option A (brand-projection helper).** The mechanical-helper pattern is well-precedented (`unwrap_first` family for ArcRun), keeps the impl bodies clean, and avoids `unsafe` audit burden. The cost (3 helpers for Catch siblings + helpers for the other scoped-effect families in steps 3.2-3.4, totaling perhaps 9-12 helpers across Phase 4 step 3) is amortised across all scoped-effect rollouts and small relative to the per-brand trait-impl surface. _Reasoning:_ The problem is a known stable-Rust limitation (GAT projections don't normalize through reference parameters in trait method bodies); the workaround is also known. Option B's `unsafe` is technically equivalent but worse for code review. Option C drops capability surface unnecessarily. Option D defers the problem to user-visible compile errors, which is the worst outcome.

**Plan revision required:** Yes (small). Step 3.1's commit boundary becomes: foundational scaffold (4 traits) + RefFunctor impls via brand-projection helpers + smart constructors + tests, all bundled. Or, if the helper-introduction is significant work itself, split as 3.1.1 (foundational scaffold), 3.1.2 (RefFunctor + helpers), 3.1.3 (smart constructors), 3.1.4 (tests) per B6 below.

#### B6. Step 3.1 sub-step splitting given the B5 deferral

**Issue.** The original step 3.1 plan committed to "one bundled commit covering the Catch sibling types + brands + 5 trait impls + smart constructors + tests" per the 2026-05-06 step-3 splitting resolution. The B5 RefFunctor deferral surfaces internal sub-step boundaries that were not visible at planning time: the foundational scaffold (4 of 5 trait impls) is mechanical from Phase 3.5's State template, while the RefFunctor work requires new helper-machinery design (per B5 Option A's brand-projection helper). The smart constructors and integration tests are separate concerns again.

**Options:**

- **A. Hold to one bundled commit.** Implement RefFunctor brand-projection helpers, fix the impls, add smart constructors, and write tests all before committing step 3.1. _Pros:_ Matches the original protocol decision; clean atomic step 3.1 closure. _Cons:_ Larger context-budget cost for the implementer (helper design + 3 RefFunctor impls + 6 smart constructors + tests in one turn); larger commit harder to review; rolling-back the commit means rolling back unrelated work.

- **B. Split into 4 sub-step commits.** 3.1.1 foundational scaffold (this turn's work, ready to commit); 3.1.2 RefFunctor + brand-projection helpers; 3.1.3 smart constructors per wrapper; 3.1.4 integration tests. Per the per-step protocol's "splitting clause when independent design risk surfaces" rule (which fired at step 2.1 -> 2.6 splitting). _Pros:_ Each sub-step's design risk is independent and reviewable; foundational scaffold can land now, blocked on no further design questions; helper design surfaces in 3.1.2 with explicit deviation logging. _Cons:_ 4 commits where 1 was planned; doc maintenance overhead per sub-step.

- **C. 2-commit split.** 3.1.1 foundational scaffold + smart constructors + tests using Functor+WrapDrop+Extract+SendFunctor only (no RefFunctor); 3.1.2 RefFunctor + brand-projection helpers added later. _Pros:_ Smart constructors and tests don't depend on RefFunctor, so they can land now; clean progress signal. _Cons:_ Programs using `NodeBrand<R, S>: RefFunctor` with `S` containing Catch brands fail to compile until 3.1.2 lands; 3.1.1 ships in a temporarily-incomplete state.

**Recommendation: Option B (4-commit split).** Mirrors step 2's empirically-validated splitting clause (step 2 split into 2.1-2.6 when per-wrapper risk surfaced; step 3.1 should split similarly when per-trait-family risk surfaces). _Reasoning:_ The B5 RefFunctor work has independent design risk (helper machinery design); the smart constructors have independent dispatch risk per wrapper; tests have independent coverage risk. Bundling all three together would obscure the per-area review surface. The per-step protocol's splitting clause was written for exactly this case. _Trade-off accepted:_ 4 commits per scoped-effect family vs the original "one bundled" target; each commit's narrative is cleaner than a bundled commit's would be.

**Plan revision required:** Yes. Update step 3 sub-step labels: `3.1 Catch` becomes `3.1.1 Catch foundational scaffold` / `3.1.2 Catch RefFunctor + brand-projection helpers` / `3.1.3 Catch smart constructors` / `3.1.4 Catch integration tests`. Mirror for 3.2 (Local/RefLocal), 3.3 (Bracket/RefBracket), 3.4 (Span); each 4-commit split as design risk surfaces, or simpler if it doesn't.
