...
**Phase 4 step 3.2.1 (commit `ef9b2eec`): Local (Val flavour) foundational scaffold.** One `feat(effects)` commit. Closes [B8 + B9 + B10](resolutions.md#resolved-2026-05-07-phase-4-step-3.2-sub-step-splitting--local-action-layout-cycle-reuse--file-organization-b8--b9--b10-closed) (8-commit Val+Ref split, B-thunk applied uniformly, separate `local.rs` / `ref_local.rs` files). Three sibling effect types at [`fp-library/src/types/effects/local.rs`](../../../fp-library/src/types/effects/local.rs) (`BoxLocal<'a, P, E, A>` where `P: ToDynFnOnce`, `Local<'a, P, E, A>` where `P: ToDynCloneFn`, `SendLocal<'a, P, E, A>` where `P: ToDynSendFn`) each holding a `Local { modify, action }` variant with per-pointer-brand pointer storage for both fields: `modify: <P>::Of<'a, dyn ClosureTrait(E) -> E>` (1-arg, consuming env per Val flavour) and `action: <P>::Of<'a, dyn ClosureTrait(()) -> A>` (B-thunk for layout-cycle break). Three brand declarations at [`fp-library/src/brands/effects.rs`](../../../fp-library/src/brands/effects.rs) plus `impl_kind!` registrations. Four of five substrate-required trait impls per brand: `Functor` on `BoxLocalBrand<BoxBrand, E>` and `LocalBrand<RcBrand, E>` (specialised via `ToDynFnOnce::new` / `ToDynCloneFn::new`); `SendFunctor` across all three (load-bearing on `SendLocalBrand<ArcBrand, E>`, stub-delegating-to-`Functor::map` on Box and Rc flavours); `WrapDrop` and `Extract` across all three (returning `Some(action(()))` / `action(())`, dropping `modify` silently). Manual `Clone` impls for `Local` (Rc-bump on both pointers) and `SendLocal` (Arc-bump); `BoxLocal` is unconditionally non-`Clone`. The fifth trait `RefFunctor` lands in step 3.2.2 mirroring step 3.1.2's deferral. `SendLocalBrand` does NOT implement `Functor` for the same structural reason `SendCatchBrand` does not: `Functor::map`'s `f: impl Fn(A) -> B + 'a` lacks the `Send + Sync` bounds needed for `<ArcBrand as ToDynSendFn>::new` closure storage; deviation logged at [deviations.md Phase 4 step 3.2.1](deviations.md), mirroring the Phase 3 `SendStateBrand` and Phase 4 `SendCatchBrand` precedents. Functor::map is simpler than Catch's because `f` is only used by `action` (modify is `E -> E`, untouched by `f`); `f` moves directly into the new action closure without the Rc/Arc-sharing pattern Catch needs. Module wired through [`fp-library/src/types/effects.rs`](../../../fp-library/src/types/effects.rs)'s `pub mod local;`. 13 doctests passing. `just verify` clean.
