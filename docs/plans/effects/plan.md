...
- **Phase 4** (scoped effects via heftia-inspired dual row): steps 0-1 plus steps 2.1-2.4 shipped; step 2.5 in-flight (POC 3 validation at [`poc_rc_run_interpret_with_either.rs`](../../../fp-library/tests/poc_rc_run_interpret_with_either.rs); `ScopedCoproduct<H, T>` and `ScopedNil` row-encoding aliases at [`scoped.rs`](../../../fp-library/src/types/effects/scoped.rs); `RcRun::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`rc_run.rs`](../../../fp-library/src/types/effects/rc_run.rs); `Run::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`run.rs`](../../../fp-library/src/types/effects/run.rs); `ArcRun::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`arc_run.rs`](../../../fp-library/src/types/effects/arc_run.rs); `RunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`run_explicit.rs`](../../../fp-library/src/types/effects/run_explicit.rs); `RcRunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` at [`rc_run_explicit.rs`](../../../fp-library/src/types/effects/rc_run_explicit.rs)). Step 2 splits into one sub-step per wrapper (2.1 RcRun, 2.2 Run, 2.3 ArcRun, 2.4 RunExplicit, 2.5 RcRunExplicit, 2.6 ArcRunExplicit) per the per-step protocol's splitting clause; the final wrapper rollout (2.6 ArcRunExplicit) combines 2.3's Send+Sync bound surface with 2.4's `'a`-lifetime threading. Steps 2a-8 follow. The Phase 4 design review ([`review/1_scoped_effects_design/review_phase_4_design.md`](review/1_scoped_effects_design/review_phase_4_design.md)) and its remediation report ([`review/1_scoped_effects_design/remediation_proposals_phase_4.md`](review/1_scoped_effects_design/remediation_proposals_phase_4.md)) shipped earlier, with three POC validations now complete ([`poc_send_catch_brand.rs`](../../../fp-library/tests/poc_send_catch_brand.rs) for the F2 parallel-Send-brand pattern; [`poc_rc_run_interpose.rs`](../../../fp-library/tests/poc_rc_run_interpose.rs) for the F1 substrate-level `Run::interpose` primitive; [`poc_rc_run_interpret_with_either.rs`](../../../fp-library/tests/poc_rc_run_interpret_with_either.rs) for the B4 substrate-level `interpret_with_either` primitive). The 2026-05-06 K1 / K2 [implementation-kickoff sequencing resolution](resolutions.md#resolved-2026-05-06-phase-4-implementation-kickoff-sequencing-k1-and-k2-poc-3-standalone-commit-first-planmd-numbering-authoritative-for-commit-boundaries) set POC 3's standalone-commit-first ordering and plan.md step numbering as the authoritative commit boundary. Phase 4 ships `Catch<'a, P, E, A>`, `Local<'a, P, E, A>` / `RefLocal`, `Bracket<'a, P, A, B>` / `RefBracket`, and `Span<'a, Tag>` scoped-effect constructors; a parallel `DispatchScopedHandlers` trait; substrate-level `Run::interpose` and `interpret_with_either` primitives on each Run wrapper.

### Next greenfield work

Phase 4 steps 0-1 plus 2.1-2.4 shipped; step 2.5 (`RcRunExplicit::interpose`) in-flight. The next concrete commit is **Phase 4 step 2.6**: `ArcRunExplicit::interpose` on the thread-safe explicit-lifetime Run wrapper, mechanically following the union of 2.3's `ArcRun::interpose` Send+Sync bound surface and HRTB-poisoning workaround helpers with 2.4's `RunExplicit::interpose` `'a`-lifetime threading. R1 + R2 risks surface together; expected to require ArcRunExplicit-specific HRTB workaround helpers analogous to ArcRun's (the Send+Sync GAT projection bound on `ArcRunExplicit`'s NodeBrand is what triggers the poisoning, not the `'a` lifetime). After 2.6 closes step 2, step 2a generalises the [`interpret_with_either`](../../../fp-library/tests/poc_rc_run_interpret_with_either.rs) substrate primitive across all six wrappers in parallel. Phase 4 then proceeds through plan.md steps 2a, 3, 4, 5, 6, 7, 8 in order with plan.md numbering as the authoritative commit boundary (per K2): substrate-level `Run::interpose` (POC 2 validated) and `interpret_with_either` (POC 3 validated, generic rollout in step 2a) primitives; `DispatchScopedHandlers` trait + per-wrapper interpret rewrite (R1 implementation); standard scoped-effect constructors using the Phase 3.5 retrofit's pointer-brand pattern (R2 implementation); bracket dispatcher with Drop-guard for panic safety (M3); standard scoped-effect rollout (`Catch`, `Local` / `RefLocal`, `Bracket` / `RefBracket`, `Span` plus `scoped_effects!` and `scoped_handlers!` macros via the new `handler_list_emitter` helper module shared with `handlers!`); standard scoped handlers; review-remediation documentation pass closing Phase 4. The Q4 / R1 / R2 half-day prototypes land during R1 implementation kickoff (alongside steps 1-4 substrate work); the R3 benchmark commit lands alongside the standard scoped-effect rollout (step 6 onward). Sub-step planning lives in plan.md's [Phase 4 section](#phase-4-scoped-effects-heftia-inspired-dual-row) below.
...
**Phase 4 step 2.4: `RunExplicit::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the explicit-lifetime Erased Run wrapper.** One `feat(effects)` commit. Lands the `interpose` primitive at [`fp-library/src/types/effects/run_explicit.rs`](../../../fp-library/src/types/effects/run_explicit.rs); union of [step 2.2's Run template](../../../fp-library/src/types/effects/run.rs) (matched-arm shape, `Coyoneda::lower` consuming variant, `EmbedIndices` extra type parameter, embed-back path via `CoproductEmbedder`) and [`RunExplicit::interpret_with`](../../../fp-library/src/types/effects/run_explicit.rs#L746)'s `'a`-lifetime bound surface plus the `Box::new(...into_free_explicit())` boxing pattern in the unmatched arm (`FreeExplicit::wrap` requires `Box<dyn ...>`-shaped continuations, unlike `Free::wrap` which takes raw projections). The `EmbedIndices` `CoproductEmbedder` bound's inner type is therefore `Box<FreeExplicit<'a, NodeBrand<R, CNilBrand>, A>>` rather than `Free<NodeBrand<R, CNilBrand>, A>`. Closure-refcount uses `<RcBrand as RefCountedPointer>::new` with `+ 'a` (not `+ 'static`), so the replacement closure can borrow from external state for the program's lifetime. R1 (Explicit-family HRTB-poisoning) anticipated by plan.md did NOT surface: `RunExplicit::interpose` compiled cleanly without the `unwrap_first` / `make_node_first` / `wrap_first_arc` workaround helpers used by `ArcRun::interpose`, mirroring `RunExplicit::interpret_with`'s direct-literal style; the `'a`-parameterised GAT projection on `FreeExplicit` does not poison normalization the way the `Of<'static, ArcFree<...ArcTypeErasedValue>>: Send + Sync` bound does on `ArcRun`. Same `EmbedIndices` deviation as 2.1. Four parallel integration tests added at [`fp-library/tests/run_interpose.rs`](../../../fp-library/tests/run_interpose.rs) under a `// -- RunExplicit --` divider, prefixed `run_explicit_t1`-`run_explicit_t4`; total tests in the file now 16 (4 each across the 4 wrappers shipped so far). The 2 remaining Explicit-family wrapper interpose primitives (RcRunExplicit, ArcRunExplicit) follow as sub-steps 2.5-2.6; R1 + R2 risks surface together in 2.6. `just verify` clean.

**Phase 4 step 2.3: `ArcRun::interpose<EBrand, Idx, RMinusE, EmbedIndices>` substrate primitive on the thread-safe Erased Run wrapper.** One `feat(effects)` commit. Lands the `interpose` primitive at [`fp-library/src/types/effects/arc_run.rs`](../../../fp-library/src/types/effects/arc_run.rs); union of [step 2.1's RcRun template](../../../fp-library/src/types/effects/rc_run.rs) (matched-arm shape, `EmbedIndices` extra type parameter, embed-back path) and [`ArcRun::interpret_with`](../../../fp-library/src/types/effects/arc_run.rs#L942)'s Send+Sync bound surface. R2 (Send+Sync propagation) cleared structurally with no new substrate machinery: closure-refcount uses `<ArcBrand as RefCountedPointer>::new` with `Send + Sync + 'static` on the user closure; traversal uses `<EBrand as SendFunctor>::send_map` and `<RMinusE as SendFunctor>::send_map`; recursion routes through the existing [`unwrap_first` / `make_node_first` / `wrap_first_arc`](../../../fp-library/src/types/effects/arc_run.rs#L1248-L1443) HRTB-poisoning workaround helpers. One incremental bound shape vs `interpret_with`: `R: WrapDrop + SendFunctor + 'static` is required at the method level (not just `R: Kind + 'static` as in `interpret_with`) because the embed-back path rewraps in `R` (not `RMinusE`), so `wrap_first_arc::<R, CNilBrand, A>` needs `R`'s `SendFunctor`-bearing where-clauses satisfied; this is mechanically expected when the operation stays in the original row, not a new structural deviation. Same `EmbedIndices` deviation as 2.1. Four parallel integration tests added at [`fp-library/tests/run_interpose.rs`](../../../fp-library/tests/run_interpose.rs) under a `// -- ArcRun --` divider, prefixed `arc_run_t1`-`arc_run_t4`; total tests in the file now 12 (4 RcRun + 4 Run + 4 ArcRun). The 3 remaining Explicit-family wrapper interpose primitives (RunExplicit, RcRunExplicit, ArcRunExplicit) follow as sub-steps 2.4-2.6; R1 risk (Explicit-family HRTB-poisoning) surfaces in 2.4, R1 + R2 in 2.6. `just verify` clean.
