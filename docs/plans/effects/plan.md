...
Three blockers surfaced before Phase 4 step 3.2 implementation kicks off: **B8** (step 3.2 sub-step splitting given that `Local` ships in both Val and Ref flavours, doubling step 3.1's substrate surface), **B9** (`Local` action-field layout cycle: same substrate placement as Catch, so the same B7-style fix likely applies), **B10** (file organization for `Local` and `RefLocal`: combined `local.rs` would balloon to ~2700 lines given that catch.rs is already 1349 lines for one effect). Full options and recommendations in the three subsections below. Step 3.1 closure context: B5 + B6 closed on 2026-05-07 per the [resolved B5 / B6 entry](resolutions.md#resolved-2026-05-07-phase-4-step-3.1-sub-step-splitting-b5--b6-closed): Option A adopted for B5 (brand-projection helper analogous to [`arc_run::unwrap_first`](../../../fp-library/src/types/effects/arc_run.rs)); Option B adopted for B6 (4-commit split into 3.1.1 foundational scaffold / 3.1.2 `RefFunctor` + helpers / 3.1.3 smart constructors / 3.1.4 integration tests). 3.1.1 shipped at `abd3d1a3`; 3.1.2 shipped at `5bb2d1ae`; 3.1.3 shipped at `205eaba4`; 3.1.4 shipped at `faab175f`. B7 (`Catch` action-field layout cycle when embedded in substrate) closed on 2026-05-07 per the [resolved B7 entry](resolutions.md#resolved-2026-05-07-phase-4-step-3.1.3-catch-action-field-layout-cycle-b7-closed): adopted **B-thunk with unit-arg `Fn(()) -> A`** form (per-pointer-brand pointer of unit-arg closure thunk, leveraging the existing pointer-abstraction `ToDyn*Fn::new` family per the matrix in [`pointer-abstraction.md`](../../../fp-library/docs/pointer-abstraction.md)).

#### Active blocker (2026-05-07): B8. Step 3.2 sub-step splitting given Val + Ref flavours

- **Issue.** Step 3.2 covers both `Local` (Val flavour) and `RefLocal` (Ref flavour) per [decisions.md section 4.5](decisions.md). Each flavour requires the same 3-sibling-type-per-pointer-brand pattern as Catch (BoxLocal / Local / SendLocal for Val plus BoxRefLocal / RefLocal / SendRefLocal for Ref), giving 6 enum types and 6 brand declarations against Catch's 3 of each. The smart-constructor surface doubles too (12 per-wrapper constructors total: 6 Run wrappers x 2 flavours), and Functor / SendFunctor / WrapDrop / Extract / RefFunctor must all be implemented per brand. The B6 closure committed step 3.1 to a 4-commit split; step 3.2's roughly-2x surface needs an analogous decision.
- **Options:**
  - **Option A: 4-commit paired Val+Ref split.** Each commit covers both flavours simultaneously: 3.2.1 scaffold for Val + Ref (6 sibling types, 6 brands, 4 trait impls each); 3.2.2 RefFunctor + brand-projection helpers for both; 3.2.3 smart constructors per wrapper for both (12 total) plus B-thunk action representation; 3.2.4 integration tests for both. Pros: matches 3.1's commit cadence; total commits stays at 4. Cons: each commit is roughly 2x the size of the equivalent 3.1 commit (3.2.3 alone ships 12 smart constructors plus a multi-flavour B-thunk refactor); review surface gets dense.
  - **Option B (recommended): 8-commit separate Val and Ref cycles.** 3.2.1-3.2.4 ship the Local (Val) full cycle (scaffold / RefFunctor / smart constructors / tests); 3.2.5-3.2.8 ship the RefLocal (Ref) full cycle in the same shape. Pros: each commit stays a focused vertical slice at the size of 3.1's commits, easier to review and debug; Val/Ref differences (closure parameter type, `E: Clone` requirement on Val, etc.) surface cleanly per phase; if R2 risk surfaces during Val it can be addressed before Ref scope expands the change. Cons: 8 commits feels granular relative to 3.1's 4; an unwary reader might miss that the second 4 are mechanically parallel to the first 4.
  - **Option C: 5-commit hybrid.** 3.2.1 combined scaffold + RefFunctor for Val and Ref; 3.2.2 Local (Val) smart constructors; 3.2.3 RefLocal (Ref) smart constructors; 3.2.4 Local + RefLocal integration tests in one file or two; 3.2.5 reserved for any deferred follow-up. Pros: shares structure where it overlaps; tests can co-locate. Cons: combined-then-split rhythm is awkward; 3.2.1 hits the same per-commit-size concern as Option A.
- **Recommendation: Option B (8-commit separate Val and Ref cycles).** Reasoning: the 4-commit-per-flavour pattern is already proven by 3.1's clean ship; Val and Ref differ in subtle but important ways (closure parameter `E` vs `&E`, derived-sub-scope ergonomics, `E: Clone` requirement) that surface more cleanly when separated; per-commit size stays at 3.1's reviewable bar; if R2 risk on the Arc family surfaces it can be triaged inside the Val cycle before Ref expansion. Total commit count is the only downside; the alternative of doubling commit size in Option A trades reviewability for commit count, the wrong direction.

#### Active blocker (2026-05-07): B9. Local action-field layout cycle (B7 reuse)
...
- **Options:**
  - **Option A (recommended): Apply B-thunk uniformly.** Store `action` as `<P>::Of<'a, dyn 'a + FnOnce/Fn(()) -> A>` per the per-pointer-brand pattern (mirroring catch.rs). Pros: proven fix; consistent across scoped effects; uses existing pointer-abstraction `ToDyn*Fn::new` family without new machinery. Cons: one extra heap allocation per scoped layer (matches Catch's overhead).
  - **Option B: Test layout first, fall back to B-thunk only if needed.** Half-day prototype implementing Local with raw `action: Run<R, S, A>` to confirm the layout cycle surfaces. Pros: avoids speculative complexity if for some reason the layout works. Cons: structurally identical to Catch's case; the layout-cycle reasoning (Free's variant payload contains a recursive Free without pointer indirection) does not depend on which scoped-effect cell holds the `action`; the prototype is essentially guaranteed to fail and would just delay the inevitable.
  - **Option C: Explore alternative layout-cycle breaks (Box<A>, custom indirection).** Per the [B7 options analysis](resolutions.md#resolved-2026-05-07-phase-4-step-3.1.3-catch-action-field-layout-cycle-b7-closed), Option A (uniform `Box<A>`) and Option B-literal (per-pointer-brand pointer for raw `A`) were rejected for Catch because they impose `A: Clone` constraints that break Functor::map composition. Same constraints apply to Local; same rejection rationale.
- **Recommendation: Option A.** The layout-cycle reasoning is structurally identical to Catch's case; the B7 resolution explicitly anticipates extension to other scoped effects (action-payload-shape pattern is part of [`pointer-abstraction.md`](../../../fp-library/docs/pointer-abstraction.md)'s matrix). Applying B-thunk uniformly across scoped effects also keeps the user-facing API consistent: `local(modify, action)` mirrors `catch(action, handler)` in the constructor's argument shape and dispatch behaviour.

#### Active blocker (2026-05-07): B10. File organization for Local and RefLocal

- **Issue.** [`catch.rs`](../../../fp-library/src/types/effects/catch.rs) is already 1349 lines for a single scoped effect (one Val flavour with three pointer-brand siblings). Local + RefLocal in a combined `local.rs` would scale linearly: ~2700 lines. The plan and decisions documents are silent on file organization; existing precedent (state.rs / reader.rs / choose.rs / except.rs / writer.rs) puts each effect in its own file but those are first-order effects with simpler structure.
- **Options:**
  - **Option A (recommended): Separate `local.rs` and `ref_local.rs`.** Local (Val) in [`fp-library/src/types/effects/local.rs`](../../../fp-library/src/types/effects/) (~1000-1500 lines); RefLocal (Ref) in [`fp-library/src/types/effects/ref_local.rs`](../../../fp-library/src/types/effects/) (~1000-1500 lines). Pros: file size matches catch.rs precedent; the Val/Ref distinction is a fundamental conceptual divide (`E -> E` vs `&E -> E` closure shape) that warrants a file boundary; users navigating to "Local" naturally find Val and "RefLocal" for Ref via filename. Cons: two files to maintain instead of one; cross-references between Val and Ref docs require relative paths.
  - **Option B: Combined `local.rs` for both Local and RefLocal.** All six sibling types in one file (~2700 lines). Pros: single file to navigate when comparing Val and Ref shapes; cross-references stay intra-file. Cons: file size exceeds catch.rs's already-large 1349 lines by 2x; LSP / Read tool / git blame all suffer at this scale; the conceptual Val/Ref divide is muddied by spatial co-location.
- **Recommendation: Option A.** File-size precedent (catch.rs at 1349 lines for one effect) is the load-bearing argument: combined would exceed reviewable bounds and complicate tool-assisted navigation. The Val/Ref distinction also fits naturally on the file axis: future scoped effects (Bracket / RefBracket in step 3.3) follow the same Val/Ref pattern, and the precedent set by 3.2 propagates to 3.3.
