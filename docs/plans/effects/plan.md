...
Step 7.4.5 has shipped: `SpanDispatcher` now uses the private H2 carrier resume hooks for raw default `Run`, raw `RcRun`, raw `ArcRun`, and the Explicit boundary families, so Span observes its tag around the selected action and returns the action result unchanged. Ordinary one-slot `DispatchScopedHandler` routes remain available for handlers that do not need an around-action carrier.

The implementation preserves the existing erased-result conventions:
default `Run` inserts a direct-erased post-action return, while
`RcRun` and `ArcRun` append a raw-erased continuation instead of using
normal `bind`, so simple Span actions keep direct payloads and nested
`CatchDispatcher` / `interpose` paths still rebox through their
existing outer continuation queues. Focused `run_span`,
`run_scoped_dispatchers`, and Span dispatcher doctest coverage is green
for the migrated paths.

Step 7.4.6 has shipped:
[`run_span.rs`](../../../fp-library/tests/run_span.rs) now covers plain
`RcRun` / `ArcRun` standard SpanDispatcher interpretation over nested
spans, repeated shared execution, and borrowed payloads through the
`RunExplicit`, `RcRunExplicit`, and `ArcRunExplicit` Span boundary
dispatchers. Existing
[`run_scoped_dispatchers.rs`](../../../fp-library/tests/run_scoped_dispatchers.rs)
coverage continues to exercise default `Run` nested Span result
propagation, Catch / Span coexistence with first-order handlers, and
Explicit facade interpretation across the boundary families.

**Next greenfield step: Phase 4 step 7.4.7, add scoped-operation
allocation benchmarks (R3).** Add a focused benchmark commit for scoped
operation overhead, comparing standard scoped operations with
equivalent first-order-only programs that simulate the same behavior
through closure capture. Include Bracket / RefBracket cases for
acquire/body/release closure-cell costs and resource guard overhead.
...
7.  Standard scoped-handler implementations as a parallel
    ...
    - **7.4 Internal continuation carrier for around-action scoped
      ...
      need explicit handler classes.
           ...
           After the scoped-effect interpreter architecture stabilizes,
           add a focused benchmark commit for scoped operation overhead.
           Compare standard scoped operations against equivalent
           first-order-only programs that simulate the same behaviour
           through closure capture. Include Bracket / RefBracket cases
           that characterize acquire/body/release closure-cell costs and
           resource guard overhead, because those scoped operations do
           not benefit from Coyoneda fusion in the same way first-order
           programs do.
