...
- **Phase 3.5** (pointer-brand-pattern retrofit): in progress. Sub-step 1 (`ToDynFnOnce` trait at [`fp-library/src/classes/to_dyn_fn_once.rs`](../../../fp-library/src/classes/to_dyn_fn_once.rs) plus `BoxBrand` impl at [`fp-library/src/types/box_ptr.rs`](../../../fp-library/src/types/box_ptr.rs)) shipped; sub-steps 2-5 (effect retrofit on `State` / `Reader` / `Choose`, [`pointer-abstraction.md`](../../../fp-library/docs/pointer-abstraction.md) update, integration-test verification, F4-closure resolutions entry) pending. `Rc<dyn FnOnce>` and `Arc<dyn FnOnce>` are operationally broken because moving out of a shared pointer invalidates other clones, so `ToDynFnOnce` is `BoxBrand`-only. Sub-step 2 retrofits `State` / `Reader` / `Choose` to use `BoxBrand` + `ToDynFnOnce` (`Box<dyn FnOnce>`) on default `Run`/`RunExplicit` instead of the current `RcBrand` + `ToDynCloneFn` (`Rc<dyn Fn>`). Closes Phase 3 prior-review F4 finding structurally rather than as accepted-tradeoff. Phase 4 then uses the same per-pointer-brand pattern.
