...
- **Phase 4** (scoped effects via heftia-inspired dual row): steps 0-1, step 2 (sub-steps 2.1-2.6), step 2a, Catch 3.1.1-3.1.4, Local / RefLocal 3.2.1-3.2.8, Bracket / RefBracket 3.3.1-3.3.8, and Span 3.4.1 have shipped. Closed blockers and design decisions are tracked in [resolutions.md](resolutions.md) and [deviations.md](deviations.md). B21 is resolved via Option A: Span remains Val-only at the user-facing level, but the implementation mirrors Catch and Local with Box/Rc/Arc action-thunk substrate cells. B22 is resolved via Option A: Span stores tags by value and adds clone/send bounds only where Rc/Arc substrates require them. The next greenfield step is 3.4.2: Span smart constructors. Q4 and R3 remain non-blocking follow-ups below.

### Next greenfield work

Phase 4 step 3.4.1 shipped on this branch: [`fp-library/src/types/effects/span.rs`](../../../fp-library/src/types/effects/span.rs) adds the Span foundational scaffold (`BoxSpan`, `Span`, `SendSpan`) plus Box/Rc/Arc brands, module export, and substrate-required trait impls. The scaffold follows B21/B22: action is a unit-argument B-thunk, tags stay by value, default Box-backed cells allow non-`Clone` tags, Rc clone/ref-map paths require `Tag: Clone`, and Arc paths require `Tag: Send + Sync` plus `Tag: Clone` only where the cell is cloned. Implementation notes live in [deviations.md](deviations.md#step-341-span-foundational-scaffold-uses-boxrcarc-action-thunks-with-by-value-tags-and-no-per-free-family-split).

**Next greenfield step:**

**Step 3.4.2: Span smart constructors per wrapper.** Add `span` smart constructors for `Run`, `RcRun`, `ArcRun`, `RunExplicit`, `RcRunExplicit`, and `ArcRunExplicit`, pairing each wrapper with its Span cell (`BoxSpan`, `Span`, or `SendSpan`) and the wrapper's underlying Free family. Preserve the B22 bound surface: default wrappers do not require `Tag: Clone`; Rc wrappers require cloneable tags where the Rc substrate requires cloneable scoped layers; Arc wrappers require `Tag: Clone + Send + Sync` and the usual Arc row-projection bounds. After this, land step 3.4.3 integration tests for suspended-layer construction and tag/action thunk materialisation.
...
The remaining non-blocking items are Q4 and R3:

- Q4 remains pending until the future `DispatchScopedHandlers` /
