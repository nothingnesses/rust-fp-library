...
**Next greenfield step: Phase 4 step 7.4.4c.1c.2, migrate Span
first.** Steps
...
selected action slot. The next step is to route Span through that
boundary-backed action-supplied carrier path and preserve a clearly named
compatibility helper only if the production migration proves one is still
needed for result-only ergonomics.
Steps
7.4.4b.3a.0 through
