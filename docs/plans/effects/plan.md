...
**Phase 4 step 3.3.1 (commit `1be2af3e`): Bracket (Val flavour) foundational scaffold + B14 closure (acquire B-thunk).** One `feat(effects)` commit. Lands the Bracket Val foundational scaffold at [`fp-library/src/types/effects/bracket.rs`](../../../fp-library/src/types/effects/bracket.rs) under Option A (the explicit B17 fallback) after the user-approved primary Option C (`FreeShape` HKT-trait decomposition) failed Rust's well-formedness check at `Functor::map`. The Option C probe wrote a 2-line `FreeShape` trait plus a blanket impl on `Free<F, A>`, then a `BoxBracket<'a, P, A, B, X>` cell with where-clause `X: 'a + FreeShape`. The probe failed at `Functor::map` on `BoxBracketBrand<BoxBrand, A, B>`, which forms `<Self as Kind>::Of<'a, X>` for unbounded `X: 'a` (the `Functor` trait's method signature is fixed). Rust's WF check on the GAT body required `X: FreeShape` (transitively from the cell's where-clause), but the bound is not in scope; rustc reported `error[E0277]: the trait bound X: FreeShape is not satisfied` and could not be added because `Functor::map`'s signature is fixed by the trait. The probe was reverted; `FreeShape` is not in the codebase. **Option A landed**: 5-param struct `Bracket<'a, P, Sub, A, B>` + 4-param brand `BracketBrand<P, Sub, A, B>`, with `Sub` carried explicitly through the cell's struct and the brand's GAT projection ignoring the GAT-filled `X` (the brand's `Of<'a, X>` resolves to `Bracket<'a, P, Sub, A, B>` regardless of `X`). The recursive type cycle through `Sub` (which references `NodeBrand<R, ScopedRow>` whose `ScopedRow` contains `BracketBrand` recursively) is broken at the value level by `PhantomData<(P, Sub, A, B)>` on the brand: the brand is zero-sized regardless of `Sub`'s layout, so Rust accepts the recursive type names without infinite-layout errors. Three sibling effect types: `BoxBracket<'a, P, Sub, A, B>` (BoxBrand + FnOnce, single-shot; non-Clone), `Bracket<'a, P, Sub, A, B>` (RcBrand + Fn, multi-shot; manual Clone via Rc-bump), `SendBracket<'a, P, Sub, A, B>` (ArcBrand + Fn + Send + Sync, thread-safe; manual Clone via Arc-bump). Each holds a `Bracket { acquire, body, release }` variant (B-thunk uniformly applied to acquire per B14: `<P>::Of<'a, dyn ClosureTrait(()) -> Free<Sub, A>>`, breaking the layout cycle; body and release take the resource as `<P>::Of<'a, A>` and return `Free<Sub, (A, B)>` / `Free<Sub, ()>` respectively). Three brand declarations at [`brands/effects.rs`](../../../fp-library/src/brands/effects.rs). Four substrate-required trait impls per brand: `Functor::map` is identity (returns `fa` unchanged because `Of<'a, X>` is independent of `X`); `SendFunctor::send_map` is identity; `WrapDrop::drop` returns `None` (the cell's body program result cannot be materialised without first running acquire to produce a resource); `Extract::extract` is a panicking `unreachable!` stub (substrate-required cascade reaches `Bracket` only on synthetic paths; production interpret routes scoped layers to the bracket dispatcher in step 6, not `Extract`). All four impls are documented in-file with explicit reasons. SendFunctor stubs on `BoxBracketBrand<BoxBrand, _, _, _>` and `BracketBrand<RcBrand, _, _, _>` mirror the `BoxLocalBrand` / `LocalBrand` SendFunctor-stub precedent. 13 doctests. Closes B17 with Option A switch logged at [deviations.md Phase 4 step 3.3.1](deviations.md). `just verify` clean.

**Phase 4 step 3.2.8 (commit `b65ac467`): RefLocal (Ref flavour) integration tests.** One `feat(effects)` commit. 22 shape-only tests land at [`fp-library/tests/run_ref_local.rs`](../../../fp-library/tests/run_ref_local.rs) across six Run wrappers, mirroring [run_local.rs](../../../fp-library/tests/run_local.rs)'s template. Per wrapper: T1 (`*_t1_ref_local_produces_scoped_layer`) verifies `ref_local(modify, action)` peels to `Err(Node::Scoped(Coproduct::Inl(_::Local { .. })))` with the per-wrapper variant tag (`BoxRefLocal::Local` for Run/RunExplicit, `RefLocal::Local` for RcRun/RcRunExplicit, `SendRefLocal::Local` for ArcRun/ArcRunExplicit; variant name uniformly `Local` per the closed B12 decision); T2 (`*_t2_action_thunk_materialises_action_program`) destructures the cell, invokes `action(())`, and asserts the materialised wrapper-typed program peels to `Ok(42)`; T3 (`*_t3_modify_transforms_environment_by_reference`) invokes `modify(&10)` and asserts `== 11` (using `|e: &i32| *e + 1` as the modify closure). T4 (`*_t4_clone_yields_two_independent_peels`) on the four Clone-able wrappers (RcRun, ArcRun, RcRunExplicit, ArcRunExplicit) clones the suspended program and confirms both peelable handles materialise the original action independently via the cell's Rc-bump / Arc-bump Clone impl. The only structural difference from the Val-cycle T3 is the `&` on the input (`modify(&10)` instead of `modify(10)`); the Ref flavour's `Fn(&E) -> E` shape returns owned `E` like the Val flavour. Test file uses `FirstRow = CNilBrand` to sidestep the `Free` layout cycle; `#![expect(clippy::panic, reason = "...")]` matches run_local.rs's precedent for pattern-match exhaustion via panic. End-to-end environment-modification semantics (the dispatcher applying `modify` to a borrow of the inherited environment before invoking `action`) are not exercised; that path comes online with the scoped-handler dispatch protocol and the standard reader handler. Closes the entire 3.2 sub-cycle (8 commits across Val + Ref). `just verify` clean (22 run_ref_local tests passing).
...
#### B19 (2026-05-08): Bracket cell hardcodes `Free<Sub, _>` substrate; only `Run::bracket` is serviceable. Resolution: split per Free family (Option C)

**Issue.** The Bracket Val foundational scaffold (step 3.3.1, commit `1be2af3e`) hardcodes `Free<Sub, _>` in all three sibling cells' field types. This is correct for `Run::bracket` (substrate = `Free<NodeBrand<R, S>, _>`) but mismatched for the five other Run wrappers, each of which has a distinct substrate:
...
The user's `body` closure for any non-Run wrapper returns that wrapper's program type, but the cell's hardcoded `Free<Sub, _>` only matches `Run`. Cross-family conversion (e.g., `RcFree` to `Free`) doesn't currently exist, would cost O(N) per scoped operation if added, and would re-introduce the Phase 3.5 F4-style runtime-erasure pattern this project deliberately moved away from.

This is the second structural concern surfaced after foundational scaffold (B17 was the first; B19 is this). Both stem from Option A's "drop X" design choice in B17: solving the WF rejection at the brand level by dropping X from the GAT projection (`Of<'a, X> = Bracket<...>` independent of X) preserved Functor's identity-shape impl but removed the auto-adaptation Catch / Local / RefLocal cells get from storing closures returning A (the GAT-filled X).

**Options considered:**

- **B (parameterise via 1-arity Kind brand `F` that bakes Sub).** Cell becomes 5-param `<'a, P, F, A, B>`; brand stays 4-param `BracketBrand<P, F, A, B>`. F is a 1-arity Kind brand whose `Of<'a, A>` resolves to the substrate program type (e.g., `FreeExplicitBrand<NodeBrand<R, S>>::Of<'a, A> = FreeExplicit<'a, NodeBrand<R, S>, A>`). Existing Explicit family already has these brands (`FreeExplicitBrand<F>`, `RcFreeExplicitBrand<F>`, `ArcFreeExplicitBrand<F>`); Erased family does not. Defining new `FreeBrand<F>`, `RcFreeBrand<F>`, `ArcFreeBrand<F>` requires GAT where clauses (`type Of<'a, A: 'a>: 'a = Free<F, A> where A: 'static`) because Erased Free's `A: 'static` bound doesn't match the trait's loose `A: 'a`. The existing `impl_kind!` macro pattern in this codebase doesn't use GAT where clauses; supporting them requires either extending the macro or writing manual Kind impls that bypass it. **Rejected**: extends macro/substrate work that isn't on the Phase 4 critical path.

- **B'' (parameterise via 2-arity Kind brand).** Cell becomes 6-param; brand becomes 5-param. The 2-arity Kind variants in `kinds.rs` (`Of<'a, A: 'a, B: 'a>: 'a`) support `<F as Kind2>::Of<'a, Sub, A>` for a family marker brand `F`. Same `A: 'static` bound dance as Option B; the 2-arity Kind helps express the substrate function but doesn't solve the Erased/Explicit bound asymmetry. Adds a 5-param brand which complicates marker-struct doctests and `scoped_effects!` macro generation in step 5. **Rejected**: same fundamental obstacle as Option B.

- **C (split into 6 cells per Free family).** Six cell types: existing 3 cells (`BoxBracket` / `Bracket` / `SendBracket`) get FIXED to use the wrapper-correct substrate per pointer brand (`BoxBracket` keeps `Free<Sub, _>` for `Run`; `Bracket` switches to `RcFree<Sub, _>` for `RcRun`; `SendBracket` switches to `ArcFree<Sub, _>` for `ArcRun`); three new Explicit-family cells get ADDED (`BoxBracketExplicit` for `RunExplicit` storing `FreeExplicit<'a, Sub, _>`; `BracketExplicit` for `RcRunExplicit` storing `RcFreeExplicit<'a, Sub, _>`; `SendBracketExplicit` for `ArcRunExplicit` storing `ArcFreeExplicit<'a, Sub, _>`). Each cell mirrors the existing pattern; brands stay 4-param.

**Resolution: Option C (split).** Three reasons:

1. **Lifetime bound asymmetry between Erased and Explicit Free families is the load-bearing constraint.** Option B / B'' both need GAT where clause support for Erased family Kind impls (Erased Free's `A: 'static` vs the trait's `A: 'a`). The existing `impl_kind!` macro doesn't use GAT where clauses; extending it is macro/substrate work off the Phase 4 critical path. Option C sidesteps the issue by hardcoding each cell's substrate (and thus its bounds) at the cell definition.

2. **Codebase precedent: the library already splits per Free family at the Run-wrapper layer.** Six Run wrappers exist (`Run` / `RcRun` / `ArcRun` / `RunExplicit` / `RcRunExplicit` / `ArcRunExplicit`), one per `(PointerBrand, FreeFamily)` pair. Splitting Bracket cells along the same axis (six cells, one per pair) is structurally consistent with the existing Run-wrapper split. Option B would unify cells via parameterisation, a different abstraction shape from the Run wrappers.

3. **Doubled surface is bounded and mechanical, not architectural.** Option C costs ~1500 additional lines (3 new Explicit cells x ~400 lines each + 3 new brands x ~25 lines + 3 sets of trait impls x ~150 lines). Each new cell is a line-for-line mirror of its existing Erased sibling with `Free` swapped to the appropriate substrate. The `scoped_effects!` macro (step 5) selects the right cell-and-brand pair based on the wrapper, which is structurally simpler than threading an F brand parameter.

**Plan-text amendment.** B19 closure rework follows in a single commit:
