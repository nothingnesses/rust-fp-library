...
**Next greenfield step: Phase 4 step 7.4.6, add focused regression tests.** Cover all six wrappers: nested Span ordering, result propagation, coexistence with first-order handlers, borrowed payloads on Explicit wrappers, multi-shot Rc behavior, Send/Sync Arc behavior, and the default-vs-shared erased continuation convention proven during 7.4.5. After 7.4.6, continue with 7.4.7 scoped-operation allocation benchmarks (R3).
