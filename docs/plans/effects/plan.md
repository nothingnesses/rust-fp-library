...
One active blocker: B17 (Bracket cell's three differently-typed program returns vs the substrate's single-GAT-parameter pattern). Closed blockers from earlier steps: B5 + B6 (3.1 sub-split, 4 commits), B7 (Catch action layout cycle, B-thunk), B8 + B9 + B10 (3.2 sub-split + Local action layout cycle reuse + file organization, 8 commits Val + Ref), B11 + B12 (`ToDynFnOnce::ref_new` matrix gap + variant naming uniformly `Local`), B14 + B15 + B16 (3.3 sub-split + Bracket acquire B-thunk reuse + RefBracket sibling-count asymmetry).

#### B17. Bracket cell's three differently-typed program returns vs substrate's single-GAT-parameter pattern

- **Issue.** Per [decisions.md table at line 480-481](decisions.md), Bracket Val and RefBracket Ref cells have THREE differently-typed program-return positions (`acquire` returns `Run<R, S, A>`, `body` returns `Run<R, S, (A, B)>` for Val or `Run<R, S, B>` for Ref, `release` returns `Run<R, S, ()>`). Catch and Local have ONLY ONE program type per cell (both action and handler in Catch return Run<R, S, A>; Local's action returns Run<R, S, A>). The substrate's GAT projection `Brand::Of<'a, X>` fills X with the program type, and Catch / Local use that single X consistently across all closure return positions. Bracket can't follow this pattern directly because three differently-typed programs over the same Sub = NodeBrand<R, S> can't be expressed via a single GAT-filled X.
- **Options.**
  - **A. 5-param struct with explicit substrate brand `Sub`: `Bracket<'a, P, Sub, A, B>` where Sub = NodeBrand<R, S>.** The struct's fields express each program type via `Free<Sub, A>`, `Free<Sub, (A, B)>`, `Free<Sub, ()>`. Brand: `BracketBrand<P, Sub, A, B>` (4 brand params). Trade-offs: structurally explicit and clean field types; matches Haskell heftia's GADT shape (`Bracket f b where Bracket :: f a -> (a -> f b) -> ...`). RISK: recursive-type-cycle concern , when Bracket is in a scoped row, Sub = NodeBrand<R, ScopedRow> where ScopedRow contains BracketBrand<P, Sub, A, B>, recursing infinitely at the type level. PhantomData<Sub> at the brand level might allow Rust to accept this (since PhantomData is sized 0), but unverified. May fail to typecheck.
  - **B. Tagged-union wrapping via `BracketStep<A, B>` enum: all three programs return the same `Run<R, S, BracketStep<A, B>>` type.** Smart constructor wraps user's programs via `Run::map` to inject the appropriate `BracketStep::AcquireResult(a)` / `BodyResult(a, b)` / `ReleaseResult` variants. Brand: `BracketBrand<P, A, B>` (3 brand params, like Catch / Local). Brand's `Of<'a, X>` = `Bracket<'a, P, A, B, X>` with X being the wrapped program type. Trade-offs: substrate-GAT-compatible (single X); brand stays 3-param (uniform with Catch / Local). RISK: smart constructor adds Run::map overhead per program (closure allocation for the wrap); dispatcher must inspect BracketStep variants; the bracket scope's "user-visible result type" is `BracketStep<A, B>` rather than just `B`, leaking implementation details unless the smart constructor auto-unwraps via an outer Run::map.
  - **C. HKT-trait decomposition: introduce `FreeShape` trait that extracts F and InnerResult from `Free<F, _>`.** Brand: `BracketBrand<P, A, B>` (3 brand params). Brand's `Of<'a, X>` = `Bracket<'a, P, A, B, X>` where X = Free<F, B> (substrate-filled). Struct fields use `<X as FreeShape>::F` to construct alternate program types (`Free<<X as FreeShape>::F, A>` for acquire). Trade-offs: keeps the brand parameter shape consistent with Catch / Local; preserves the substrate's user-visible result type as just B; no recursive-type concern (F is only referenced via FreeShape's associated type, not as a struct parameter directly). RISK: introduces a new trait (FreeShape) as substrate machinery; impl complexity increases; the FreeShape-bounded HRTB might surface its own substrate issues (untested).
  - **D. Type-erased program storage: use `Box<dyn Any>` or similar for the Run programs.** Field types are erased; dispatcher downcasts at runtime. Brand: 3-param. Trade-offs: simplest type-level shape. RISK: loses type safety; runtime downcast cost; deviates significantly from the existing typed-substrate pattern (Catch / Local use fully-typed closure storage). Likely incompatible with the Phase 4 Functor / SendFunctor / RefFunctor cascade requirements (the trait impls expect typed projections, not Any).
- **Recommendation: Option C (HKT-trait decomposition via FreeShape).** Reasoning: (i) preserves the brand's parameter shape (3 params, uniform with Catch / Local); (ii) preserves the user-visible result type as just B (no `BracketStep` leak); (iii) introduces a small, self-contained substrate addition (FreeShape trait) that's mechanically defined and testable; (iv) avoids the recursive-type concern of Option A; (v) avoids the dispatcher-complexity / wrap-overhead cost of Option B; (vi) avoids the type-safety loss of Option D. The FreeShape trait would be a single ~10-line addition: `pub trait FreeShape { type F; type Inner; }` plus `impl<F, A> FreeShape for Free<F, A> { type F = F; type Inner = A; }` (and parallel impls for the other Free variants). Risk: untested; if FreeShape's HRTB surfaces issues, fall back to Option A (5-param struct, accept the recursive-type concern and verify Rust accepts it). Awaiting user confirmation on Option C, with Option A as the explicit fallback.

  3.3.1 (Bracket Val foundational scaffold + B14 closure) is the next greenfield step but **blocked on B17 resolution**.
