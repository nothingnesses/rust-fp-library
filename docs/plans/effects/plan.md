...
**Phase 4 step 3.2.8: RefLocal (Ref flavour) integration tests.** One `feat(effects)` commit. 22 shape-only tests land at [`fp-library/tests/run_ref_local.rs`](../../../fp-library/tests/run_ref_local.rs) across six Run wrappers, mirroring [run_local.rs](../../../fp-library/tests/run_local.rs)'s template. Per wrapper: T1 (`*_t1_ref_local_produces_scoped_layer`) verifies `ref_local(modify, action)` peels to `Err(Node::Scoped(Coproduct::Inl(_::Local { .. })))` with the per-wrapper variant tag (`BoxRefLocal::Local` for Run/RunExplicit, `RefLocal::Local` for RcRun/RcRunExplicit, `SendRefLocal::Local` for ArcRun/ArcRunExplicit; variant name uniformly `Local` per the closed B12 decision); T2 (`*_t2_action_thunk_materialises_action_program`) destructures the cell, invokes `action(())`, and asserts the materialised wrapper-typed program peels to `Ok(42)`; T3 (`*_t3_modify_transforms_environment_by_reference`) invokes `modify(&10)` and asserts `== 11` (using `|e: &i32| *e + 1` as the modify closure). T4 (`*_t4_clone_yields_two_independent_peels`) on the four Clone-able wrappers (RcRun, ArcRun, RcRunExplicit, ArcRunExplicit) clones the suspended program and confirms both peelable handles materialise the original action independently via the cell's Rc-bump / Arc-bump Clone impl. The only structural difference from the Val-cycle T3 is the `&` on the input (`modify(&10)` instead of `modify(10)`); the Ref flavour's `Fn(&E) -> E` shape returns owned `E` like the Val flavour. Test file uses `FirstRow = CNilBrand` to sidestep the `Free` layout cycle; `#![expect(clippy::panic, reason = "...")]` matches run_local.rs's precedent for pattern-match exhaustion via panic. End-to-end environment-modification semantics (the dispatcher applying `modify` to a borrow of the inherited environment before invoking `action`) are not exercised; that path comes online with the scoped-handler dispatch protocol and the standard reader handler. Closes the entire 3.2 sub-cycle (8 commits across Val + Ref). `just verify` clean (22 run_ref_local tests passing).

**Phase 4 step 3.2.7 (commit `15300200`): RefLocal (Ref flavour) smart constructors per wrapper.** One `feat(effects)` commit. Six per-wrapper `ref_local` smart constructors land at [`run.rs`](../../../fp-library/src/types/effects/run.rs) (`Run::ref_local`), [`rc_run.rs`](../../../fp-library/src/types/effects/rc_run.rs) (`RcRun::ref_local`), [`arc_run.rs`](../../../fp-library/src/types/effects/arc_run.rs) (`ArcRun::ref_local`), [`run_explicit.rs`](../../../fp-library/src/types/effects/run_explicit.rs) (`RunExplicit::ref_local`), [`rc_run_explicit.rs`](../../../fp-library/src/types/effects/rc_run_explicit.rs) (`RcRunExplicit::ref_local`), and [`arc_run_explicit.rs`](../../../fp-library/src/types/effects/arc_run_explicit.rs) (`ArcRunExplicit::ref_local`); each takes `(modify, action)` (PureScript Run's `local` argument convention) and returns a wrapper-typed program suspended at the scoped Local layer. Per-wrapper bound shapes mirror `local`'s. The implementation is mechanically derived from the corresponding `local` smart constructors with three surface-level swaps: `modify` parameter signature changes from `Fn(E) -> E` to `Fn(&E) -> E` (Box: `FnOnce(&E) -> E`); the modify field constructed via `<P>::ref_new` (closing the matrix gap landed in step 3.2.5) instead of `<P>::new`; the cell type swapped to `BoxRefLocal` / `RefLocal` / `SendRefLocal`. ArcRun::ref*local reuses the existing `make_node_scoped` / `wrap_first_arc` HRTB-poisoning helpers from step 3.1.3. The smart-constructor name `ref_local` follows the `ref*\*` prefix convention used uniformly across the codebase. 6 smart-constructor doctests (one per wrapper).

**Phase 4 step 3.2.6 (commit `6ca9157a`): RefLocal (Ref flavour) `RefFunctor` impls + brand-projection helpers.** One `feat(effects)` commit. Mirrors step 3.2.2's pattern for the Ref flavour. Two `#[doc(hidden)]` helpers ship at [`fp-library/src/types/effects/ref_local.rs`](../../../fp-library/src/types/effects/ref_local.rs): `ref_local_modify_ref` extracts the `&Rc<dyn Fn(&E) -> E>` modify reference; `ref_local_action_thunk_ref` extracts the `&Rc<dyn Fn(()) -> A>` action thunk reference. Each helper takes a separate `'b` borrow lifetime distinct from the ref-local effect's content lifetime `'a`. `BoxRefLocalBrand<BoxBrand, E>::ref_map` builds the new ref-local with `unreachable!`-stub modify and action closures (suppressed via `#[expect(clippy::unreachable, reason = "...")]` mirroring the `BoxLocalBrand` precedent) because both `Box<dyn FnOnce(&E) -> E>` and `Box<dyn FnOnce(()) -> A>` cannot be replicated through a reference. `RefLocalBrand<RcBrand, E>::ref_map` is faithful: clones the modify Rc unchanged (modify's `&E -> E` signature does not depend on the result type), clones the action thunk's Rc, and post-composes `func` over the action's output via `<RcBrand as ToDynCloneFn>::new`. `SendRefLocalBrand<ArcBrand, E>` deliberately omits `RefFunctor` (mirrors `SendLocalBrand`-no-`RefFunctor` and `SendCatchBrand`-no-`RefFunctor` precedents). Compared to step 3.2.2's Val-cycle impl, the only shape differences are: the helper return type `&Rc<dyn Fn(&E) -> E>` (Ref) vs `&Rc<dyn Fn(E) -> E>` (Val) and the doctest closure shape `|e: &i32| *e + 1` (Ref) vs `|e: i32| e + 1` (Val); the RefFunctor::ref_map composition pattern itself is identical because `func` only goes through the action thunk. 4 new doctests; total ref_local.rs doctests now 17.

**Phase 4 step 3.2.5 (commit `1668b2e5`): RefLocal (Ref flavour) foundational scaffold + `ToDynFnOnce::ref_new` matrix completion.** One `feat(effects)` commit. Closed B11 + B12. Four pieces in one focused unit. **Closure-trait matrix completion (B11):** [`ToDynFnOnce`](../../../fp-library/src/classes/to_dyn_fn_once.rs) gained `ref_new<'a, A: 'a, B: 'a>(f: impl 'a + FnOnce(&A) -> B) -> Self::Of<'a, dyn 'a + FnOnce(&A) -> B>` plus a free-function shim `to_ref_dyn_fn_once`; `BoxBrand`'s impl gained the parallel `ref_new` returning `Box::new(f)` at [`box_ptr.rs`](../../../fp-library/src/types/box_ptr.rs). **RefLocal foundational scaffold:** three sibling effect types at [`ref_local.rs`](../../../fp-library/src/types/effects/ref_local.rs) (`BoxRefLocal<'a, P, E, A>` / `RefLocal<'a, P, E, A>` / `SendRefLocal<'a, P, E, A>`) each holding a `Local { modify, action }` variant (B12: variant name uniformly `Local`) with `modify: <P>::Of<'a, dyn ClosureTrait(&E) -> E>` and `action: <P>::Of<'a, dyn ClosureTrait(()) -> A>` (B-thunk per the closed B9 decision). Three brand declarations at [`brands/effects.rs`](../../../fp-library/src/brands/effects.rs). Four of five substrate-required trait impls per brand. Manual `Clone` for `RefLocal` (Rc-bump) and `SendRefLocal` (Arc-bump); `BoxRefLocal` is non-`Clone`. `SendRefLocalBrand` does NOT impl `Functor` (mirrors precedents). 13 doctests.
...
Three blockers / sub-decisions surface for step 3.3 (Bracket / RefBracket scoped-effect constructor): B14 (mechanical reuse of B7 / B9 precedent for the `acquire` field's layout cycle, low-stakes), B15 (already closed in [decisions.md line 484](decisions.md): RefBracket has only 2 sibling types instead of 3 due to BoxBrand exclusion, surfaced for transparency and step-3.3 sub-split planning), and B16 (step 3.3 sub-split decision; mirrors B6 / B8 with the asymmetric Ref-sibling-count adjustment). Closed blockers from earlier steps: B5 + B6 (3.1 sub-split, 4 commits), B7 (Catch action layout cycle, B-thunk), B8 + B9 + B10 (3.2 sub-split + Local action layout cycle reuse + file organization, 8 commits Val + Ref), B11 + B12 (`ToDynFnOnce::ref_new` matrix gap + variant naming uniformly `Local`).

#### B14. Bracket `acquire` field layout cycle (B7 / B9 reuse)

- **Issue.** Per [decisions.md table at line 480-481](decisions.md), `Bracket<'a, P, A, B>` and `RefBracket<'a, P, A, B>` both hold `acquire: Run<R, S, A>` as a direct field. The substrate placement is identical to Catch's pre-B7 `action: A` field and Local's pre-B9 `action: A` field: `Free -> NodeBrand::Scoped -> Coproduct -> Bracket -> acquire: Free<...>` -> back to `Free`, completing a layout cycle. The B7 / B9 resolutions adopted B-thunk uniformly for Catch / Local; the question is whether to apply the same uniformly to Bracket / RefBracket. Note that the `body` and `release` fields don't need the B-thunk treatment because their `Run<R, S, ...>` return values live inside their closures (inside the closure body, materialised on call), not as direct fields of the cell.
- **Options.**
  - **A. Apply B-thunk uniformly (mirror B9 closure for Local).** Store `acquire` as `<P>::Of<'a, dyn ClosureTrait(()) -> Run<R, S, A>>` for both Bracket Val and RefBracket Ref siblings. The smart constructor wraps the user's `acquire: Run<R, S, A>` parameter via `<P>::new(move |_: ()| acquire)`. The dispatcher (step 6) calls the thunk to materialise the Run program at execution time. Mirrors the Catch / Local precedents; requires no new substrate machinery. Cost: small (one extra thunk wrap per smart constructor; mechanical translation of the Local pattern).
  - **B. Use uniform `Box<Run<R, S, A>>` indirection.** Pointer-sized layout breaks the cycle but pays a `Box` allocation on multi-shot wrappers where the per-pointer-brand projection would otherwise be `Rc` / `Arc`; introduces a layout shape distinct from Catch / Local's B-thunk pattern, fragmenting the abstraction. Rejected by precedent (B7 considered and rejected this option for the same reason).
  - **C. Per-pointer-brand pointer for raw `Run<R, S, A>` (mirror B-literal).** Faithful per-pointer-brand pattern but extracting `Run<R, S, A>` from `Rc<Run<R, S, A>>` requires `Run<R, S, A>: Clone` which adds an undesirable substrate constraint to dispatcher code. Rejected by precedent.
- **Recommendation: Option A (apply B-thunk uniformly, mirroring B7 / B9 resolutions).** Mechanical reuse of established precedent; no new design risk; consistent surface across Catch / Local / Bracket. The smart-constructor wrap pattern is identical to Catch / Local's: user passes a Run, smart constructor wraps in `<P>::new(move |_: ()| run)`. Resolution can be bundled into the foundational scaffold commit (3.3.1) without separate resolution discussion, given the precedent. Surfaced for transparency only.

#### B15. Bracket Val and Ref sibling counts asymmetric (closed in decisions.md, surfaced for sub-split planning)

- **Issue.** Per [decisions.md line 484](decisions.md), `RefBracket<'a, P, ...>` requires `P` to be a refcounted brand (`RcBrand` or `ArcBrand`); `BoxBrand` does not satisfy `P::Of<A>: Clone` and is rejected at the type level. This means the Val and Ref cycles ship asymmetric surface areas:
  - **Bracket (Val flavour):** 3 sibling types (`BoxBracket` / `Bracket` / `SendBracket`) and 3 brands (`BoxBracketBrand` / `BracketBrand` / `SendBracketBrand`); smart constructors ship on all 6 Run wrappers. Surface roughly mirrors the Local Val cycle.
  - **RefBracket (Ref flavour):** only 2 sibling types (`RefBracket` / `SendRefBracket`) and 2 brands (`RefBracketBrand` / `SendRefBracketBrand`); smart constructors ship on only 4 wrappers (`RcRun`, `ArcRun`, `RcRunExplicit`, `ArcRunExplicit`). `Run::ref_bracket` and `RunExplicit::ref_bracket` are NOT defined (no BoxBrand-compatible RefBracket exists).

  Compared to the Local cycle (where both Val and Ref had 3 siblings each / 6 smart constructors each), Bracket's Ref cycle has roughly two-thirds the surface area of its Val cycle. This bears on the step 3.3 sub-split (B16): the Ref cycle's foundational scaffold, RefFunctor + helpers, and smart constructors will be smaller commits than their Val parallels.

- **Options.** Already closed in decisions.md; listing for transparency:
  - **A. RefBracket has 2 siblings (Rc, Arc).** Locked in by decisions.md.
  - **B. Add a hypothetical `BoxRefBracket` with `Box<dyn FnOnce(&A) -> ...>` shape.** Rejected by [decisions.md line 539](decisions.md): the `&A` shape "collapses into a useless signature on closer inspection: the returned `Run<R, S, B>` outlives the `&A` borrow."
- **Recommendation: Option A (already closed in decisions.md).** Implementation plan: Bracket Val foundational scaffold lands 3 sibling types and 3 brands (`BoxBracket` / `Bracket` / `SendBracket` plus `BoxBracketBrand` / `BracketBrand` / `SendBracketBrand`); RefBracket Ref foundational scaffold lands 2 sibling types and 2 brands (`RefBracket` / `SendRefBracket` plus `RefBracketBrand` / `SendRefBracketBrand`). Variant names uniformly `Bracket` across all five sibling enums per the B12 precedent. Surfaced so step 3.3 sub-split (B16) can plan accordingly.

#### B16. Step 3.3 sub-step splitting given Val + Ref + asymmetric Ref-sibling counts

- **Issue.** Step 3.3 covers both Bracket (Val flavour) and RefBracket (Ref flavour), mirroring the 3.2 split. But the structural shape diverges from 3.2 in two ways:
  - **More fields per type (3 vs 2):** Bracket has 3 fields (`acquire` / `body` / `release`) where Local has 2 (`modify` / `action`). Foundational scaffold commits will be larger (more enum field destructuring, more Functor::map composition surface, more brand-projection helpers). Smart-constructor commits will be larger (3-argument signatures with `(A, B)` tuple-threading on Val).
  - **Asymmetric sibling counts (per B15):** Val cycle ships 3 siblings × 6 wrappers = 18 surface points; Ref cycle ships 2 siblings × 4 wrappers = 8 surface points. Ref-cycle commits are roughly half the size of Val-cycle commits.
  - **B-thunk on `acquire` (per B14):** Bundled into foundational scaffold commit; small mechanical addition, no separate commit.
- **Options.**
  - **A. 8-commit symmetric split (mirror 3.2): 3.3.1-3.3.4 Val cycle (foundational scaffold + B14 / RefFunctor + helpers / smart constructors / integration tests); 3.3.5-3.3.8 Ref cycle (same pattern).** Each commit is a focused vertical slice. Trade-offs: precedent-aligned with 3.2's pattern; Ref-cycle commits are smaller (only 2 brands / 4 smart constructors / fewer integration tests) but still focused and reviewable on their own; 1:1 Val/Ref parallel simplifies cross-referencing during review and future maintenance; commit count parity simplifies plan tracking and fixed across all four scoped-effect cycles (Catch 4 / Local 8 / Bracket 8 / Span TBD). Risk: minimal.
  - **B. 7-commit split: 3.3.1-3.3.4 Val cycle, 3.3.5-3.3.7 Ref cycle (compress two smaller Ref pieces into one).** E.g., merge "Ref RefFunctor + helpers" with "Ref smart constructors" into a single commit. Trade-offs: smaller commit count; but mixing two distinct categories in one commit reduces review focus (a commit that lands BOTH RefFunctor impls AND smart constructors is reviewing two separate concerns); breaks the 1:1 Val/Ref parallel pattern from 3.2.
  - **C. 6-commit split: 3.3.1-3.3.4 Val cycle, 3.3.5-3.3.6 Ref cycle (merge foundational + RefFunctor + smart constructors into one Ref commit; integration tests separate).** Trade-offs: even smaller commit count; substantial compression on Ref side; significantly breaks the per-step protocol's "vertical slice per commit" principle that has worked across 3.1 (4 commits) and 3.2 (8 commits).
- **Recommendation: Option A (8-commit symmetric split mirroring 3.2).** Reasoning: (i) precedent works — 3.1's 4-commit split and 3.2's 8-commit split both produced focused, reviewable commits with no regrets; (ii) smaller Ref-cycle commits in Option A are still bounded review surface and clearly typed (a "Ref smart constructors" commit ships smart constructors and only smart constructors; a "Ref integration tests" commit ships only tests); (iii) maintaining the 1:1 Val/Ref parallel simplifies reviewing the diff between Val and Ref commits (Val 3.3.1 vs Ref 3.3.5 are the same step in different flavours, easy to compare); (iv) the 8-commit count is mechanically determined by the per-step protocol's structure (foundational scaffold / RefFunctor + helpers / smart constructors / integration tests x 2 flavours = 8); (v) Options B and C trade clarity for marginal commit-count reduction. Awaiting user confirmation.

  3.1 sub-steps shipped at `abd3d1a3` / `5bb2d1ae` / `205eaba4` / `faab175f`. Local Val cycle steps 3.2.1 / 3.2.2 / 3.2.3 / 3.2.4 shipped at `ef9b2eec` / `cbe401a4` / `970ad399` / `ba080e44`. RefLocal Ref cycle steps 3.2.5 / 3.2.6 / 3.2.7 / 3.2.8 shipped at `1668b2e5` / `6ca9157a` / `15300200` / `b65ac467`.
