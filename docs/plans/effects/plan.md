...
- **Phase 2** (Run substrate and first-order effects): complete. All 10 steps; the `poc-effect-row/` workspace was deleted in 10b after its tests migrated to `fp-library/tests/run_row_canonicalisation.rs` in 10a.
...
`Send + Sync` recovery ordering. B46 is resolved via Option C first,
with Option B as the fallback: add a private lifecycle-generated-action
carrier hook for Bracket / RefBracket, because the selected body action
...
use and Arc `Send + Sync` obligations. Step 7.4.4c.3b migrated shared
Explicit Catch, Local, and RefLocal to the same indexed boundary
surface: `CatchDispatcher`, `LocalDispatcher`, and
`RefLocalDispatcher` now resume shared Explicit boundaries while
preserving recovery-before-outer-continuation ordering, Reader
environment modification, repeated Rc use, and Arc `Send + Sync`
obligations. Step 7.4.4c.3c migrated shared Explicit Bracket and
RefBracket to the same indexed boundary surface: `BracketDispatcher`
...
dispatch for the shipped standard scoped dispatchers. Step 7.4.5
migrated `SpanDispatcher` onto the private H2 carrier path for
default `Run`, `RcRun`, `ArcRun`, and the Explicit boundary families
while preserving ordinary one-slot scoped dispatch for non-carrier
handlers. Step 7.4.6 added focused Span regressions for plain
...
- **Phase 5** (integration tests, benches, and documentation): complete.
  ...
  The following production-code split moved `ArcRunExplicit`'s public
  first-order and scoped smart constructors into
  `arc_run_explicit/smart_constructors.rs`.
  The next production-code split mirrored that public smart-constructor
  concern for `ArcRun` in `arc_run/smart_constructors.rs`.
  The following production-code split moved default `Run`'s public
  first-order and scoped smart constructors into
  `run/smart_constructors.rs`.
  ...
  The next production-code split moved `RcRunExplicit`'s public
  first-order and scoped smart constructors into
  `rc_run_explicit/smart_constructors.rs`.
  ...
  5.7 added `fp-library/docs/custom-effects.md`,
  ...
  Phase 5 step 7.1.1 added the neutral scoped Writer substrate:
  Box/Rc/Arc `censor` cells, Box/Rc/Arc `listen` cells whose brands
  ...
  implementation steps land. B70 is resolved via Option A: thread the
  consumed scoped-row member brand / index through the Explicit
  boundary types and use that evidence to build the residual ordinary
  ...
  5 step 8 added `fp-library/docs/run.md`,
...
- [Resolved (2026-05-16): B71 Explicit boundary carrier walk over-constrains non-consumed scoped handlers](resolutions.md#resolved-2026-05-16-b71-explicit-boundary-carrier-walk-over-constrains-non-consumed-scoped-handlers)
  ...
  directly, so only that handler proves carrier-aware semantics and
  non-consumed scoped handlers remain ordinary residual dispatch.
- [Resolved (2026-05-16): B70 Explicit boundary residual dispatch lacks boundary-position evidence](resolutions.md#resolved-2026-05-16-b70-explicit-boundary-residual-dispatch-lacks-boundary-position-evidence)
  : B70 adopts Option A: thread the consumed scoped-brand /
  member-index evidence through Explicit boundary values so residual
  ordinary scoped dispatch can skip exactly the boundary-only head and
  preserve dispatch for the rest of the scoped row.
...
- [Resolved (2026-05-15): B68 Writer `listen` needs preserving accumulation, not consume-only accumulation](resolutions.md#resolved-2026-05-15-b68-writer-listen-needs-preserving-accumulation-not-consume-only-accumulation)
  : B68 adopts Option A: add a private one-pass preserving
  accumulation protocol for Writer `listen`, with a Writer-specific
  preserving traversal retained only as the fallback if the general
...
- [Resolved (2026-05-15): B65 Box-backed Writer censor transform callability](resolutions.md#resolved-2026-05-15-b65-box-backed-writer-censor-transform-callability)
  ...
  as single-shot `FnOnce` thunks.
- [Resolved (2026-05-15): B64 standard Writer handler accumulation contract and surface](resolutions.md#resolved-2026-05-15-b64-standard-writer-handler-accumulation-contract-and-surface)
  : B64 adopts Option A: standard Writer scoped handlers use the
  existing `Monoid` / `Semigroup` classes for log accumulation, with
  `W: Monoid + Clone` only on aggregation / re-emission paths and
  `Send + Sync` added on Arc-family paths.
...
- [Resolved (2026-05-14): B59 `Run::interpose` needs a result-polymorphic replacement protocol before it can be boundary-aware](resolutions.md#resolved-2026-05-14-b59-runinterpose-needs-a-result-polymorphic-replacement-protocol-before-it-can-be-boundary-aware)
  ...
  `Run::interpose` through it, and keep the closure-taking convenience
  only for first-order-only `Run<R, CNilBrand, A>` programs. Private
  TypeErasedValue-only helpers and typed-boundary internals are not the
  primary fix because they would either leave the public single-shot
  continuation hazard in place or fail to make replacement callable at
...
- [Resolved (2026-05-14): B53 Explicit interpreter facade avoids exposing private H2 carrier traits](resolutions.md#resolved-2026-05-14-b53-explicit-interpreter-facade-avoids-exposing-private-h2-carrier-traits)
  ...
  facade gives indexed Explicit boundaries a uniform
  interpreter-like surface without exposing
  `DispatchScopedCarrierHandlers` or cementing effect-specific
  dispatcher calls as the only public route.
- [Resolved (2026-05-13): B52 production indexed boundary needs action-supplied resume vocabulary](resolutions.md#resolved-2026-05-13-b52-production-indexed-boundary-needs-action-supplied-resume-vocabulary)
  : B52 adopts Option C for 7.4.4c.1c.0: generalise the
  lifecycle-shaped outer-only Explicit continuation path into an
  action-supplied scoped resume contract shared by direct indexed
  boundaries and Bracket / RefBracket lifecycle dispatch. This avoids
  duplicating selected actions in private carriers and avoids making
  lifecycle terminology the generic abstraction for Span, Local, Catch,
  and other around-action effects.
...
- [Resolved (2026-05-12): B46 Bracket / RefBracket selected action is lifecycle-generated](resolutions.md#resolved-2026-05-12-b46-bracket--refbracket-selected-action-is-lifecycle-generated)
  ...
  carrier directly because the body action only exists after acquire
  produces the resource. Step 7.4.4b.3c now starts with a private
  lifecycle-generated-action carrier hook, with Bracket-specific
  lifecycle layers retained as the fallback if the generalized hook
...
9. **Generic `lift` combinator** (PureScript Run's
   ...
   **9f. Switch `ArcRunExplicit` to `SendFunctor`-routed dispatch.**
   ...
   `fp-library/src/types/effects/arc_run_explicit.rs`.
...
4. Re-run integration tests for `State`, `Reader`, `Choose` to verify the retrofit preserves observable behaviour. Existing tests:
   `fp-library/tests/run_state.rs` (18 tests),
   `fp-library/tests/run_reader.rs` (12 tests),
   `fp-library/tests/run_choose.rs` (4 tests).
...
2. Substrate-level `Run::interpose<EBrand, Idx>` primitive on
   ...
   `fp-library/tests/poc_rc_run_interpose.rs`;
...
3. Standard scoped-effect constructors. Each closure-bearing
   ...
   mirroring how Phase 3 `State<'a, P, S, A>`
...
7.  Standard scoped-handler implementations as a parallel
    ...
    - **7.4 Internal continuation carrier for around-action scoped
      ...
      need explicit handler classes.
           ...
           `run_standard_scoped_handlers.rs`
           integration tests.
           ...
           ordering (B46 Option C first, Option B fallback).** Add
           carrier-backed Bracket and RefBracket paths through a
           lifecycle-generated-action carrier hook. The dispatcher must
           preserve acquire -> body -> effectful release -> return body
           result ordering, use the constructor flavour's resource
           ownership semantics, and keep panic/drop cleanup claims
           limited to ordinary Rust `Drop` behaviour. If the lifecycle
           hook proof requires public API churn, unsafe erasure, or
           unbounded generic callbacks, fall back to Bracket-specific
           lifecycle carrier layers rather than weakening the
           ...
           selected action is a real row projection value.
             ...
             coverage in `run_explicit.rs`: a typed delayed Span frame
             exposes the scoped row as
             `SBrand::Of<'a, ActionProgram>` while carrying the outer
             continuation separately, preserves borrowed action payloads,
             ...
             concrete wall.
               ...
               over the new representation. B52 Option C is adopted, so
               begin by generalising the action-supplied resume
               vocabulary, then cover construction, `pure`, `send`,
               scoped constructors, `bind`, `map`, `peel` / view
               conversion, interpretation, extraction, and interpose
               paths. Ordinary first-order and pure programs must keep
               their current behaviour; only delayed around-action
               scoped frames should use the private boundary path.
                 ...
                 - **7.4.4c.1c.1 Add production `RunExplicitBoundary`
                   ...
                   slot. Focused coverage proves a borrowed Span action
                   layer can be split from the boundary, supplied to the
                   action-supplied continuation, and mapped to a distinct
                   final result type.
                 - **7.4.4c.1c.2 Migrate Span first (shipped).** Route
                   Span through the action-supplied carrier-aware path.
                   `RunExplicit::span` now returns the indexed boundary
                   instead of an ordinary `RunExplicit` program, and
                   `SpanDispatcher` exposes a boundary resume path that
                   observes the tag, runs result-preserving post-action
                   work, and then resumes the final continuation. Focused
                   coverage proves boundary `bind` / `map` preserve the
                   selected action slot until dispatch. No result-only
                   compatibility helper was needed for the migrated test
                   surface.
                 - **7.4.4c.1c.3 Recheck core operations
                   (shipped).** Focused coverage proves `pure`,
                   `send`, ordinary first-order interpretation, ordinary
                   scoped interpretation over a manually suspended Span
                   layer, extraction, and interpose through scoped
                   layers still use the ordinary `RunExplicit<Final>`
                   path unless an around-action constructor returns an
                   indexed boundary.

               - **7.4.4c.1d Prove the other single-shot
                 around-action effects still fit (shipped).** Reuse the
                 7.4.4b carrier-cell proofs as regression coverage while
                 moving the `RunExplicit` proof path to action-owned
                 scoped-row projection. Local / RefLocal boundary
                 dispatch projects action-owned `BoxLocal` /
                 `BoxRefLocal` layers and transforms the supplied
                 selected action before outer continuation resume. Catch
                 boundary dispatch projects an action-owned `BoxCatch`
                 layer and preserves recovery ordering plus recovery
                 rethrow behaviour. Bracket boundary dispatch projects a
                 `BoxBracketExplicit` lifecycle layer and preserves
                 acquire -> body -> release -> outer-continuation
                 ordering. RefBracket's production boundary proof stays
                 with step 7.4.4c.3 because the real RefBracket Explicit
                 substrate is Rc/Arc-backed, not the single-shot
                 `RunExplicit` / `FreeExplicit` boundary surface.

               - **7.4.4c.1e Gate the fallback (shipped).** No concrete
                 Rust compiler, safety, privacy, or HKT/class-composition
                 wall surfaced in 7.4.4c.1a-1d, so the B49 Option C
                 private standard-effect carrier row-shape fallback is
                 not activated. If a later step hits such a wall, record
                 that wall in `resolutions.md` before adopting the
                 fallback; do not switch to it solely because it is
                 smaller.
             ...
             interpreters.
               ...
               RefLocal constructors (shipped).** Move the existing focused
               ...
               RefBracket constructors (shipped).** Move lifecycle-generated
             ...
             - **7.4.4c.4 Wire indexed Explicit boundaries through a
             public facade (B53 Option C).** Add a narrow public
             boundary-handler surface over the private H2 carrier
             protocol so indexed Explicit boundaries can expose
             interpreter-like methods without naming
             `DispatchScopedCarrierHandlers` in public bounds. Preserve
             the ordinary `DispatchScopedHandlers` path for direct
             Explicit programs whose scoped handlers simply produce the
             next program.
               ...
               delegating internally to H2 carrier handlers without
           ...
           - **7.4.5 Migrate Span to the carrier path (shipped).**
           `SpanDispatcher` uses the H2 carrier so it observes tags
           around the interpreted action and returns the action result
           unchanged. The raw default `Run` path preserves the direct
           erased-result convention; the raw `RcRun` / `ArcRun` paths
           append a raw-erased continuation without normal `bind`, so
           simple direct payloads and nested `CatchDispatcher` /
           `interpose` reboxing both use the existing continuation
           queues correctly.
           Ordinary scoped-dispatcher behavior remains unchanged for
           handlers that do not need an around-action carrier.

           - **7.4.6 Add focused regression tests (shipped).** Cover all
           six wrappers. Tests include plain `RcRun` / `ArcRun` nested
           Span result propagation through `SpanDispatcher`, repeated
           shared execution, borrowed payloads on Explicit wrappers,
           existing default `Run` nested Span propagation, existing
           Catch / Span coexistence with first-order handlers, and
           existing Explicit facade interpretation across the boundary
           families.

           - **7.4.7 Add scoped-operation allocation benchmarks (R3)
           (shipped).** Adds Criterion benchmarks comparing Bracket /
           RefBracket scoped construction and dispatcher execution
           against equivalent non-scoped bind chains that simulate the
           same behavior through closure capture. The benchmark covers
           default `Run` Bracket plus `RcRun` Bracket / RefBracket so
           Box-backed single-shot closure cells, Rc-backed multi-shot
           closure cells, and RefBracket pointer-clone lifecycle
           overhead are represented.

           - **7.4.8 Revisit the default `Run` raw scoped-dispatch
           adapter only if custom Box-backed scoped handlers become
           public API.** The current default `Run` raw scoped dispatch
           path uses dispatcher-specific raw-head impls. A blanket
           adapter from `DispatchScopedHandler` conflicts with the
           Box-backed `CatchDispatcher` specialization under Rust's
           coherence rules, and that is acceptable while only standard
           dispatchers consume this path. If custom Box-backed scoped
           handlers become a concrete public design target, add a
           dedicated design step comparing dispatcher-specific raw-head
           impls, a new non-overlapping adapter trait, and any necessary
           public API changes.
...
3. **Heftia current-effect semantic regressions are covered by 2.17.**
   ...
   - Choose + Catch ordering: each handler order has the expected
     branch-local or whole-program recovery shape for both the
     `pure true <|> throw` case and the `throw <|> pure true` case.
   - Custom first-order effect interpreted into Throw: handling Catch
     before vs after the custom effect interpreter produces the
     expected caught vs uncaught result.
...
4. **Expand cross-cutting effects composition regressions.** Add a
   ...
   - **4.4 Restore the cross-cutting composition matrix (shipped).**
     ...
     `run_effect_composition_matrix.rs`.
...
5. **Effects ergonomics, naming, diagnostics, and organization
   ...
   - **5.3 Split large effects modules by stable concern after the
     ...
     scoped-dispatch protocols into `run/representation.rs`. Second
     ...
     carrier into `rc_run/raw_scoped.rs`. Seventh production-code slice
     shipped: split `ArcRunExplicit`'s public first-order and scoped
     smart constructors into `arc_run_explicit/smart_constructors.rs`.
     Eighth production-code slice shipped: split `ArcRun`'s public
     first-order and scoped smart constructors into
     `arc_run/smart_constructors.rs`.
     Ninth production-code slice shipped: split default `Run`'s public
     first-order and scoped smart constructors into
     `run/smart_constructors.rs`.
     ...
     first-order and scoped smart constructors into
     `rc_run/smart_constructors.rs`.
     Twelfth production-code slice shipped: split `RcRunExplicit`'s
     public first-order and scoped smart constructors into
     `rc_run_explicit/smart_constructors.rs`.
     Thirteenth production-code slice shipped: split first-order
     ...
     Seventeenth production-code slice shipped: split `Local`'s
     ...
     Nineteenth production-code slice shipped: split `Catch`'s
     Explicit carrier-aware boundary, focused carrier helpers, and
     Rc/Arc Explicit carrier facades into
   ...
   - **5.7 Write the manual custom-effect authoring guide before
     ...
     `fp-library/docs/custom-effects.md`
...
7. **Expand standard effect families after the Phase 5 cleanup.** Do
   ...
   continuation, resumption, async, IO, or target-monad protocol. The
   Phase 5 expansion is intentionally bounded to Writer, Empty, and the
   Writer-dependent NonDet semantic port; optional smaller first-order
   ports and runtime-sensitive ports live in Phase 6+ with explicit
   revisit triggers.
   - **7.1 Implement Writer higher-order semantics first.** Add
     ...
     **7.1.4c.3 Decide and implement the Explicit-family route
     ...
     Option C / Option D remains unactivated.
     - **7.1.4d Implement `listen` on the same Monoid contract through
       B68 Option A.** Add a one-pass preserving accumulation protocol:
       accumulate the selected action's `Tell`s, rebuild each original
       `Tell` so the outer Writer handler still observes the original
       sequence, and resume the B63 operation-result continuation with
       `(action_value, observed_log)`. Preserve the existing first-order
       `tell` surface and do not introduce `RefWriter` in this step.
     ...
     - **7.1.4d.4a Split Explicit boundary handler-list obligations
       (B69 Option B, B70 Option A).** Update the
       `RunExplicitBoundary`, `RcRunExplicitBoundary`, and
       `ArcRunExplicitBoundary` path so the initial boundary consumes
       the matching scoped-handler cell through
       `DispatchScopedBoundaryHandlers` without requiring fake
       ordinary `DispatchScopedHandler` semantics for the
       result-changing boundary head. Preserve ordinary scoped-layer
       dispatch after boundary resume by carrying the consumed
     ...
     - **7.1.4d.4a.1 Thread consumed boundary-member evidence through
       ...
       `IntoScopedBoundaryParts` impls, and every Explicit
       around-action smart-constructor return type. This step is
       intentionally API-breaking; do not add compatibility aliases
     ...
     - **7.1.4d.4b Prove the B69 split before restoring the full
       listen tests through B71 Option B.** Add focused compile/runtime
...
- **Optional smaller first-order ports for guide or regression value.**
  Input, Output, Fresh, Log, KVStore, and similar first-order effects
  remain deferred unless they demonstrate a reusable authoring,
  handler, or regression pattern not already covered by Reader, State,
  Writer, Except, Choose, Empty, and the custom-effect guide. _Why
  deferred:_ Phase 5's standard-effect expansion is bounded to Writer,
  Empty, and the Writer-dependent NonDet semantic port; adding more
  first-order effects now would broaden scope without a concrete gap.
  _Trigger:_ a custom-effect guide example, user report, or focused
  regression needs one of these effects to demonstrate behaviour the
  existing effect set cannot show. Review trace:
  [Writer and NonDet gaps](review/2-effects-system-architecture/effects-system-review.md#writer-and-nondet-gaps)
  option 3 and
  [Custom-effect authoring](review/2-effects-system-architecture/effects-system-review.md#custom-effect-authoring).
- **Runtime-sensitive Heftia ports.** Coroutine, Concurrent, CC,
  Shift, Provider, Unlift, Stream, Subprocess, Timer, Parallel,
  DatabaseProvider, and similar runtime-sensitive effects remain
  deferred until the library has a concrete continuation-exposure,
  async, IO, or target-monad policy that makes their semantics
  precise. _Why deferred:_ these effects require runtime and
  continuation policies beyond the current scoped-row action semantics.
  _Trigger:_ an adopted continuation, async, IO, or target-monad design
  that can state their semantics without special-case shortcuts.
  Review trace:
  [Writer and NonDet gaps](review/2-effects-system-architecture/effects-system-review.md#writer-and-nondet-gaps)
  option 4.
...
- The Heftia semantic regression ports for current effects pass:
