...
- **Phase 4** (scoped effects via heftia-inspired dual row): steps 0-1, step 2 (sub-steps 2.1-2.6), step 2a, Catch 3.1.1-3.1.4, Local / RefLocal 3.2.1-3.2.8, Bracket / RefBracket 3.3.1-3.3.8, and Span 3.4.1-3.4.2 have shipped. Closed blockers and design decisions are tracked in [resolutions.md](resolutions.md) and [deviations.md](deviations.md). B21 is resolved via Option A: Span remains Val-only at the user-facing level, but the implementation mirrors Catch and Local with Box/Rc/Arc action-thunk substrate cells. B22 is resolved via Option A: Span stores tags by value and adds clone/send bounds only where Rc/Arc substrates require them. The next greenfield step is 3.4.3: Span integration tests. Q4 and R3 remain non-blocking follow-ups below.

### Next greenfield work

Phase 4 step 3.4.2 shipped on this branch: `span` smart constructors now exist on all six Run wrappers (`Run`, `RcRun`, `ArcRun`, `RunExplicit`, `RcRunExplicit`, `ArcRunExplicit`). Each constructor injects the wrapper's matching Span cell (`BoxSpan`, `Span`, or `SendSpan`) and preserves the B22 bound surface: default wrappers do not require `Tag: Clone`, Rc wrappers require cloneable tags, and Arc wrappers require `Tag: Clone + Send + Sync` plus the usual Arc row-projection bounds.

**Next greenfield step:**

**Step 3.4.3: Span integration tests.** Add integration tests for suspended-layer construction and tag/action thunk materialisation across `Run`, `RcRun`, `ArcRun`, `RunExplicit`, `RcRunExplicit`, and `ArcRunExplicit`. Keep the tests self-contained and focus on the substrate shape: the full end-to-end scoped-dispatcher semantics still belongs to the later standard scoped-handler rollout.
