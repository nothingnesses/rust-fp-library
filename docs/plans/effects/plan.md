...
**Next greenfield step: Phase 4 step 7.4.4c.1c.3, recheck core
operations.** Steps
...
result-only compatibility helper was needed for the migrated tests. The
next step is to recheck core operations and prove ordinary pure,
first-order, scoped interpretation, extraction, and interpose paths still
use `RunExplicit<Final>` unless an around-action boundary is present.
Steps
7.4.4b.3a.0 through
