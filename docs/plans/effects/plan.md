...
No active blockers. Closed blockers from earlier steps: B5 + B6 (3.1 sub-split, 4 commits), B7 (Catch action layout cycle, B-thunk), B8 + B9 + B10 (3.2 sub-split + Local action layout cycle reuse + file organization, 8 commits Val + Ref), B11 + B12 (`ToDynFnOnce::ref_new` matrix gap + variant naming uniformly `Local`), B14 + B15 + B16 (3.3 sub-split + Bracket acquire B-thunk reuse + RefBracket sibling-count asymmetry). B17 closed on 2026-05-08 per the [resolved B17 entry](resolutions.md#resolved-2026-05-08-phase-4-step-3.3.1-bracket-cells-three-differently-typed-program-returns-vs-substrates-single-gat-parameter-pattern-b17-closed): adopt Option C (HKT-trait `FreeShape` decomposition) as primary; Option A (5-param struct with explicit substrate brand) as explicit fallback if FreeShape's HRTB surfaces structural issues during 3.3.1 implementation.

3.3.1 (Bracket Val foundational scaffold + B14 closure + FreeShape substrate trait per B17) is the next greenfield step.
