...
**Step 3.3.3: Bracket Val smart constructors per wrapper** (six per-wrapper smart constructors). Concrete contents:

- `Run::bracket` paired with `BoxBracket` (BoxBrand + Free). Restored from stashed probe (`git stash@{0}`).
- `RcRun::bracket` paired with `Bracket` (RcBrand + RcFree).
- `ArcRun::bracket` paired with `SendBracket` (ArcBrand + ArcFree). Reuses HRTB-poisoning workaround helpers.
...
**Step 3.3.3** (Bracket Val smart constructors per wrapper): six per-wrapper smart constructors mapping each wrapper to its appropriate Erased or Explicit cell. Restores stashed Run::bracket probe and adds 5 more wrapper constructors.

3.1 sub-steps shipped at `abd3d1a3` / `5bb2d1ae` / `205eaba4` / `faab175f`. Local Val cycle steps 3.2.1 / 3.2.2 / 3.2.3 / 3.2.4 shipped at `ef9b2eec` / `cbe401a4` / `970ad399` / `ba080e44`. RefLocal Ref cycle steps 3.2.5 / 3.2.6 / 3.2.7 / 3.2.8 shipped at `1668b2e5` / `6ca9157a` / `15300200` / `b65ac467`. Bracket Val foundational scaffold (3.3.1) shipped at `1be2af3e`; Bracket Val `RefFunctor` (3.3.2) shipped at `46754fc0`; B18 surfaced at `e5ff559f` and closed via POC at `0cfe3e2b`; B19 surfaced at `de4cb852`, closed by docs at `f7cc7ea0`, closure-implementation shipped on top.
