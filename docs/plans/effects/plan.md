...
**Phase 4 step 3.2.5: RefLocal (Ref flavour) foundational scaffold + `ToDynFnOnce::ref_new` matrix completion.** One `feat(effects)` commit. Closes B11 + B12 (closure-trait matrix gap on `ToDynFnOnce`; variant naming uniformly `Local`). Four pieces in one focused unit. **Closure-trait matrix completion (B11):** [`ToDynFnOnce`](../../../fp-library/src/classes/to_dyn_fn_once.rs) gains `ref_new<'a, A: 'a, B: 'a>(f: impl 'a + FnOnce(&A) -> B) -> Self::Of<'a, dyn 'a + FnOnce(&A) -> B>` plus a free-function shim `to_ref_dyn_fn_once`; `BoxBrand`'s impl gains the parallel `ref_new` returning `Box::new(f)` at [`box_ptr.rs`](../../../fp-library/src/types/box_ptr.rs). The matrix in [`pointer-abstraction.md`](../../../fp-library/docs/pointer-abstraction.md) is now structurally complete: every closure trait ships both `new` and `ref_new`. **RefLocal foundational scaffold:** three sibling effect types at [`ref_local.rs`](../../../fp-library/src/types/effects/ref_local.rs) (`BoxRefLocal<'a, P, E, A>` where `P: ToDynFnOnce`, `RefLocal<'a, P, E, A>` where `P: ToDynCloneFn`, `SendRefLocal<'a, P, E, A>` where `P: ToDynSendFn`) each holding a `Local { modify, action }` variant (B12: variant name uniformly `Local`) with per-pointer-brand pointer storage: `modify: <P>::Of<'a, dyn ClosureTrait(&E) -> E>` (1-arg, borrows env per Ref flavour) and `action: <P>::Of<'a, dyn ClosureTrait(()) -> A>` (B-thunk for layout-cycle break per the closed B9 decision). Three brand declarations at [`brands/effects.rs`](../../../fp-library/src/brands/effects.rs) plus `impl_kind!` registrations. Four of five substrate-required trait impls per brand: `Functor` on `BoxRefLocalBrand<BoxBrand, E>` and `RefLocalBrand<RcBrand, E>` (specialised via `ToDynFnOnce::new` / `ToDynCloneFn::new`); `SendFunctor` across all three (load-bearing on `SendRefLocalBrand<ArcBrand, E>`, stub-delegating-to-`Functor::map` on Box and Rc flavours); `WrapDrop` and `Extract` across all three (returning `Some(action(()))` / `action(())`, dropping `modify` silently). Manual `Clone` impls for `RefLocal` (Rc-bump on both pointers) and `SendRefLocal` (Arc-bump); `BoxRefLocal` is unconditionally non-`Clone`. The fifth trait `RefFunctor` lands in step 3.2.6 mirroring step 3.2.2's deferral. `SendRefLocalBrand` does NOT implement `Functor` for the same `Send + Sync` bound mismatch as `SendLocalBrand` and `SendCatchBrand`; deviation logged at [deviations.md Phase 4 step 3.2.5](deviations.md). Module wired through [`fp-library/src/types/effects.rs`](../../../fp-library/src/types/effects.rs)'s `pub mod ref_local;`. 13 doctests passing. `just verify` clean.

**Phase 4 step 3.2.4 (commit `ba080e44`): Local (Val flavour) integration tests.** One `feat(effects)` commit. 22 shape-only tests land at [`fp-library/tests/run_local.rs`](../../../fp-library/tests/run_local.rs) across six Run wrappers, mirroring [run_catch.rs](../../../fp-library/tests/run_catch.rs)'s template. Per wrapper: T1 (`*_t1_local_produces_scoped_layer`) verifies `local(modify, action)` peels to `Err(Node::Scoped(Coproduct::Inl(_::Local { .. })))` with the per-wrapper variant tag (`BoxLocal::Local` for Run/RunExplicit, `Local::Local` for RcRun/RcRunExplicit, `SendLocal::Local` for ArcRun/ArcRunExplicit); T2 (`*_t2_action_thunk_materialises_action_program`) destructures the cell, invokes `action(())`, and asserts the materialised wrapper-typed program peels to `Ok(42)`; T3 (`*_t3_modify_transforms_environment`) invokes `modify(10)` and asserts `== 11` (using `|e: i32| e + 1` as the modify closure). T4 (`*_t4_clone_yields_two_independent_peels`) on the four Clone-able wrappers (RcRun, ArcRun, RcRunExplicit, ArcRunExplicit) clones the suspended program and confirms both peelable handles materialise the original action independently via the cell's Rc-bump / Arc-bump Clone impl. T3's structural difference from Catch's T3 is the only design adjustment: Catch's T3 verified `handler("oops")` produced a wrapper-typed recovery program because handler's signature is `E -> A` (the program type); Local's `modify` is `E -> E` so T3 asserts a scalar transformation directly. Test file uses `FirstRow = CNilBrand` to sidestep the `Free` layout cycle (an `IdentityBrand`-style first-order row has no pointer indirection on the variant payload); `#![expect(clippy::panic, reason = "...")]` matches run_catch.rs's precedent for pattern-match exhaustion via panic. End-to-end environment-modification semantics (the dispatcher applying `modify` to the inherited environment before invoking `action` via `interpret_with_either`) are not exercised; that path comes online with the scoped-handler dispatch protocol and the standard reader handler. `just verify` clean (22 run_local tests passing).

**Phase 4 step 3.2.3 (commit `970ad399`): Local (Val flavour) smart constructors per wrapper.** One `feat(effects)` commit. Six per-wrapper `local` smart constructors land at [`run.rs`](../../../fp-library/src/types/effects/run.rs) (`Run::local`), [`rc_run.rs`](../../../fp-library/src/types/effects/rc_run.rs) (`RcRun::local`), [`arc_run.rs`](../../../fp-library/src/types/effects/arc_run.rs) (`ArcRun::local`), [`run_explicit.rs`](../../../fp-library/src/types/effects/run_explicit.rs) (`RunExplicit::local`), [`rc_run_explicit.rs`](../../../fp-library/src/types/effects/rc_run_explicit.rs) (`RcRunExplicit::local`), and [`arc_run_explicit.rs`](../../../fp-library/src/types/effects/arc_run_explicit.rs) (`ArcRunExplicit::local`); each takes `(modify, action)` (PureScript Run's `local` convention) and returns a wrapper-typed program suspended at the scoped Local layer (verified via `peel().is_err()` doctests). Per-wrapper bound shapes mirror `catch`: BoxLocalBrand (`'static` lifetime, `FnOnce` single-shot) for default Run/RunExplicit; LocalBrand (`Rc + Fn`, `A: Clone + 'static` for Erased / `A: Clone + 'a` for Explicit) for Rc family; SendLocalBrand (`Arc + Fn + Send + Sync`, `A: Clone + Send + Sync`) for Arc family. ArcRun::local routes through the existing `make_node_scoped` / `wrap_first_arc` HRTB-poisoning workaround helpers landed in step 3.1.3; ArcRunExplicit::local needs no such helpers (Explicit-family `Send + Sync` projection bounds suffice). RunExplicit::local carries `#[expect(clippy::type_complexity)]` for the same reason `RunExplicit::catch` does (`Box<FreeExplicit<...>>` deep-chain). The implementation is mechanically derived from the corresponding `catch` smart constructors with two adjustments: argument order swapped to `(modify, action)`; `modify` field built via `<P>::new(move |e: E| modify(e))` instead of a unit-arg thunk (modify's `E -> E` shape doesn't need the B-thunk indirection because `E` doesn't recurse through the substrate). 6 smart-constructor doctests (one per wrapper); total local.rs doctests now 17 plus the 6 smart-constructor doctests across the six wrapper files. `just verify` clean.
