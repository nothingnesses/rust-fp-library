...
#### B18 (2026-05-08): User-facing recursive type alias rejection on Bracket-containing scoped rows

**Issue.** Under Option A (the design adopted for B17 after Option C's WF rejection), `BracketBrand<P, Sub, A, B>` carries `Sub` as a brand-level type parameter. A user defining a scoped row that contains `BoxBracketBrand<BoxBrand, NodeBrand<R, S>, A, B>` writes a recursive type alias:

```rust,ignore
type FirstRow = CNilBrand;
type ScopedRow = CoproductBrand<
    BoxBracketBrand<BoxBrand, NodeBrand<FirstRow, ScopedRow>, A, B>,
    CNilBrand,
>;
```

Rust rejects this with `error[E0391]: cycle detected when expanding type alias 'ScopedRow'`. Rust forbids recursive type aliases; the cycle must be broken via a nominal type (struct).

This issue is **not** a problem with the smart constructor's signature itself (the [stashed `Run::bracket` probe](../../../fp-library/src/types/effects/run.rs) compiles cleanly at the function-definition level, validating that Concern 1's recursive trait bound through `Sub` works fine). The issue is **user-facing**: any program using `Bracket` requires a row type that contains the brand, and that row's natural definition is a cyclic alias.

The probe was stashed via `git stash push -m "WIP: Run::bracket smart constructor probe (B18 blocker on user-facing recursive-row alias)"`; restore via `git stash apply stash@{0}`. The probe ships under [`fp-library/src/types/effects/run.rs`](../../../fp-library/src/types/effects/run.rs) (140-line addition: a new `impl<R, ScopedRow, A, B> Run<R, ScopedRow, (A, B)>` block plus `pub fn bracket`).

Catch / Local / RefLocal don't have this issue because their brands have no `Sub` parameter (the cell's program return type is the GAT-filled `X` directly); user rows containing `BoxCatchBrand<BoxBrand, E>` etc. are non-recursive. Option C (FreeShape decomposition with 3-param brand) would also have avoided the user-facing recursive alias, but Option C failed Rust's WF check at `Functor::map` (closed in B17). Bracket is the first scoped effect with self-referential brand structure under Option A.

**Options:**

- **A. Marker-struct workaround documented in user-facing patterns; defer ergonomics to `scoped_effects!` macro (step 5).** Users define a marker struct (zero-sized, manually implementing `Kind` / `WrapDrop` / `Functor` / `SendFunctor`) that breaks the type-alias cycle:

  ```rust,ignore
  struct ScopedRowMarker;
  // Manual Kind impl points to the recursive row body; Rust accepts this because
  // the recursion is in the impl body, not the struct definition.
  impl_kind! {
      impl for ScopedRowMarker {
          type Of<'a, A: 'a>: 'a = <CoproductBrand<
              BoxBracketBrand<BoxBrand, NodeBrand<FirstRow, ScopedRowMarker>, A1, B1>,
              CNilBrand,
          > as Kind>::Of<'a, A>;
      }
  }
  // Plus parallel WrapDrop / Functor / SendFunctor delegating impls.
  ```

  Then the user calls `Run::<FirstRow, ScopedRowMarker, _>::bracket(...)`. Verbose at the user level (~20-30 lines of boilerplate per program); the planned `scoped_effects!` macro (step 5) would generate this automatically. Step 3.3.3's smart constructor doctests use the marker pattern explicitly to demonstrate the API; step 3.3.4's integration tests follow the same pattern.

  **Trade-offs:** keeps the closed B17 design; preserves Option A's per-pointer-brand cell structure; ergonomic friction is real but bounded by macro generation in step 5; no runtime overhead; full static type checking. Downside: step 3.3.3 / 3.3.4 / 3.3.7 / 3.3.8 doctests + tests carry verbose marker-struct setup until step 5 lands.

- **B. Switch to dynamic-typed acquire/release fields via `Box<dyn Any>`.** Make BracketBrand 3-param (`BracketBrand<P, A, B>`, no `Sub`); store `acquire` and `release` as `Box<dyn Any>` in the cell. The smart constructor and dispatcher know `Sub = NodeBrand<R, S>` at their respective call sites and downcast via `Any::downcast<Free<NodeBrand<R, S>, A>>()`.

  **Trade-offs:** user rows are non-recursive (matching Catch/Local ergonomics); no marker-struct boilerplate. Downside: loses static type checking on acquire/release contents (downcasts are runtime-checked); a substrate-type mismatch at dispatch becomes a runtime panic instead of a compile error; `Box<dyn Any>` requires `'static` bounds on all stored Free programs (which we already have); structurally re-introduces the Phase 3.5 F4-style runtime-erasure pattern this project deliberately moved away from.

- **C. Reopen B17 and re-evaluate Option C with deeper structural exploration.** The Option C probe was rejected on a single error site (`Functor::map`'s WF check). Maybe a different formulation with `<X as FreeShape>::F` confined to non-trait-impl positions could work. Investigation cost: half-day to a day of structural exploration.

  **Trade-offs:** if it works, user rows are non-recursive (matching Catch/Local); the brand stays 3-param uniformly with other scoped brands. Downside: might not work after deeper investigation (the WF rule is general, not limited to one site); delays Phase 4 by 1-2 days.

**Recommendation: Option A.** Reasoning:

1. The marker-struct workaround is mechanical and well-understood (precedent in Haskell effect libraries via macro-generated boilerplate).
2. Step 5's `scoped_effects!` macro is already planned; it can hide the marker-struct complexity behind the same macro user calls today for `effects!`. The verbose setup is temporary.
3. Step 3.3.3's six per-wrapper smart constructors don't change shape under Option A; they just need marker-struct doctests.
4. Option B's dynamic typing is a structural regression (loses compile-time guarantees the rest of the substrate provides).
5. Option C's deeper investigation is high-uncertainty and high-cost; the structural failure at `Functor::map` looked terminal, not a corner case.
6. Per the closed B17 resolution's spirit (Option A as fallback), proceeding with Option A's natural extension keeps Phase 4 moving without re-litigating settled design.

**Plan-text amendment (pending user confirmation).** Step 3.3.3 ships six per-wrapper `bracket` smart constructors (`Run::bracket`, `RcRun::bracket`, `ArcRun::bracket`, `RunExplicit::bracket`, `RcRunExplicit::bracket`, `ArcRunExplicit::bracket`). Each smart-constructor doctest uses a marker-struct row pattern explicitly. A new deviation entry at [deviations.md Phase 4 step 3.3.3](deviations.md) documents the marker-struct workaround, its rationale, and its planned macro-generation in step 5. Step 5's `scoped_effects!` macro design (already deferred) gains a sub-task: generate marker structs + delegating Kind / WrapDrop / Functor / SendFunctor impls for scoped rows containing `Bracket`-family brands. The stashed Run::bracket probe (`git stash@{0}`) is restored and adapted to use a marker-struct doctest in the foundational 3.3.3 commit; the other 5 wrappers follow.

3.3.3 (Bracket Val smart constructors per wrapper) is the next greenfield step, awaiting B18 resolution.

3.1 sub-steps shipped at `abd3d1a3` / `5bb2d1ae` / `205eaba4` / `faab175f`. Local Val cycle steps 3.2.1 / 3.2.2 / 3.2.3 / 3.2.4 shipped at `ef9b2eec` / `cbe401a4` / `970ad399` / `ba080e44`. RefLocal Ref cycle steps 3.2.5 / 3.2.6 / 3.2.7 / 3.2.8 shipped at `1668b2e5` / `6ca9157a` / `15300200` / `b65ac467`. Bracket Val foundational scaffold (3.3.1) shipped at `1be2af3e`; Bracket Val `RefFunctor` (3.3.2) shipped at `46754fc0`.

Closed blockers from earlier steps: B5 + B6 (3.1 sub-split, 4 commits), B7 (Catch action layout cycle, B-thunk), B8 + B9 + B10 (3.2 sub-split + Local action layout cycle reuse + file organization, 8 commits Val + Ref), B11 + B12 (`ToDynFnOnce::ref_new` matrix gap + variant naming uniformly `Local`), B14 + B15 + B16 (3.3 sub-split + Bracket acquire B-thunk reuse + RefBracket sibling-count asymmetry), B17 (Bracket cell three-program-types vs substrate single-GAT-parameter; Option C `FreeShape` failed Rust's WF check at `Functor::map`, Option A 5-param struct fallback shipped per the B17 plan-text amendment, deviation logged at [deviations.md Phase 4 step 3.3.1](deviations.md)).
