...
- **Phase 4** (scoped effects via heftia-inspired dual row): steps 0-1, step 2 (sub-steps 2.1-2.6), step 2a, Catch 3.1.1-3.1.4, Local / RefLocal 3.2.1-3.2.8, and Bracket / RefBracket 3.3.1-3.3.7 have shipped, including the B19 per-Free-family substrate split. Closed blockers and design decisions are tracked in [resolutions.md](resolutions.md) and [deviations.md](deviations.md). The next greenfield step is 3.3.8: RefBracket Ref integration tests. Q4 and R3 remain non-blocking follow-ups below.

### Next greenfield work

Phase 4 step 3.3.7 shipped on this branch: `ref_bracket` smart constructors now exist on [`RcRun`](../../../fp-library/src/types/effects/rc_run.rs), [`ArcRun`](../../../fp-library/src/types/effects/arc_run.rs), [`RcRunExplicit`](../../../fp-library/src/types/effects/rc_run_explicit.rs), and [`ArcRunExplicit`](../../../fp-library/src/types/effects/arc_run_explicit.rs). The Rc wrappers store `RefBracket` / `RefBracketExplicit` cells through `RcBrand` and `ToDynCloneFn`; the Arc wrappers store `SendRefBracket` / `SendRefBracketExplicit` cells through `ArcBrand` and `ToDynSendFn` with `Send + Sync` bounds. Acquire remains a unit-arg B-thunk; body and release receive resource pointer clones.

**Next greenfield step:**

**Step 3.3.8: RefBracket Ref integration tests.** Add `tests/run_ref_bracket.rs` covering the four refcounted wrappers per B15: `RcRun`, `ArcRun`, `RcRunExplicit`, and `ArcRunExplicit`. Exercise suspended-layer construction for each wrapper and assert the constructors store RefBracket cells whose acquire/body/release closures return the expected programs. Keep Arc coverage mindful of the B20-style `Send + Sync` recursion risk; if a full `ArcRun::ref_bracket` marker-row test overflows, use the same bounded coverage pattern as `tests/run_bracket.rs` and record any new blocker only if the constructor itself no longer type-checks. Step 3.4 then lands Span.
...
Only Q4 and R3 remain pending, and both are non-blocking for the
current RefBracket sub-cycle. R1 and R2 were addressed by shipped
