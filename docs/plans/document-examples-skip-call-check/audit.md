...
Generated 2026-05-26. Refreshed after the Step 5 effects cleanup.
...
`just document-examples` count is `1051` because it includes the intentional
...
| `unnecessary_skip`         |   336 | Remove `skip_call_check` when the existing example already calls the item, or rewrite the example if the direct call is only an unreachable stand-in and the skip is still justified. |
...
`fp-library/src/types/effects` has zero objective invalid entries. Its stale
placeholder reasons and unnecessary skips are clean.
