...
Rather than a single `InferableBrand` trait, there is a family of traits,
one per Kind arity, using the same content-hash naming scheme as the Kind
traits. Each `InferableBrand_{hash}` trait corresponds to the `Kind_{hash}`
trait with the same signature:

| Kind trait              | Signature                        | InferableBrand trait              | Used by                        |
| ----------------------- | -------------------------------- | --------------------------------- | ------------------------------ |
| `Kind_ad6c20556a82a1f0` | `type Of<A>;`                    | `InferableBrand_ad6c20556a82a1f0` | Simple type constructors       |
| `Kind_cdc7cd43dac7585f` | `type Of<'a, A: 'a>: 'a;`        | `InferableBrand_cdc7cd43dac7585f` | Most functor/monad types       |
| `Kind_5b1bcedfd80bdc16` | `type Of<A, B>;`                 | `InferableBrand_5b1bcedfd80bdc16` | Bifunctor brands               |
| `Kind_266801a817966495` | `type Of<'a, A: 'a, B: 'a>: 'a;` | `InferableBrand_266801a817966495` | Bifunctor brands with lifetime |

The traits are generated by `trait_kind!` itself: when `trait_kind!` creates
a `Kind_{hash}` trait, it also creates the corresponding `InferableBrand_{hash}`
trait using the same content hash. No separate macro is needed. A
`InferableBrand!` macro (analogous to `Kind!`) resolves the trait name from
the signature:

```rust
// Resolves to InferableBrand_cdc7cd43dac7585f
InferableBrand!(type Of<'a, A: 'a>: 'a;)
```
...
pub trait InferableBrand_cdc7cd43dac7585f {
...
impl<'a, A> InferableBrand_cdc7cd43dac7585f for Thunk<'a, A> {
    type Brand = ThunkBrand;
}
// etc.
```

### Blanket `InferableBrand` for References

A blanket impl enables brand inference for borrowed containers, which is
required for Ref dispatch:

```rust
impl<'a, T: InferableBrand_cdc7cd43dac7585f + ?Sized> InferableBrand_cdc7cd43dac7585f for &'a T {
    type Brand = T::Brand;
}
```
...
- **Multi-brand exclusion preserved:** `&Result<A, E>` does not implement
  `InferableBrand` because `Result<A, E>` does not, so the blanket's
  `T: InferableBrand` bound is unsatisfied.
...
The InferableBrand family enables brand inference for operations at every
...
implement `InferableBrand_266801a817966495` (the two-parameter Kind) even
though they cannot implement `InferableBrand_cdc7cd43dac7585f` (the
...
impl<A, E> InferableBrand_266801a817966495 for Result<A, E> {
...
The new `map` function uses `InferableBrand` to infer the brand. The `FA`
type parameter flows into both `InferableBrand` (to resolve Brand) and
...
pub fn map<'a, FA, A: 'a, B: 'a, Marker>(
    f: impl FunctorDispatch<'a, <FA as InferableBrand>::Brand, A, B, FA, Marker>,
    fa: FA,
) -> <<FA as InferableBrand>::Brand as Kind_cdc7cd43dac7585f>::Of<'a, B>
where
    FA: InferableBrand,
{
    f.dispatch(fa)
...
- **No GAT equality bound (preferred).** The constraint
  `<FA as InferableBrand>::Brand: Kind<Of<'a, A> = FA>` is likely unnecessary
...
- `InferableBrand` resolves the `Brand` parameter from `FA`.
...
via `InferableBrand`. A single `map` function handles both Val and Ref
dispatch with brand inference, with no separate function needed for Ref.

The blanket `impl InferableBrand for &T` is essential here: when `FA = &Vec<i32>`,
...
use the dispatch trait system, but `InferableBrand` can still infer Brand
...
works via the blanket `impl InferableBrand for &T`:
...
The macro generates `bind(expr, ...)` calls (which use `InferableBrand`
...
blanket `impl InferableBrand for &T`: `m_do!(ref { a <- lazy_a; ... })`
...
There is no `FA` to resolve `InferableBrand` from. Return-type inference
...
return type back through `InferableBrand` to determine the Brand. This
...
## Types That CAN Implement `InferableBrand`
...
(no `InferableBrand` needed): `Free`, `Trampoline`, `TryTrampoline`,
`TrySendThunk`, `Additive`, `Multiplicative`, `Conjunctive`, `Disjunctive`,
`Dual`, `First`, `Last`, `Endomorphism`, `Endofunction`.

## Types That CANNOT Implement Arity-1 `InferableBrand`

These types are reachable through multiple arity-1 brands, making `map`
inference ambiguous. They cannot implement `InferableBrand_cdc7cd43dac7585f`
...
However, all of these types CAN implement the arity-2 `InferableBrand`
(`InferableBrand_266801a817966495`) because each has exactly one bifunctor
...
The `InferableBrand` trait proposed here is analogous to `TypeApp`, but
...
`InferableBrand` impls for library-defined types (`Identity`, `Thunk`, etc.)
...
Users cannot implement `InferableBrand` for their own types unless the trait
...
also want to implement `InferableBrand`. This is valid under orphan rules as
long as the user's type is local.

### Error Messages for Types Without `InferableBrand`

When a user writes `map(f, Ok(5))`, the compiler says
"`InferableBrand` is not implemented for `Result<i32, _>`." This is correct
...
pub trait InferableBrand { ... }
```

### The GAT Equality Bound (Verified, Likely Unnecessary)

The constraint `<FA as InferableBrand>::Brand: Kind_cdc7cd43dac7585f<Of<'a, A> = FA>`
...
`InferableBrand` resolves at concrete types only. Generic HKT functions
...
// Fails: generic FA, compiler cannot resolve InferableBrand
fn generic_map<FA: InferableBrand>(fa: FA) { ... }
...
`Lazy<'a, A, Config>`). The `InferableBrand` impl must be generic over the
lifetime:

```rust
impl<'a, A: 'a> InferableBrand for Thunk<'a, A> {
    type Brand = ThunkBrand;
}
```
...
the Kind signature hash. It can generate the corresponding `InferableBrand`
impl automatically:
...
// impl InferableBrand_cdc7cd43dac7585f for Option<A> { type Brand = OptionBrand; }
```

The macro uses the same content hash to select the InferableBrand trait
variant, so the Kind and InferableBrand impls are always paired correctly.

Types with multiple brands must opt out via a `#[no_inferable_brand]`
attribute:
...
`#[no_inferable_brand]` on a multi-brand type results in a conflicting impl
compiler error, which is a clear signal.

Note that a bifunctor type like `Result` would have `#[no_inferable_brand]`
on its partially-applied `impl_kind!` entries (which use the one-parameter
Kind) but would have a hand-written or separately generated InferableBrand
...
// Two-parameter: unambiguous, generate InferableBrand
impl_kind! {
    for ResultBrand {
        type Of<'a, A: 'a, B: 'a>: 'a = Result<A, B>;
    }
}
// Generates: impl InferableBrand_266801a817966495 for Result<A, B> { type Brand = ResultBrand; }
```
...
generic parameters on the `InferableBrand` impl. For simple cases
...
automatically skip InferableBrand generation for those invocations.
...
InferableBrand generation (the "primary" type is ambiguous) and suggest
`#[no_inferable_brand]`.

**Inventory of `impl_kind!` invocations:**

- ~15 invocations generate InferableBrand normally (core container types).
- 10 invocations need `#[no_inferable_brand]` (multi-brand arity-1 types:
...
2. The blanket `impl InferableBrand for &T` composes correctly with the Ref
...
1. **Inference-based functions get the clean names.**
   `map`, `bind`, `apply`, etc. use `InferableBrand` for inference. The
...
2. **`InferableBrand` on the concrete type, not a `TypeApp`-style witness.**
   ...
   supports multiple `TypeApp` impls per type (one per TCon). `InferableBrand`
...
3. **Default generation via `impl_kind!`, opt-out for exceptions.**
   `impl_kind!` generates `InferableBrand` by default. Types with multiple
   brands use `#[no_inferable_brand]` to opt out. The conflicting impl error
...
5. **`pure` keeps its current signature (confirmed by POC).**
   ...
   resolve `InferableBrand` from. Return-type inference was tested and
...
6. **`compose_kleisli` variants keep their current signatures.**
   ...
   `InferableBrand` from. These functions are not renamed.

7. **Blanket `impl InferableBrand for &T` is part of the core design.**
...
8. **Per-function rename at the re-export level, then tiered inference.**
   ...
   Each wrapper constrains `FA: InferableBrand` and delegates to the
   ...
   - **Tier 4 (bifunctor arity-2):** `bimap` and bifunctor traversals
     use the arity-2 `InferableBrand` trait. `bimap` is in Tier 1 for
     the dispatch wrapper; the arity-2 InferableBrand impl is a
     separate step.
...
Changes to trait bounds, `InferableBrand` impls, and dispatch signatures
...
1. **[Done] POC: Verify the GAT equality bound works.** Confirmed that
   `InferableBrand` + `FunctorDispatch` composes correctly. Inference works
...
2. **[Done] Define `InferableBrand` trait** in
   `fp-library/src/classes/default_brand.rs`. Includes blanket
   `impl InferableBrand for &T` and `#[diagnostic::on_unimplemented]`.

3. **[Done] Implement `InferableBrand` for core types.** Option, Vec,
   Identity, Thunk, SendThunk, CatList, (A,).

4. **[Done] Implement `InferableBrand` for parameterized types.** Lazy,
...
6. **[Done] Tier 1: Full inference for dispatch functions.** 15 inference
   ...
   `bimap` deferred to Tier 4 (needs arity-2 InferableBrand).
...
7a. **[Done] Fix closureless dispatch files.** Standardize the 6 closureless
...
8. **[Done] Extend `trait_kind!` to also generate `InferableBrand_{hash}` traits.**
   ...
   and `InferableBrand!` macro implementations (hash computation,
...
macros generate `_explicit` variants. `InferableBrand` is implemented
...
5. **`bimap` and bi-fold/traverse deferred to Tier 4.** These use the
   ...
   `InferableBrand` trait for the two-parameter Kind. This is step 10.
