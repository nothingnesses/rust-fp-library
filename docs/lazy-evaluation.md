...
2. **`Thunk` vs `SendThunk`**: `Thunk` wraps `Box<dyn FnOnce() -> A + 'a>` and is `!Send`. `SendThunk` wraps `Box<dyn FnOnce() -> A + Send + 'a>` and can cross thread boundaries. Use `SendThunk` when you need truly lazy `memoize_arc()` (converting to `ArcLazy` without eagerly evaluating), or when building deferred computation chains that will be consumed on another thread.
...
fn main() {
    ...
    // Wraps a cheap reference to the cached result.
    let view = TryThunk::new({
        let result = result.clone();
        move || result.evaluate().cloned()
    });

    assert_eq!(view.evaluate(), Ok(50));
}
```
