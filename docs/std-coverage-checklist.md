...
| **`Filterable`** / **`Compactable`** | `Iterator::filter`, `Option::filter`, `Vec::retain` | Abstractions for data structures that can filter out elements or compact `Option`s inside them.              | `fp-library/src/classes/filterable.rs`, `fp-library/src/classes/compactable.rs` |
...
| **`SendEndofunction`** | `Fn(A) -> A + Send + Sync`   | Thread-safe wrapper for endofunctions.                                         | `fp-library/src/types/send_endofunction.rs` |
...
| **`ref_counted_new`** | `Rc::new`, `Arc::new` | Wraps a value in a clonable pointer.                    | `fp-library/src/functions.rs` |
