...
//! effects subsystem consumes: [`Coproduct`], [`CNil`], plus the
//! [`CoprodInjector`] / [`CoprodUninjector`] / [`CoproductSubsetter`] /
//! [`CoproductEmbedder`] / [`CoproductSelector`] / [`CoproductTaker`]
//! traits, and the [`Here`] / [`There`] / [`HCons`] / [`HNil`] index
//! types.
//!
//! ## Bridging to fp-library's Brand system
//!
//! No newtype wrapper around [`Coproduct`] is needed at this layer. The
//! Brand-level integration lives at `crate::brands::CoproductBrand`: a
//! local Brand parameterised by head and tail Brand types, with
//! `Of<'a, A>` resolving to `Coproduct<H::Of<'a, A>, T::Of<'a, A>>`.
//! Because `Kind_*` is fp-library's trait, the Brand impl satisfies the
//! orphan rules (own-trait + local-Brand-type) without any wrapper.
//!
//! ## Frunk trait names
//!
...
//! `CoproductEmbedder` (embed into a superset). HList-style names
//! ("Plucker" / "Sculptor" / "Embedder") are common in upstream
//! literature; the Coproduct-style names are what frunk_core actually
