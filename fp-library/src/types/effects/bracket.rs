...
mod inner {
	...
	where
		...
		Bracket {
			/// The acquire program, stored as a unit-arg B-thunk
			/// (`<P as RefCountedPointer>::Of<'a, dyn 'a + Fn(()) -> Free<Sub, A>>`,
			...
			acquire: <P as RefCountedPointer>::Of<'a, dyn 'a + Fn(()) -> Free<Sub, A>>,
			/// The body closure
			/// (`<P as RefCountedPointer>::Of<'a, dyn 'a + Fn(<P>::Of<'a, A>) -> Free<Sub, (A, B)>>`,
			...
			body: <P as RefCountedPointer>::Of<
				'a,
				dyn 'a + Fn(<P as RefCountedPointer>::Of<'a, A>) -> Free<Sub, (A, B)>,
			>,
			/// The release closure
			/// (`<P as RefCountedPointer>::Of<'a, dyn 'a + Fn(<P>::Of<'a, A>) -> Free<Sub, ()>>`,
			...
			release: <P as RefCountedPointer>::Of<
				'a,
				dyn 'a + Fn(<P as RefCountedPointer>::Of<'a, A>) -> Free<Sub, ()>,
			>,
		},
	}
	...
	where
		...
		Bracket {
			/// The acquire program, stored as a unit-arg B-thunk
			/// (`<P as SendRefCountedPointer>::Of<'a, dyn 'a + Fn(()) -> Free<Sub, A> + Send + Sync>`,
			/// multi-shot via [`Fn`]; clone via Arc-bump; thread-safe).
			acquire:
				<P as SendRefCountedPointer>::Of<'a, dyn 'a + Fn(()) -> Free<Sub, A> + Send + Sync>,
			/// The body closure
			/// (`<P as SendRefCountedPointer>::Of<'a, dyn 'a + Fn(<P>::Of<'a, A>) -> Free<Sub, (A, B)> + Send + Sync>`,
			/// multi-shot via [`Fn`]; clone via Arc-bump; thread-safe).
			#[expect(
				clippy::type_complexity,
				reason = "Bracket cells store closures returning Free programs derived from the substrate brand Sub; the nested GAT and Free projections cannot be aliased without losing the per-pointer-brand structure ToDynFnOnce / ToDynCloneFn / ToDynSendFn dispatch over."
			...
			body: <P as SendRefCountedPointer>::Of<
				'a,
				dyn 'a
					+ Fn(<P as SendRefCountedPointer>::Of<'a, A>) -> Free<Sub, (A, B)>
					+ Send
					+ Sync,
			>,
			/// The release closure
			/// (`<P as SendRefCountedPointer>::Of<'a, dyn 'a + Fn(<P>::Of<'a, A>) -> Free<Sub, ()> + Send + Sync>`,
			/// multi-shot via [`Fn`]; clone via Arc-bump; thread-safe).
			#[expect(
				clippy::type_complexity,
				reason = "Bracket cells store closures returning Free programs derived from the substrate brand Sub; the nested GAT and Free projections cannot be aliased without losing the per-pointer-brand structure ToDynFnOnce / ToDynCloneFn / ToDynSendFn dispatch over."
			...
			release: <P as SendRefCountedPointer>::Of<
				'a,
				dyn 'a + Fn(<P as SendRefCountedPointer>::Of<'a, A>) -> Free<Sub, ()> + Send + Sync,
			>,
		},
	}

	impl_kind! {
		impl<P: ToDynSendFn, Sub: WrapDrop + 'static, A: Send + Sync + 'static, B: Send + Sync + 'static> for SendBracketBrand<P, Sub, A, B> {
		...
		B: Send + Sync + 'static,
	{
		...
		/// };
		///
		/// let original: SendBracket<'static, ArcBrand, ThunkBrand, i32, i32> = SendBracket::Bracket {
		/// 	acquire: <ArcBrand as ToDynSendFn>::new(|_: ()| Free::<ThunkBrand, _>::pure(7)),
		...
		B: Send + Sync + 'static,
	{
		...
		/// };
		///
		/// let bracket: SendBracket<'static, ArcBrand, ThunkBrand, i32, i32> = SendBracket::Bracket {
		/// 	acquire: <ArcBrand as ToDynSendFn>::new(|_: ()| Free::<ThunkBrand, _>::pure(7)),
		/// 	body: <ArcBrand as ToDynSendFn>::new(|_a: std::sync::Arc<i32>| {
		/// 		Free::<ThunkBrand, _>::pure((7, 42))
		/// 	}),
		...
		/// 	}),
		/// };
		/// let mapped = <SendBracketBrand<ArcBrand, ThunkBrand, i32, i32> as SendFunctor>::send_map(
		/// 	|x: i32| x + 1,
		/// 	bracket,
		/// );
		...
		B: Send + Sync + 'static,
	{
		...
		/// };
		/// assert_eq!(
		/// 	<SendBracketBrand<ArcBrand, ThunkBrand, i32, i32> as WrapDrop>::drop::<i32>(bracket),
		/// 	None
		/// );
		/// ```
		fn drop<'a, X: 'a>(
