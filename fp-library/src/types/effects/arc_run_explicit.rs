...
mod inner {
		...
		S: WrapDrop + SendFunctor + 'static,
	{
		...
		/// Interprets this `ArcRunExplicit` program with a state value
		...
		/// 	handlers! {
		/// 		IdentityBrand: move |op: Identity<ArcRunExplicit<'static, FirstRow, Scoped, i32>>| {
		...
		/// Stateful variant of
		/// [`interpret_rec`](ArcRunExplicit::interpret_rec). See
		...
		/// 	handlers! {
		/// 		IdentityBrand: move |op: Identity<Option<ArcRunExplicit<'static, FirstRow, Scoped, i32>>>| {
		/// 			*counter_for_handler.lock().unwrap() += 1;
		/// 			op.0
		/// 		},
		/// 	},
