...
pub(crate) mod inner {
	...
	pub(crate) struct RcRunExplicitActionSuppliedScopedContinuation<'a, R, S, Action, Final, K>
	where
		R: WrapDrop + Functor + 'static,
		S: WrapDrop + Functor + 'static,
		Action: Clone + 'a,
	...
	impl<'a, R, S, Action, Final, K> IntoScopedBoundaryParts<'a>
		for RcRunExplicitBoundary<'a, R, S, Action, Final, K>
	where
		R: WrapDrop + Functor + 'static,
		S: WrapDrop + Functor + 'static,
		Action: Clone + 'a,
		...
		K: Fn(Action) -> RcRunExplicit<'a, R, S, Final> + 'a,
	{
		type Carrier = RcRunExplicitActionSuppliedScopedContinuation<'a, R, S, Action, Final, K>;
		...
		>): Clone,
	{
		...
		fn resume_rc_with_supplied_action(
			...
			supplied_action().bind(move |action_value: Action| -> RcRunExplicit<'a, R, S, Final> {
				outer(action_value)
			})
		}
	}
