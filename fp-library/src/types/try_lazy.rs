...
mod inner {
	...
	impl<A, E> From<TryThunk<A, E>> for TryLazy<A, E, RcLazyConfig> {
		#[document_signature]
		#[document_parameters("The fallible thunk to convert.")]
		fn from(eval: TryThunk<A, E>) -> Self {
	...
	impl<A, E> Deferrable for TryLazy<A, E, RcLazyConfig>
	where
		A: Clone,
		E: Clone,
	{
	...
	impl_kind! {
		impl<E, Config: LazyConfig> for TryLazyBrand<E, Config> {
			#[document_default]
	...
	impl<A, E> SendDeferrable for TryLazy<A, E, ArcLazyConfig>
	where
		A: Clone + Send + Sync,
		E: Clone + Send + Sync,
	{
