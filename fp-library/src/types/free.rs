...
mod tests {
	...
	fn test_free_monad_associativity() {
		...
		let rhs = Free::<ThunkBrand, _>::pure(5).bind(|x| f(x).bind(g)).evaluate();
	...
	fn test_free_monad_associativity_with_effects() {
		...
		let rhs = Free::<ThunkBrand, _>::pure(7).bind(|x| f(x).bind(g)).evaluate();
	...
	fn test_free_monad_associativity_three_functions() {
		...
		// m >>= (\x -> f(x) >>= (\y -> g(y) >>= h))
		let rhs = Free::<ThunkBrand, _>::pure(10).bind(|x| f(x).bind(|y| g(y).bind(h))).evaluate();
