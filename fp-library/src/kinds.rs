...
//! 2. An **`InferableBrand_{hash}`** trait implemented by concrete types to map back
//!    to their canonical brand (the reverse mapping), enabling brand inference.
//!
...
//!
//! Traits providing the reverse mapping (concrete type -> brand) for brand inference.
//!
//! Each `InferableBrand_{hash}` trait has:
//! * An associated `type Brand: Kind_{hash}` identifying the canonical brand.
//! * A `#[diagnostic::on_unimplemented]` attribute for actionable error messages.
//! * A blanket `impl for &T` enabling inference for borrowed containers.
//!
//! Only types with a single unambiguous brand at a given arity implement the
//! corresponding `InferableBrand` trait. Types with multiple brands (e.g.,
//! `Result` at arity 1) do not, and require `_explicit` function variants.
//! `InferableBrand` impls are auto-generated by [`impl_kind!`](fp_macros::impl_kind)
//! unless `#[no_inferable_brand]` is specified.
//!
//! # Trait Name Examples
//!
...
//! `InferableBrand_cdc7cd43dac7585f` with `type Brand = OptionBrand`.
