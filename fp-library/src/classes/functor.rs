...
mod inner {
	...
	///
	/// // Identity: map(identity, fa) = fa
	/// assert_eq!(map::<OptionBrand, _, _, _>(identity, Some(5)), Some(5));
	/// assert_eq!(map::<OptionBrand, _, _, _>(identity, None::<i32>), None);
	///
	/// // Composition: map(compose(f, g), fa) = map(f, map(g, fa))
	/// let f = |x: i32| x + 1;
	/// let g = |x: i32| x * 2;
	/// assert_eq!(
	/// 	map::<OptionBrand, _, _, _>(compose(f, g), Some(5)),
	/// 	map::<OptionBrand, _, _, _>(f, map::<OptionBrand, _, _, _>(g, Some(5))),
	/// );
	/// ```
	///
	...
	///
	/// // Identity: map(identity, fa) = fa
	/// assert_eq!(map::<VecBrand, _, _, _>(identity, vec![1, 2, 3]), vec![1, 2, 3]);
	///
	/// // Composition: map(compose(f, g), fa) = map(f, map(g, fa))
	/// let f = |x: i32| x + 1;
	/// let g = |x: i32| x * 2;
	/// assert_eq!(
	/// 	map::<VecBrand, _, _, _>(compose(f, g), vec![1, 2, 3]),
	/// 	map::<VecBrand, _, _, _>(f, map::<VecBrand, _, _, _>(g, vec![1, 2, 3])),
	/// );
	/// ```
