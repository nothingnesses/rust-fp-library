...
pub trait Compactable: Kind_cdc7cd43dac7585f {
	fn compact<'a, A: 'a>(
		fa: Apply!(<Self as trait { type Of<'a, T: 'a>: 'a; }>::Of<
			'a,
			Apply!(<OptionBrand as trait { type Of<'a, T: 'a>: 'a; }>::Of<'a, A>),
		>)
	) -> Apply!(<Self as trait { type Of<'a, T: 'a>: 'a; }>::Of<'a, A>);

	fn separate<'a, E: 'a, O: 'a>(
		fa: Apply!(<Self as trait { type Of<'a, T: 'a>: 'a; }>::Of<'a, Result<O, E>>)
	) -> Pair<
...
pub fn compact<'a, Brand: Compactable, A: 'a>(
	fa: Apply!(<Brand as trait { type Of<'a, T: 'a>: 'a; }>::Of<
		'a,
		Apply!(<OptionBrand as trait { type Of<'a, T: 'a>: 'a; }>::Of<'a, A>),
	>)
...
pub fn separate<'a, Brand: Compactable, E: 'a, O: 'a>(
	fa: Apply!(<Brand as trait { type Of<'a, T: 'a>: 'a; }>::Of<'a, Result<O, E>>)
) -> Pair<
