...
mod inner {
	...
	pub trait RefFoldable {
		...
		#[document_examples(
			skip_call_check,
			reason = "The reference-mode class method `ref_fold_map` is demonstrated through the public explicit wrapper, whose name omits the internal `ref_` prefix and selects reference dispatch from the borrowed argument."
		)]
		///
		/// ```
		/// use fp_library::{
		...
		#[document_examples(
			skip_call_check,
			reason = "The reference-mode class method `ref_fold_right` is demonstrated through the public explicit wrapper, whose name omits the internal `ref_` prefix and selects reference dispatch from the borrowed argument."
		)]
		///
		/// ```
		/// use fp_library::{
		...
		#[document_examples(
			skip_call_check,
			reason = "The reference-mode class method `ref_fold_left` is demonstrated through the public explicit wrapper, whose name omits the internal `ref_` prefix and selects reference dispatch from the borrowed argument."
		)]
		///
		/// ```
		/// use fp_library::{
