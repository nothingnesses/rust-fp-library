...
mod inner {
	...
	#[document_examples(
		skip_call_check,
		reason = "The reference-mode class method `ref_apply_first` is demonstrated through the public explicit wrapper, whose name omits the internal `ref_` prefix and selects reference dispatch from the borrowed argument."
	)]
	///
	/// ```
	/// use fp_library::{
