...
mod inner {
	...
	/// assert_eq!(
	/// 	bimap_explicit::<ResultBrand, _, _, _, _, _, _>((|x: &i32| *x, |x: &i32| *x), &ok),
	/// 	ok,
	/// );
	/// assert_eq!(
	/// 	bimap_explicit::<ResultBrand, _, _, _, _, _, _>((|x: &i32| *x, |x: &i32| *x), &err),
	/// 	err,
	/// );
	///
	...
	/// 	bimap_explicit::<ResultBrand, _, _, _, _, _, _>(
	/// 		(f2, g2),
	/// 		&bimap_explicit::<ResultBrand, _, _, _, _, _, _>((f1, g1), &ok),
	/// 	),
	/// );
	/// ```
		...
		for BifunctorFirstAppliedBrand<Brand, A>
	{
		...
		///
		/// let x = Result::<i32, i32>::Ok(5);
		/// let y = map_explicit::<BifunctorFirstAppliedBrand<ResultBrand, i32>, _, _, _, _>(
		/// 	|s: &i32| *s * 2,
		/// 	&x,
		/// );
		/// assert_eq!(y, Ok(10));
		...
		for BifunctorSecondAppliedBrand<Brand, B>
	{
		...
		/// let y = map_explicit::<BifunctorSecondAppliedBrand<ResultBrand, i32>, _, _, _, _>(
		/// 	|e: &i32| *e * 2,
		/// 	&x,
		/// );
		/// assert_eq!(y, Err(10));
...
mod tests {
	...
	fn prop_ref_bifunctor_identity(
		...
		bimap_explicit::<ResultBrand, _, _, _, _, _, _>((|x: &i32| *x, |x: &i32| *x), &ok) == ok
			&& bimap_explicit::<ResultBrand, _, _, _, _, _, _>((|x: &i32| *x, |x: &i32| *x), &err)
				== err
	}
	...
	fn prop_ref_bifunctor_composition(
		...
		let composed_ok = bimap_explicit::<ResultBrand, _, _, _, _, _, _>(
			(|x: &i32| f2(&f1(x)), |x: &i32| g2(&g1(x))),
			&ok,
		);
		let sequential_ok = bimap_explicit::<ResultBrand, _, _, _, _, _, _>(
			(f2, g2),
			&bimap_explicit::<ResultBrand, _, _, _, _, _, _>((f1, g1), &ok),
		);

		let composed_err = bimap_explicit::<ResultBrand, _, _, _, _, _, _>(
			(|x: &i32| f2(&f1(x)), |x: &i32| g2(&g1(x))),
			&err,
		);
		let sequential_err = bimap_explicit::<ResultBrand, _, _, _, _, _, _>(
			(f2, g2),
			&bimap_explicit::<ResultBrand, _, _, _, _, _, _>((f1, g1), &err),
		);
