//! Dispatch for [`FilterableWithIndex::partition_with_index`](crate::classes::FilterableWithIndex::partition_with_index) and
//! [`RefFilterableWithIndex::ref_partition_with_index`](crate::classes::RefFilterableWithIndex::ref_partition_with_index).
//!
//! Provides the [`PartitionWithIndexDispatch`] trait and a unified [`partition_with_index`] free function
...
pub(crate) mod inner {
	...
	// -- Val: Fn(Brand::Index, A) -> bool -> FilterableWithIndex::partition_with_index --
	...
	// -- Ref: Fn(Brand::Index, &A) -> bool -> RefFilterableWithIndex::ref_partition_with_index --
	...
	///   container is owned, dispatches to [`FilterableWithIndex::partition_with_index`].
	/// - If the closure takes references (`Fn(Index, &A) -> bool`) and the
	///   container is borrowed (`&fa`), dispatches to
	///   [`RefFilterableWithIndex::ref_partition_with_index`].
	///
