...
mod inner {
	...
	///
	/// assert_eq!(
	/// 	bind::<VecBrand, _, _, _, _>(alt::<VecBrand, _>(x.clone(), y.clone()), f),
	/// 	alt::<VecBrand, _>(bind::<VecBrand, _, _, _, _>(x, f), bind::<VecBrand, _, _, _, _>(y, f)),
	/// );
	/// ```
	///
...
mod tests {
	...
	fn distributivity_cat_list(
		...
		bind::<CatListBrand, _, _, _, _>(alt::<CatListBrand, _>(x.clone(), y.clone()), f)
			== alt::<CatListBrand, _>(
