...
pub(crate) mod inner {
	...
	pub trait MapSecondDispatch<'a, Brand: Kind_266801a817966495, A: 'a, B: 'a, C: 'a, FA, Marker> {
		...
		#[document_examples(
			skip_call_check,
			reason = "This `map_second` dispatch hook is documented through the public inference or explicit wrapper; direct calls to `dispatch` require constructing the internal dispatch receiver and marker mode, so the example uses the supported public API instead."
		)]
		///
		/// ```
		/// use fp_library::{
