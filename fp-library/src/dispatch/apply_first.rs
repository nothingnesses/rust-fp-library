...
pub(crate) mod inner {
	...
	pub trait ApplyFirstDispatch<'a, Brand: Kind_cdc7cd43dac7585f, A: 'a, B: 'a, Marker> {
		...
		#[document_examples(
			skip_call_check,
			reason = "This `apply_first` dispatch hook is documented through the public inference or explicit wrapper; direct calls to `dispatch` require constructing the internal dispatch receiver and marker mode, so the example uses the supported public API instead."
		)]
		///
		/// ```
		/// use fp_library::{
