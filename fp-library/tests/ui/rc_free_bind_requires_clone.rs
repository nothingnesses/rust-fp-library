// Verifies that the Rc-store `Free::bind` requires `A: Clone`.
//
...
// shared. The `A: Clone` bound on `bind` enforces that the fallback is
// always available.
...
fn main() {
	...
	let _bound = program.bind(|x: NotClone| Free::pure(NotClone(x.0 + 1)));
}
