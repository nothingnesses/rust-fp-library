...
fn handle_rc_handled_local(program: RcHandledLocalProg) -> i32 {
	program.handle(
		...
		scoped_handlers! {
			LocalBrand<RcBrand, i32>: local_handler::<_, RcHandledLocalFirstRowMinusReader, _>(),
		},
	)
}
...
fn handle_arc_handled_local(program: ArcHandledLocalProg) -> i32 {
	program.handle(
		...
		scoped_handlers! {
			SendLocalBrand<ArcBrand, i32>: local_handler::<_, ArcHandledLocalFirstRowMinusReader, _>(),
		},
	)
}
...
fn run_explicit_t1_local_boundary_uses_modified_environment() {
	...
	let result = prog.handle(
		...
		scoped_handlers! {
			BoxLocalBrand<BoxBrand, i32>: local_handler::<_, RxFirstRowMinusReader, _>(),
		},
	);
...
fn rc_run_explicit_t1_local_boundary_uses_modified_environment() {
	...
	let result = prog.handle(
		...
		scoped_handlers! {
			LocalBrand<RcBrand, i32>: local_handler::<_, RcxFirstRowMinusReader, _>(),
		},
	);
...
fn arc_run_explicit_t1_local_boundary_uses_modified_environment() {
	...
	let result = prog.handle(
		...
		scoped_handlers! {
			SendLocalBrand<ArcBrand, i32>: local_handler::<_, AcxFirstRowMinusReader, _>(),
		},
	);
