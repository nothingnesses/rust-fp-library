// Production-style POC for a `InferableBrand` trait enabling closure-directed
...
// `InferableBrand<Brand, A> for FA` trait that admits multiple impls per
...
//
// 1. Coherence of a blanket `impl<FA: InferableBrand> InferableBrand<FA::Brand, A> for FA`
//    combined with direct `InferableBrand` impls on multi-brand types. (Finding:
//    fails with E0119; see the "direct impls for every brand" comment
//    below.)
// 2. Lifetime-generic InferableBrand trait with a lifetime-carrying GAT
//    (`type Out<B: 'a>: 'a`) compiling and resolving at call sites.
// 3. Return-type computation: whether a function returning
//    `<FA as InferableBrand<'a, Brand, A>>::Out<B>` normalises to the expected
//    concrete type at call sites.
// 4. Composition with borrowed containers via a `&T` blanket for InferableBrand
...
// goal is to exercise Rust's trait selection on the InferableBrand signature
...
// -------------------------------------------------------------------------
// InferableBrand_cdc7cd43dac7585f: the reverse-direction trait keyed on (FA, Brand, A)
// -------------------------------------------------------------------------
//
...
// -------------------------------------------------------------------------
// Direct InferableBrand impls for every brand.
// -------------------------------------------------------------------------
//
// An earlier iteration of this POC tried a blanket:
//
//   impl<FA: InferableBrand> InferableBrand<FA::Brand, A> for FA
//
...
// and InferableBrand collide for the same reason.
//
// The blanket approach therefore does NOT work on stable rustc. This
// POC gives every brand (single- and multi-) a direct InferableBrand impl.
// Coherence is trivially safe because each impl is keyed on a distinct
// Brand parameter.

impl<'a, A: 'a> InferableBrand_cdc7cd43dac7585f<'a, fp_library::brands::OptionBrand, A>
	for Option<A>
{
	type Out<B: 'a> = Option<B>;
}

impl<'a, A: 'a> InferableBrand_cdc7cd43dac7585f<'a, fp_library::brands::VecBrand, A> for Vec<A> {
	type Out<B: 'a> = Vec<B>;
}
...
// -------------------------------------------------------------------------
//
// With no InferableBrand-based InferableBrand blanket in play, this single `&T`
// blanket is unopposed: trait selection has exactly one path from &Option,
// &Result, etc. to their InferableBrand impls.
...
where
	...
	Brand: Kind_cdc7cd43dac7585f,
{
	type Out<B: 'a> = <T as InferableBrand_cdc7cd43dac7585f<'a, Brand, A>>::Out<B>;
}

// -------------------------------------------------------------------------
// A map-style function using InferableBrand as the dispatch trait.
// -------------------------------------------------------------------------
//
// Simplified shape: no FunctorDispatch, no Val/Ref marker. The return type
// exercises Blocker 3 (does `<FA as InferableBrand<...>>::Out<B>` normalise at call
// sites?).
//
...
where
	Brand: Kind_cdc7cd43dac7585f,
	Self: InferableBrand_cdc7cd43dac7585f<'a, Brand, A>, {
	fn dispatch_map(
		self,
		f: impl FnMut(A) -> B,
	) -> Self::Out<B>;
...
pub fn slot_map<'a, FA, A: 'a, B: 'a, Brand>(
	f: impl FnMut(A) -> B,
	fa: FA,
) -> <FA as InferableBrand_cdc7cd43dac7585f<'a, Brand, A>>::Out<B>
where
	Brand: Kind_cdc7cd43dac7585f,
	FA: InferableBrand_cdc7cd43dac7585f<'a, Brand, A> + MapDispatch<'a, Brand, A, B>, {
	fa.dispatch_map(f)
}
...
// MapDispatch for Lazy is omitted deliberately. The lifetime-bearing InferableBrand
...
// concerned with. The lifetime-bearing test below validates the InferableBrand
...
// -------------------------------------------------------------------------
//
// Validates that the blanket impl (InferableBrand -> InferableBrand) is reachable by
// trait selection and that `InferableBrand::Out<B>` normalises to the expected
// concrete type.
...
// Lifetime-bearing type: validates Blocker 2 at the type level. The InferableBrand
...
fn lifetime_bearing_type_slot_resolution() {
	...
	fn rebind_lazy<'a>(
		x: <RcLazy<'a, i32> as InferableBrand_cdc7cd43dac7585f<
			'a,
...
// types with InferableBrand) and direct InferableBrand impls on multi-brand
...
// checks the InferableBrand resolution path only, not full Val/Ref production
...
fn annotation_matrix_single_brand_no_annotation() {
	...
	// Option<i32> -> A = i32 is forced by InferableBrand.
...
//     //   `Result<i32, i32>: InferableBrand_cdc7cd43dac7585f<'_, _, i32>` found
// }
...
// InferableBrand::Out<B> into the target shape?

#[test]
fn return_type_flows_into_match() {
	// Forces the return type to match Result<i32, String>, which must
	// normalise from
	// <Result<i32, String> as InferableBrand<'_, ResultErrAppliedBrand<String>, i32>>::Out<i32>.
...
fn return_type_chains_with_further_slot_map() {
	...
	// normalise in a position that's both a call argument and a InferableBrand input.
