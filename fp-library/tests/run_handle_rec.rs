...
// (`handle_rec` / `run_rec`) on all six Run wrappers. Each
...
// Arc/Mutex for the Arc family) directly on `handle_rec`. The
...
#[test]
fn run_handle_rec_threads_state() {
...
#[test]
fn rc_run_handle_rec_threads_state() {
	let counter: Rc<RefCell<i32>> = Rc::new(RefCell::new(0));
	let counter_for_handler = Rc::clone(&counter);
...
#[test]
fn arc_run_handle_rec_threads_state_via_mutex() {
...
#[test]
fn run_explicit_handle_rec_thunk() {
...
#[test]
fn run_explicit_handle_rec_option() {
	let prog: RunExplicit<'static, RunRow, CNilBrand, i32> =
...
#[test]
fn run_explicit_handle_rec_threads_state() {
	let counter: Rc<RefCell<i32>> = Rc::new(RefCell::new(0));
	let counter_for_handler = Rc::clone(&counter);
	let prog: RunExplicit<'static, RunRow, CNilBrand, i32> =
		RunExplicit::lift::<IdentityBrand, _>(Identity(7));
...
#[test]
fn rc_run_explicit_handle_rec_thunk() {
...
#[test]
fn rc_run_explicit_handle_rec_option() {
	let prog: RcRunExplicit<'static, RcRunRow, CNilBrand, i32> =
...
#[test]
fn arc_run_explicit_handle_rec_option() {
...
#[test]
fn arc_run_explicit_handle_rec_threads_state() {
	let counter: Arc<Mutex<i32>> = Arc::new(Mutex::new(0));
	let counter_for_handler = Arc::clone(&counter);
	let prog: ArcRunExplicit<'static, ArcRunRow, CNilBrand, i32> =
