...
- **Run effects subsystem** (experimental; the effects API is unstable and may change between releases): Added six `Run` wrappers (`Run`, `RcRun`, `ArcRun`, `RunExplicit`, `RcRunExplicit`, `ArcRunExplicit`), `Node`, `VariantF`, `ScopedCoproduct`, first-order and scoped handler-list carriers, and natural-transformation builders (`nt`, `scoped_nt`).
- **Standard first-order effects**: Added `State`, `Reader`, `Except`, `Writer`, `Choose`, `Empty`, `Coroutine`, `Fail`, `Fresh`, `Input`, `KVStore`, `Log`, and `Output` effect types and smart constructors across the applicable Run wrappers.
- **Async effect interpretation**: Added the `Await` future base-lift effect (`AwaitBrand`) with the `Run::await_future` constructor and the `Run::run_async` driver, which awaits embedded futures and interprets a default `Run` program as a runtime-agnostic future. The await effect may sit at any position in the first-order row.
