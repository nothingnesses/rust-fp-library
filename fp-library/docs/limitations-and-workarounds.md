...
The `ArcFreeExplicit` substrate additionally has a Send-aware by-value classification (the `Send*` parallel of the by-value table above). The substrate delegates through `F: SendFunctor`; the brand-level Send-aware coverage is still limited because the per-`A` `Clone` cascade on `bind`'s where-clause is intrinsic to the `Arc<Inner>` data shape and remains the binding constraint:
...
The inherent `ArcFreeExplicit::map` method is the concrete-type counterpart to the unreachable brand-level `SendFunctor::send_map`. `SendLift::send_lift2` (and the rest of the applicative cascade) has no inherent counterpart on `ArcFreeExplicit`; callers can compose `bind` with `pure` directly when they need lifted binary application.

The parallel `ArcRunExplicitBrand` SendRef-family coverage has the same limitation. The natural reference shape (delegate `SendRefFunctor::send_ref_map` to the wrapper's inherent `ArcRunExplicit::ref_map`) is rejected by rustc with four blockers, three of which are unresolvable in stable Rust:
...
complex continuation-carrier and boundary-dispatch protocols are private
