...
Sub-step 9i lands `SendRefFunctor` (and other Send-aware Ref-family traits) on the parallel `ArcRunExplicitBrand` via inherent-method delegation rather than substrate-brand delegation, sidestepping the per-`A` HRTB at the wrapper level. The analogous strategy does not apply to `ArcFreeExplicitBrand` because brand-level consumers expect HKT polymorphism rather than concrete-type dispatch; instead, sub-step 9d landed inherent `ArcFreeExplicit::map` as a concrete-type counterpart to the unreachable brand-level `SendFunctor::send_map`. `SendLift::send_lift2` (and the rest of the applicative cascade) has no inherent counterpart on `ArcFreeExplicit`; callers can compose `bind` with `pure` directly when they need lifted binary application.
