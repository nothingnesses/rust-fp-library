...
trait Kind_cdc7cd43dac7585f {        // Forward: Brand -> Type
    type Of<'a, A: 'a>: 'a;
}

trait InferableBrand_cdc7cd43dac7585f {  // Reverse: Type -> Brand
    type Brand: Kind_cdc7cd43dac7585f;
}

impl<T: InferableBrand_cdc7cd43dac7585f + ?Sized>
    InferableBrand_cdc7cd43dac7585f for &T  // Blanket ref impl
{
    type Brand = T::Brand;
}
```
...
impl Kind_cdc7... for OptionBrand {          // Forward
    type Of<'a, A: 'a>: 'a = Option<A>;
}

impl<A> InferableBrand_cdc7... for Option<A> {  // Reverse
