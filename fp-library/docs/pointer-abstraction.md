...
**Use-case:** single-shot continuation cells in effect types such as [`BoxState`](../src/types/effects/state.rs), [`BoxReader`](../src/types/effects/reader.rs), [`BoxChoose`](../src/types/effects/choose.rs), and default `Run` scoped-effect closure storage, where the program-tree's single-shot semantics make `FnOnce` the precise type and `Box<dyn Fn>` would over-promise multi-shot semantics. Multi-shot wrappers (`RcRun` / `ArcRun` and their `Explicit` siblings) keep using the cloneable `dyn Fn` paths via `ToDynCloneFn` / `ToDynSendFn`.
