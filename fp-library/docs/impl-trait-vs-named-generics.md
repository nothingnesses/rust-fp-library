...
- **Fewer type parameters** in turbofish: `map_explicit::<Brand, _, _, _>` vs `map_explicit::<Brand, _, _, _, _>`. With brand inference, the turbofish is eliminated entirely for unambiguous types: `map(f, x)`.
