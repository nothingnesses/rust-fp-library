...
- **Fewer type parameters** in turbofish: `explicit::map::<Brand, _, _>` vs `explicit::map::<Brand, _, _, _>`. With brand inference, the turbofish is eliminated entirely for unambiguous types: `map(f, x)`.
