...
//!
//! ### Type Parameters Only - [`extract_type_params`]
//!
...
//!
//! ### All Parameters - [`extract_all_params`]
//!
...
//! // Given: fn foo<'a, T, U, const N: usize>(x: &'a T) -> [U; N]
//! let generics: syn::Generics = /* ... */;
//! let all_params = extract_all_params(&generics);
//! // Returns: ["'a", "T", "U", "N"]
...
//!     let generic_names = extract_type_params_set(&sig.generics);
//!
...
//!     let impl_type_params = extract_type_params(&item_impl.generics);
//!     // Returns: ["A", "B"]
//!
...
//!
//! ```rust,ignore
//! use crate::analysis::generics::{extract_type_params_set, analyze_fn_bounds};
//!
...
//!
//! The module provides separate functions ([`extract_type_params`],
//! [`extract_all_params`], [`extract_type_params_set`],
...
use crate::{
	analysis::traits::extract_fn_type_from_bound, conversion::HmAst, core::config::Config,
};
...
pub fn extract_all_params(generics: &Generics) -> Vec<String> {
	generics
		.params
		.iter()
		.map(|p| match p {
...
///
/// ```rust,ignore
/// let type_params = extract_type_params_set(&sig.generics);
///
/// if type_params.contains("T") {
///     // "T" is a type variable
/// }
/// ```
pub fn extract_type_params_set(generics: &Generics) -> HashSet<String> {
	extract_type_params(generics).into_iter().collect()
}
		...
		if let GenericParam::Type(type_param) = param {
			...
			for bound in &type_param.bounds {
				if let TypeParamBound::Trait(trait_bound) = bound
					&& let Some(sig_ty) =
						extract_fn_type_from_bound(trait_bound, &fn_bounds, &generic_names, config)
				{
		...
		for predicate in &where_clause.predicates {
				...
				&& type_path.path.segments.len() == 1
			{
				...
				for bound in &predicate_type.bounds {
					if let TypeParamBound::Trait(trait_bound) = bound
						&& let Some(sig_ty) = extract_fn_type_from_bound(
							trait_bound,
							&fn_bounds,
							&generic_names,
							config,
