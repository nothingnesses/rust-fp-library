...
fn expand_define_effect(item_macro: ItemMacro) -> syn::Result<Vec<Item>> {
	...
	match input.effect_name.to_string().as_str() {
		"Reader" => parse_generated_items(expand_reader_effect_items()),
		_ => Err(syn::Error::new(
...
fn parse_generated_items(tokens: TokenStream) -> syn::Result<Vec<Item>> {
	Ok(syn::parse2::<GeneratedItems>(tokens)?.items)
}

fn expand_reader_effect_items() -> TokenStream {
	quote! {
		...
		/// ([`Run`](crate::types::effects::run::Run) /
		/// [`RunExplicit`](crate::types::effects::run_explicit::RunExplicit))
		/// use this cell for `ask`, while multi-shot wrappers
