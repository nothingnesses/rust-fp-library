...
use crate::hm_conversion::KindAssocTypeInput;
...
pub struct DocumentationBuilder<'a> {
    name: &'a Ident,
    assoc_types: &'a [KindAssocTypeInput],
}

impl<'a> DocumentationBuilder<'a> {
    /// Create a new documentation builder.
    pub fn new(name: &'a Ident, assoc_types: &'a [KindAssocTypeInput]) -> Self {
    ...
    fn build_summary(&self) -> String {
        let assoc_types_summary = self
            ...
            .map(|assoc| {
                ...
                let output_bounds_tokens = if output_bounds.is_empty() {
                    quote!()
                } else {
                    quote!(: #output_bounds)
                };
    ...
    fn format_assoc_type(&self, assoc: &KindAssocTypeInput) -> String {
        let ident = &assoc.ident;
...
mod tests {
    ...
    fn test_documentation_builder_single_assoc() {
        ...
        let assoc_type: KindAssocTypeInput = parse_quote!(type Of<A>;);
    ...
    fn test_documentation_builder_multiple_assoc() {
        ...
        let assoc1: KindAssocTypeInput = parse_quote!(type Of<A>;);
        let assoc2: KindAssocTypeInput = parse_quote!(type SendOf<B>: Send;);
    ...
    fn test_documentation_builder_with_bounds() {
        ...
        let assoc_type: KindAssocTypeInput = parse_quote!(type Of<'a, T: 'a + Clone>: Debug;);
        let assoc_types = vec![assoc_type];
