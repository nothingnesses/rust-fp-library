...
pub(super) fn reader_effect_items_tokens() -> TokenStream {
	quote! {
	...
	#[document_type_parameters("The pointer brand used for the continuation.", "The environment type.")]
		...
		E: Send + Sync + 'static,
	{
		...
		fn send_map<'a, A: Send + Sync + 'a, B: Send + Sync + 'a>(
			...
			match fa {
				SendReader::Ask(k) => SendReader::Ask(<P as ToDynSendFn>::new(move |e: E| f((*k)(e)))),
			}
		}
	}
